...
@section('content')
  ...
  @include('managers.includes.card', ['title' => 'Grupos'])
  ...
  <div class="widget-content searchable-container list">
        ...
        <div class="col-md-12 col-xl-12">
          <form class="position-relative form-search" action="{{ request()->fullUrl() }}" method="GET">
            <div class="row justify-content-between g-2 ">
              ...
              <div class="col-auto">
                <a href=" {{ route('manager.tickets.groups.create') }}" class="btn btn-primary">
    ...
    <div class="card card-body">
          ...
          <tbody>
              @foreach ($groups as $key => $group)
                <tr class="search-items">
                  <td>
                    <span class="usr-email-addr" data-email="{{ $group->title }}">{{ Str::upper( Str::lower($group->title) )  }}</span>
                  </td>
                  <td>
                      <span class="badge {{ $group->available == 1 ? 'bg-light-primary' : 'bg-light-secondary' }} rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">
                           {{ $group->available == 1 ? 'Publico' : 'Oculto' }}
                      </span>
                  </td>
                  <td>
                    <span class="usr-ph-no" data-phone="{{ date('Y-m-d', strtotime($group->updated_at)) }}">{{ date('Y-m-d', strtotime($group->updated_at)) }}</span>
                  </td>
                  <td class="text-left">
      ...
      <div class="result-body ">
        <span>Mostrar {{ $groups->firstItem() }}-{{ $groups->lastItem() }} de {{ $groups->total() }} resultados</span>
        <nav>
