  ...
  <div class="widget-content searchable-container list">
    ...
    <div class="card card-body">
        ...
        <table class="table search-table align-middle text-nowrap">
          ...
          @foreach ($tickets as $key => $ticket)
            <tr class="search-items">
              ...
              <td>
                <span class="usr-email-addr" data-email="{{ $ticket->title }}">{{ Str::upper( Str::lower($ticket->title) )  }}</span>
              </td>

              <td>
                  <span class="badge {{ $ticket->available == 1 ? 'bg-light-primary' : 'bg-light-secondary' }} rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">

                       {{ $ticket->available == 1 ? 'Publico' : 'Oculto' }}
                  </span>
              </td>
              <td>
                <span class="usr-ph-no" data-phone="{{ date('Y-m-d', strtotime($ticket->updated_at)) }}">{{ date('Y-m-d', strtotime($ticket->updated_at)) }}</span>
              </td>
              <td class="text-left">
      ...
      <div class="result-body ">
        <span>Mostrar {{ $tickets->firstItem() }}-{{ $tickets->lastItem() }} de {{ $tickets->total() }} resultados</span>
        <nav>
