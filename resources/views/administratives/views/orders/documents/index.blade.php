              ...
              <div class="col-auto">
                <div class="input-group">
                  <select class="form-select select2" name="available" data-minimum-results-for-search="Infinity">
                    ...
                    <option value="1" @isset($available) @if ($available==1) selected @endif @endisset>  Cargados</option>
                    <option value="0" @isset($available) @if ($available==0) selected  @endif @endisset>  Pendiente</option>
                  </select>
                </div>
              </div>
              <div class="col-auto">
          ...
          <tbody>
              @foreach ($documents as $key => $document)
                <tr class="search-items">
                  ...
                  <td>
                        {{ strtoupper($document->customer?->firstname) }}  {{ strtoupper($document->customer?->lastname) }}
                  </td>
                  <td>
                      <span class="badge {{ $document->upload_at!=null && $document->media->count()>0 == 1 ? 'bg-light-primary' : 'bg-light-secondary' }} rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">
                           {{ $document->upload_at!=null && $document->media->count()>0 ? 'Cargados' : 'Pendiente' }}
                      </span>
                  </td>
                  <td>
