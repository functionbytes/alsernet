            ...
            <div class="carousel-slider-wrapper carousel-6-columns text-center" id="carousel-6-columns-3" data-slick="{{ json_encode([
                ...
                @foreach ($brands as $brand)
                    <div class="brand-logo">
                        @if ($brand->website)
                            <a href="{{ $brand->website }}"><img alt="" class="img-grey-hover" src="{{ RvMedia::getImageUrl($brand->logo, null, false, RvMedia::getDefaultImage()) }}" alt="{{ $brand->name }}"/></a>
                        @else
                            <img alt="" class="img-grey-hover" src="{{ RvMedia::getImageUrl($brand->logo, null, false, RvMedia::getDefaultImage()) }}" alt="{{ $brand->name }}"/>
                        @endif
                    </div>
                @endforeach
            </div>
