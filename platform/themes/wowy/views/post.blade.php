@php
    ...
    Theme::layout($layout);
    Theme::set('pageTitle', $post->title);
    Theme::set('hasBreadcrumb', true);
@endphp

<div class="single-page">
    <div class="single-header style-2">
        <h1 class="mb-30">{{ $post->name ?? $post->title }}</h1>
                    ...
                    <article class="wow fadeIn animated hover-up mb-30">
                        <div class="post-thumb img-hover-scale">
                            <a href="{{ $relatedItem->url }}">
                                <img src="{{ RvMedia::getImageUrl($relatedItem->image, 'medium', false, RvMedia::getDefaultImage()) }}" alt="{{ $relatedItem->name ?? $relatedItem->title }}">
                            </a>
                            @php $firstCategory = $relatedItem->categories->first(); @endphp
                            @if ($firstCategory)
                        ...
                        <div class="entry-content-2">
                            <h3 class="post-title mb-15">
                                <a href="{{ $relatedItem->url }}">{{ $relatedItem->name ?? $relatedItem->title }}</a></h3>
