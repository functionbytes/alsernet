@if ($posts->isNotEmpty())
    <div class="single-header mb-80">
        <h1 class="font-xxl text-brand">{{ Theme::get('pageTitle', __('Blog')) }}</h1>

        <div class="entry-meta meta-1 font-xs mt-15 mb-15">
            <span class="post-by has-dot d-inline-block">{{ __(':count Categories', ['count' => \Modules\Blog\Models\BlogCategory::query()->published()->count()]) }}</span>
            <span class="post-on d-inline-block has-dot">{{ __(':count Articles', ['count' => \Modules\Blog\Models\BlogPost::query()->published()->count()]) }}</span>
            <span class="hit-count d-inline-block has-dot">{{ __(':count Views', ['count' => number_format(\Modules\Blog\Models\BlogPost::query()->published()->sum('views'))]) }}</span>
        </div>
    </div>

    <div class="loop-grid pr-30">
        <div class="row">
            @foreach ($posts as $post)
                @if ($loop->first)
                    <div class="col-12">
                        <article class="first-post mb-30 wow fadeIn animated hover-up">
                            <div class="img-hover-slide position-relative overflow-hidden">
                                <span class="top-right-icon bg-dark"><i class="fa fa-bookmark"></i></span>
                                <div class="post-thumb img-hover-scale">
                                    <a href="{{ $post->url }}">
                                        <img src="{{ RvMedia::getImageUrl($post->image, null, false, RvMedia::getDefaultImage()) }}" alt="{{ $post->name ?? $post->title }}">
                                    </a>
                                </div>
                            </div>
                            <div class="entry-content">
                                <div class="entry-meta meta-1 mb-30">
                                    @php $firstCategory = $post->categories->first(); @endphp
                                    @if ($firstCategory)
                                        <a class="entry-meta meta-0" href="{{ $firstCategory->url }}">
                                            <span class="post-in background4 text-brand font-xs">{{ $firstCategory->name }}</span>
                                        </a>
                                    @endif
                                    <div class="font-sm">
                                        <span><span class="mr-10 text-muted"><i class="fa fa-eye" aria-hidden="true"></i></span>{{ number_format($post->views) }}</span>
                                    </div>
                                </div>
                                <h2 class="post-title mb-20">
                                    <a href="{{ $post->url }}">{{ $post->name ?? $post->title }}</a></h2>
                                <p class="post-exerpt font-medium text-muted mb-30">{{ $post->description }}</p>
                                <div class="mb-20 entry-meta meta-2">
                                    <div class="font-xs ">
                                        <span class="post-by">{{ __('By') }} {{ $post->user?->name }}</span>
                                        <span class="post-on has-dot ms-2">{{ $post->created_at->translatedFormat('M d, Y') }}</span>
                                        <span class="time-reading has-dot ms-2">{{ __(':count mins read', ['count' => get_time_to_read($post)]) }}</span>
                                        <span class="hit-count has-dot ms-2">{{ __(':count Views', ['count' => number_format($post->views)]) }}</span>
                                    </div>
                                    <div class="float-right">
                                        <a href="{{ $post->url }}" class="read-more">{{ __('Read more') }} <span class="ml-10"><i class="fa fa-arrow-right fw-300 text-brand ml-5" aria-hidden="true"></i></span></a>
                                    </div>
                                </div>
                            </div>
                        </article>
                    </div>
                @else
                    <div class="col-lg-6">
                        <article class="wow fadeIn animated hover-up mb-30">
                            <div class="post-thumb img-hover-scale">
                                <a href="{{ $post->url }}">
                                    <img src="{{ RvMedia::getImageUrl($post->image, 'medium', false, RvMedia::getDefaultImage()) }}" alt="{{ $post->name ?? $post->title }}">
                                </a>
                                @php $firstCategory = $post->categories->first(); @endphp
                                @if ($firstCategory)
                                    <div class="entry-meta">
                                        <a class="entry-meta meta-2" href="{{ $firstCategory->url }}">{{ $firstCategory->name }}</a>
                                    </div>
                                @endif
                            </div>
                            <div class="entry-content-2">
                                <h3 class="post-title mb-15">
                                    <a href="{{ $post->url }}">{{ $post->name ?? $post->title }}</a></h3>
                                <p class="post-exerpt mb-30">{{ $post->description }}</p>
                                <div class="entry-meta meta-1 font-xs color-grey mt-10 pb-10">
                                    <div>
                                        <span class="post-on has-dot"> <i class="fa fa-clock"></i> {{ $post->created_at->translatedFormat('M d, Y') }}</span>
                                    </div>
                                    <a href="{{ $post->url }}" class="text-brand">{{ __('Read more') }} <i class="fa fa-arrow-right fw-300 text-brand ml-5"></i></a>
                                </div>
                            </div>
                        </article>
                    </div>
                @endif
            @endforeach
        </div>
    </div>

    {!! $posts->withQueryString()->links(Theme::getThemeNamespace() . '::partials.custom-pagination') !!}
@endif
