@php
    ...
    $cartItems = $cartService->getCartItems()->load('product');
    ...
    <div class="container">
        <div class="row">
            <div class="col-12 section--shopping-cart">
                <form class="form--shopping-cart" method="post" action="{{ route('cart.update', ['rowId' => 'bulk']) }}">
                    @csrf
                            ...
                            <tbody>
                                @foreach($cartItems as $key => $cartItem)
                                    @php
                                        ...
                                        $isOutOfStock = $product && ($product->quantity ?? 0) <= 0 && $product->with_storehouse_management;
                                    @endphp
                                        ...
                                        <tr>
                                            <td class="image product-thumbnail">
                                                ...
                                                <a href="{{ route('shop.product', $product->slug) }}">
                                                    <img src="{{ RvMedia::getImageUrl($product->featured_image, 'thumb', false, RvMedia::getDefaultImage()) }}" alt="{{ $product->name }}" />
                                                </a>
                                            </td>
                                            <td class="product-des product-name">
                                                <p class="product-name"><a href="{{ route('shop.product', $product->slug) }}">{{ $product->name }}  @if ($isOutOfStock) <span class="stock-status-label">({{ __('Out of stock') }})</span> @endif</a></p>
                                            ...
                                            <td class="text-right" data-title="{{ __('Subtotal') }}">
                                                <span>{{ format_price($product->final_price * $cartItem->qty) }}</span>
                                            </td>
                                            <td class="action" data-title="{{ __('Remove') }}">
                                                <a href="#" class="text-muted remove-cart-button" data-url="{{ route('cart.remove', $cartItem->id) }}"><i class="fa fa-trash-alt"></i></a>
                                            </td>
                                        </tr>
                                    @endif
                                @endforeach
                        ...
                        <div class="row mb-50">
                                        ...
                                        <div class="">
                                            <div class="coupon form-coupon-wrapper">
                                                <div class="form-row row justify-content-center">
                                                    ...
                                                    <div class="form-group col-lg-12">
                                                        <button class="btn btn-apply-coupon-code w-100" type="button" data-url="#"><i class="fa fa-bookmark mr-5"></i>{{ __('Apply') }}</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="col-lg-6 col-md-12">
                                <div class="border p-md-4 p-30 border-radius-10 cart-totals">
                                            ...
                                            <tbody>
                                                @if (EcommerceHelper::isTaxEnabled())
                                                    <tr>
                                                        ...
                                                        <td class="cart_total_amount"><span class="font-lg fw-900 text-brand">{{ format_price(0) }}</span></td>
                                                    </tr>
                                                @endif
                                                @if (($couponDiscountAmount ?? 0) > 0 && session('applied_coupon_code'))
                                                    <tr>
                                                        <td class="cart_total_label">{{ __('Coupon code: :code', ['code' => session('applied_coupon_code')]) }} (<small><a class="btn-remove-coupon-code text-danger" data-url="#" href="javascript:void(0)" data-processing-text="{{ __('Removing...') }}">{{ __('Remove') }}</a></small>)<span></td>
                                                ...
                                                @endif
                                                @php
                                                    $subTotal = $cartItems->sum(fn ($item) => ($item->product?->final_price ?? 0) * $item->qty);
                                                    $couponDiscountAmount = $couponDiscountAmount ?? 0;
                                                    $promotionDiscountAmount = $promotionDiscountAmount ?? 0;
                                                    $total = max(0, $subTotal - $promotionDiscountAmount - $couponDiscountAmount);
                                                @endphp
                                                <tr>
                                                    <td class="cart_total_label">{{ __('Total') }} <small>({{ __('Shipping fees not included') }})</small></td>
                                                    <td class="cart_total_amount"><strong><span class="font-xl fw-900 text-brand">{{ format_price($total) }}</span></span></strong></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    ...
                                    <div class="mt-10">
                                        <a  class="btn btn-primary w-100" href="{{ route('shop.index') }}"><i class="fa fa-arrow-left me-2"></i>{{ __('Continue Shopping') }}</a>
                                    </div>
                                </div>
