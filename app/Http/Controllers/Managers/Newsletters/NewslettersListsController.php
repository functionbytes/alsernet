...
class NewslettersListsController extends Controller
{
    ...
    public function index(Request $request){
            ...
            $lists = NewsletterList::orderBy('title' , 'desc');
            ...
            return view('managers.views.newsletters.lists.index')->with([
                'lists' => $lists,
                'available' => $available,
                'searchKey' => $searchKey,
            ]);
      ...
      public function create(){
          ...
          return view('managers.views.newsletters.lists.create')->with([
      ...
      public function edit($slack){
            ...
            return view('managers.views.newsletters.lists.edit')->with([
              'list' => $list,
                'availables' => $availables,
                'langs' => $langs,
            ]);
    ...
    public function details(Request $request,$slack){
        ...
        if ($searchKey) {
            $items = $items->join('newsletters', 'newsletters.id', '=', 'newsletter_lists_users.newsletter_id')
                ...
                ->select('newsletters.*' , 'newsletter_lists_users.id as id');
        }else{
            $items = $items->join('newsletters', 'newsletters.id', '=', 'newsletter_lists_users.newsletter_id')->select('newsletters.*' , 'newsletter_lists_users.id as id');
        }

        $items = $items->paginate(100);

        return view('managers.views.newsletters.lists.details')->with([
            'list' => $list,
            'items' => $items,
            'availables' => $availables,
            'searchKey' => $searchKey,
    ...
    public function includes(Request $request,$slack){
        ...
        $newsletters = Newsletter::whereNotIn('id', $itemsListIds)->latest()->pluck('email', 'id');

        return view('managers.views.newsletters.lists.includes')->with([
            'list' => $list,
    ...
    public function updateIncludes(Request $request){
        ...
        $list = NewsletterList::uid($request->list);
    ...
    public function update(Request $request){
        ...
        $list = NewsletterList::uid($request->uid);
      ...
      public function store(Request $request){
          ...
          if (NewsletterList::where('code', Str::upper($request->code))->exists()) {
              return response()->json([
                  'success' => false,
    ...
    public function destroy($slack){
        $list = NewsletterList::uid($slack);
        $list->delete();
        return redirect()->route('manager.newsletters.lists');
    }
