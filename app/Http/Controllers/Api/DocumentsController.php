...
class DocumentsController extends ApiController
{
    ...
    public function documentUpload(Request $request)
    {
        ...
        if (! $document) {
            return response()->json([
                'status'  => 'failed',
                'message' => 'No se encontró ningún documento con este UID.',
            ], 404);
        }
        ...
        if ($request->hasFile('file')) {
            ...
            foreach ($request->file('file') as $uploadedFile) {
                try {
                    ...
                    $document->addMedia($uploadedFile)
                        ->preservingOriginal()
                        ->toMediaCollection('documents', 'media');

                    Log::info('Archivo añadido con éxito: ' . $uploadedFile->getClientOriginalName());
                } catch (\Exception $e) {
                    Log::error('Error al añadir media: ' . $e->getMessage() . ' - Archivo: ' . $uploadedFile->getClientOriginalName());
                    return response()->json([
                        'status' => 'failed',
                        'message' => 'Error al procesar el archivo: ' . $uploadedFile->getClientOriginalName() . ' - ' . $e->getMessage()
                    ], 500);
                }
            }

            $document->upload_at = now()->setTimezone('Europe/Madrid');
            $document->save();

            return response()->json(['status' => 'success'], 200);
        }

        return response()->json(['status' => 'failed', 'message' => 'No se ha subido ningún archivo.'], 400);
    }
