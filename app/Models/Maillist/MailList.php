...
use App\Models\SubscriberFields;
...
class MailList extends Model
{
    ...
    {
        return $this->belongsToMany('App\Models\Campaign', 'campaigns_lists_segments', 'mail_list_id', 'campaign_id');
    }
    ...
    public function updateOrCreateFieldsFromRequest($request)
    {
        ...
        foreach ($request->fields as $key => $item) {
            ...
            if ($this->getEmailField()->uid != $item['uid']) {
                ...
                // Check tag exsit
                $tag = \App\Models\Field::formatTag($item['tag']);
        ...
        foreach ($request->fields as $uid => $item) {
            $field = \App\Models\Field::findByUid($item['uid']);
            if (!$field) {
            ...
            if ($this->getEmailField()->uid != $field->uid) {
                // save exsit field
                $item['tag'] = \App\Models\Field::formatTag($item['tag']);
                $field->fill($item);
                $field->save();
                ...
                if (isset($item['options'])) {
                    foreach ($item['options'] as $key2 => $item2) {
                        $option = new \App\Models\FieldOption($item2);
