...
$__env->startSection('title', 'Gestor de Medios'); ?>

<?php $__env->startPush('styles'); ?>
<link rel="stylesheet" href="<?php echo e(asset('modules/Media/css/media-manager.css')); ?>">
<style>
...
<?php if (ExtendBlade::isRenderingLivewireComponent()) { ?><!--[if BLOCK]><![endif]--><?php } ?><?php if ($pickerMode) { ?>
...
<div id="mediaManagerApp">
    ...
    <?php echo $__env->make('core::components.card', ['title' => 'Gestor de Medios'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <select id="mediaDiskSelect" class="form-select media-disk-select">
                                <?php if (ExtendBlade::isRenderingLivewireComponent()) { ?><!--[if BLOCK]><![endif]--><?php SupportCompiledWireKeys::openLoop(); ?><?php } ?><?php $__currentLoopData = $availableDisks;
$__env->addLoop($__currentLoopData);
foreach ($__currentLoopData as $disk) {
    ...
    $loop = $__env->getLastLoop(); ?><?php if (ExtendBlade::isRenderingLivewireComponent()) { ?><?php SupportCompiledWireKeys::startLoopIteration(); ?><?php } ?>
                                <option value="<?php echo e($disk['name']); ?>"><?php echo e($disk['label']); ?> (<?php echo e($disk['driver']); ?>)</option>
    ...
    createApp({
        data() {
            return {
                ...
                activeDisk: '<?php echo e($activeDisk); ?>',
                ...
                selectionMode: false,
                pickerMode: <?php echo e($pickerMode ? 'true' : 'false'); ?>,
                // Preview lightbox
                previewItem: null,
                previewItemName: '',
                previewType: '',
                ...
                activityLogFilter: '',
                activityLogEntries: [],
                isAdmin: <?php echo e(auth()->user()?->can('media.manage') ? 'true' : 'false'); ?>,
            }
        },
        computed: {
            getViewTitle() {
        ...
        methods: {
            async loadMedia(folderId = 0, page = 1) {
                ...
                this.hasMore = true;
                try {
                    const response = await fetch(`<?php echo e(route('media.list')); ?>?folder_id=${folderId}&page=1&per_page=30&search=${this.searchQuery}`);
            ...
            async changeActiveDisk(disk) {
                ...
                try {
                    const response = await fetch('<?php echo e(route('media.set-disk')); ?>', {
                        method: 'POST',
                        headers: {
            ...
            async handleFileUpload(event) {
                ...
                try {
                    for (let file of files) {
                        ...
                        const response = await fetch('<?php echo e(route('media.files.upload')); ?>', {
                            method: 'POST',
                            body: formData,
                            headers: {
            ...
            async confirmCreateFolder() {
                ...
                try {
                    const response = await fetch('<?php echo e(route('media.folders.create')); ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            async confirmRenameFolder() {
                ...
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/folders')); ?>/${this.renameFolderData.id}/rename`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            async deleteFolder(folder) {
                ...
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/folders')); ?>/${folder.id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
            ...
            async deleteFile(file) {
                ...
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/files')); ?>/${file.id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
            ...
            async copyFile(file) {
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/files')); ?>/${file.id}/copy`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
            ...
            async confirmRenameFile() {
                ...
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/files')); ?>/${this.renameFileData.id}/rename`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            copyFileLink(file) {
                this.copyLinkData.url = file.public_url || `<?php echo e(url('/media')); ?>/${file.url}`;
            ...
            async confirmDeleteFile() {
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/files')); ?>/${this.deleteFileData.id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
            ...
            copyIndirectLink(file) {
                const indirectUrl = `<?php echo e(url('panel/media/files')); ?>/${file.id}`;
            ...
            async loadTrash() {
                ...
                this.hasMore = true;
                try {
                    const response = await fetch(`<?php echo e(route('media.list')); ?>?folder_id=0&page=1&per_page=30&search=${this.searchQuery}&view=trash`);
                    const data = await response.json();
            ...
            async loadRecent() {
                ...
                this.hasMore = true;
                try {
                    const response = await fetch(`<?php echo e(route('media.list')); ?>?folder_id=0&page=1&per_page=30&search=${this.searchQuery}&view=recent`);
                    const data = await response.json();
            ...
            async loadFavorites() {
                ...
                this.hasMore = true;
                try {
                    const response = await fetch(`<?php echo e(route('media.list')); ?>?folder_id=0&page=1&per_page=30&search=${this.searchQuery}&view=favorites`);
                    const data = await response.json();
            ...
            async emptyTrash() {
                ...
                try {
                    const response = await fetch('<?php echo e(route('media.trash.empty')); ?>', {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
            ...
            async toggleFavorite(file) {
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/files')); ?>/${file.id}/toggle-favorite`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
            ...
            async restoreFile(file) {
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/files')); ?>/${file.id}/restore`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
            ...
            async restoreFolder(folder) {
                try {
                    const response = await fetch(`<?php echo e(url('panel/media/folders')); ?>/${folder.id}/restore`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
            ...
            async confirmUploadFromUrl() {
                ...
                try {
                    const response = await fetch('<?php echo e(route('media.files.upload-url')); ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            async uploadFile(index) {
                ...
                try {
                    ...
                    await new Promise((resolve, reject) => {
                        ...
                        xhr.open('POST', '<?php echo e(route('media.files.upload')); ?>');
            ...
            async bulkRequest(action, folderId = null) {
                ...
                for (const batch of batches) {
                    try {
                        await $.ajax({
                            url: '<?php echo e(route('media.bulk-action')); ?>',
                            method: 'POST',
            ...
            async loadQuota() {
                try {
                    const res = await $.get('<?php echo e(url('panel/media/quota')); ?>');
            ...
            async handleFolderUpload(event) {
                ...
                for (const path of sortedPaths) {
                    ...
                    try {
                        const res = await $.ajax({
                            url: '<?php echo e(route('media.folders.create')); ?>',
                            method: 'POST',
                ...
                for (const [path, pathFiles] of foldersMap.entries()) {
                    ...
                    for (const file of pathFiles) {
                        ...
                        try {
                            await $.ajax({
                                url: '<?php echo e(route('media.files.upload')); ?>',
                                method: 'POST',
                                data: fd,
                                processData: false,
                                contentType: false,
            ...
            async loadDuplicates() {
                ...
                try {
                    const res = await $.get('<?php echo e(url('panel/media/duplicates')); ?>', { mode: this.duplicatesMode });
            ...
            async loadMore() {
                ...
                this.currentPage++;
                try {
                    const res = await $.get('<?php echo e(route('media.list')); ?>', {
            ...
            async openHeatmap() {
                ...
                const res = await $.get('<?php echo e(route('media.list')); ?>', { view: 'recent' }).catch(() => ({ files: [] }));
                const days = [];
                for (let i = 89; i >= 0; i--) {
                    const d = new Date();
...
<?php echo $__env->make('layouts.theme', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/developerts/Herd/system/modules/Media/app/Providers/../../resources/views/index.blade.php ENDPATH**/ ?>