<script src="<?php echo e(url('others/filemanager/js/jquery-ui-1.12.1/jquery-ui.min.js')); ?>"></script>
...
<script>
    ...
    const urls = {
        fieldsStore:  '<?php echo e(route('settings.forms.fields.store', $form)); ?>',
        fieldsReorder:'<?php echo e(route('settings.forms.fields.reorder', $form)); ?>',
        fieldEditBase:'<?php echo e(route('settings.forms.fields.index', $form)); ?>',
        preview:      '<?php echo e(route('settings.forms.preview', $form)); ?>',
    };
    ...
    // ─── Vista previa en tab ───────────────────────────────────────────────────
    const previewPublicUrl = '<?php echo e(route('forms.public.preview.public', [$form, $previewToken])); ?>';
    ...
    $(document).on('change', '.btn-protection-toggle', function () {
        ...
        $.ajax({
            url: '<?php echo e(route('settings.forms.protection', $form)); ?>',
            method: 'PATCH',
            data: payload,
            headers: { 'X-CSRF-TOKEN': csrfToken },
            success: function () {
    ...
    $('#btnAddStep').on('click', function () {
        ...
        $.ajax({
            url: '<?php echo e(route('settings.forms.steps.add', $form)); ?>',
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': csrfToken },
            success: function (res) {
    ...
    const htmlSourceUrl = '<?php echo e(route('settings.forms.html-source', $form)); ?>';
    ...
    function saveCodeEditors() {
        ...
        $.ajax({
            url: '<?php echo e(route('settings.forms.update', $form)); ?>',
            method: 'POST',
            data: {
                _method: 'PATCH',
                _token: csrfToken,
                name: '<?php echo e(addslashes($form->name)); ?>',
    ...
    window._formFields = <?php echo json_encode($form->fields->where('is_visible', true)->map(fn ($f) => ['key' => $f->key, 'label' => $f->label])->values()) ?>;
    ...
    // ─── Formula shortcodes ─────────────────────────────────────────────────────
    const allFormFields = <?php echo json_encode($form->fields->map(fn ($f) => ['key' => $f->name, 'label' => $f->label, 'type' => $f->type])->values()) ?>;
...
<?php /**PATH /Users/developerts/Herd/system/modules/Forms/resources/views/forms/partials/_edit-scripts.blade.php ENDPATH**/ ?>