...
You'll find plenty to play with in the [examples](https://github.com/PHPMailer/PHPMailer/tree/master/examples) folder, which covers many common scenarios including sending through Gmail, building contact forms, sending to Mailer lists, and more.

If you are re-using the instance (e.g. when sending to a Mailer list), you may need to clear the recipient list to avoid sending duplicate messages. See [the Mailer list example](https://github.com/PHPMailer/PHPMailer/blob/master/examples/Mailer_list.phps) for further guidance.
