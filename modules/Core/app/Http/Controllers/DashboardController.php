            ...
            try {
                if (class_exists(Review::class)) {
                    $critical = Review::whereNull('google_reply_text')
                        ...
                        ->count();
                    if ($critical > 0) {
                        $items[] = ['type' => 'reviews_critical', 'severity' => 'danger', 'icon' => 'fas fa-star', 'message' => $critical.' reseña'.($critical > 1 ? 's' : '').' de 1-2 estrellas sin responder', 'link' => route('reviews.index')];
                    }
                }
            } catch (\Throwable $e) {
