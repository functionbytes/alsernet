...
@php
    // Mismo criterio "opcional, cae si está desactualizado" que tickets-app.js/css
    // en el listado (ver index.blade.php y scripts/build-tickets-app.mjs).
    // ticket-detail.js es un único IIFE autocontenido (no dividido en varios
    // ficheros), así que aquí no hace falta manifest ni concatenar nada:
    // ticket-detail.min.js se sirve SOLO si existe Y es más reciente que su
    // única fuente; si no, cae al fichero sin minificar de siempre.
    $detailSrcPath = public_path('modules/helpdesktickets/js/ticket-detail.js');
    $detailMinPath = public_path('modules/helpdesktickets/js/ticket-detail.min.js');
    $detailSrcMtime = @filemtime($detailSrcPath);
    $detailMinMtime = @filemtime($detailMinPath);
    $useDetailMin = $detailMinMtime !== false && $detailSrcMtime !== false && $detailMinMtime >= $detailSrcMtime;
@endphp
@if ($useDetailMin)
<script src="{{ asset('modules/helpdesktickets/js/ticket-detail.min.js') }}?v={{ $detailMinMtime }}"></script>
@else
<script src="{{ asset('modules/helpdesktickets/js/ticket-detail.js') }}?v={{ $detailSrcMtime }}"></script>
@endif
@endpush
