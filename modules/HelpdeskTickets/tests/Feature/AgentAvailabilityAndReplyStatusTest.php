...
/**
 * Comportamientos del panel de tickets al responder:
 *
 ...
 *  3. Quien responde a un ticket SIN dueño se convierte en el dueño (ajuste
 *     tickets.assign_on_reply, pedido el 8-sep-2026) — pero solo si estaba sin
 *     asignar: no se le quita el ticket a un compañero por contestar.
 */
class AgentAvailabilityAndReplyStatusTest extends TestCase
{
    ...
    protected function tearDown(): void
    {
        ...
        foreach (['tickets.status_on_reply', 'tickets.status_on_reply_slug', 'tickets.assign_on_reply'] as $clave) {
    ...
    // ── Asignación al responder ────────────────────────────────

    public function test_responder_a_un_ticket_sin_asignar_te_lo_asigna(): void
    {
        Event::fake([TicketAssigned::class]);

        $this->assertNull($this->ticket->assignee_id);

        $this->enviar('Hola, ya te ayudo con esto.', false);

        $this->assertSame($this->agente->id, $this->ticket->fresh()->assignee_id);
        Event::assertDispatched(
            TicketAssigned::class,
            fn (TicketAssigned $e) => $e->agent->id === $this->agente->id
        );
    }

    public function test_responder_a_un_ticket_ya_asignado_no_se_lo_quita_a_otro_agente(): void
    {
        Event::fake([TicketAssigned::class]);

        $duenoOriginal = User::factory()->create();
        $this->ticket->update(['assignee_id' => $duenoOriginal->id]);

        // Otro agente (distinto del dueño) contesta el mismo ticket.
        $this->enviar('Yo también reviso este caso.', false);

        $this->assertSame($duenoOriginal->id, $this->ticket->fresh()->assignee_id);
        Event::assertNotDispatched(TicketAssigned::class);
    }

    public function test_una_nota_interna_no_asigna_nada(): void
    {
        $this->assertNull($this->ticket->assignee_id);

        $this->enviar('Nota para el equipo, el cliente no la ve.', true);

        $this->assertNull($this->ticket->fresh()->assignee_id);
    }

    public function test_con_el_ajuste_de_asignacion_desactivado_no_se_asigna(): void
    {
        Setting::set('tickets.assign_on_reply', false, 'tickets');
