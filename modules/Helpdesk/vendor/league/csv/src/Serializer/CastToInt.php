...
final class CastToInt implements TypeCasting
{
    ...
    private function init(ReflectionProperty|ReflectionParameter $reflectionProperty): bool
    {
        ...
        foreach (Type::list($reflectionProperty) as $found) {
            ...
            if ($type === null && $found[0]->isOneOf(Type::Mixed, Type::Int, Type::Float)) {
                $type = $found;
            }
        }

        $type !== null || throw throw MappingFailed::dueToTypeCastingUnsupportedType($reflectionProperty, $this, 'int', 'float', 'null', 'mixed');
