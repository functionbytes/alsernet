...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::connection($this->connection)->table('helpdesk_customers', function (Blueprint $table) {
            if (! Schema::connection($this->connection)->hasColumn('helpdesk_customers', 'portal_token')) {
                $table->string('portal_token', 64)->nullable()->unique()->after('internal_notes');
            }

            if (! Schema::connection($this->connection)->hasColumn('helpdesk_customers', 'portal_token_expires_at')) {
                $table->timestamp('portal_token_expires_at')->nullable()->after('portal_token');
            }

            if (! Schema::connection($this->connection)->hasColumn('helpdesk_customers', 'portal_password')) {
    ...
    public function down(): void
    {
        ...
        Schema::connection($this->connection)->table('helpdesk_customers', function (Blueprint $table) {
            $columns = array_filter(
                ...
                fn (string $col) => Schema::connection($this->connection)->hasColumn('helpdesk_customers', $col)
            );
