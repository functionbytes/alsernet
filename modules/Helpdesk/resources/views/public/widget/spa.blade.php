...
<head>
    ...
    @if(file_exists(public_path('build-helpdesk/widget/main.css')))
        <link rel="stylesheet" href="{{ asset('build-helpdesk/widget/main.css') }}">
    @endif
...
<body>
    ...
    <div id="widget-root" data-launcher="false" data-preview="false" data-inline="true"></div>

    @if(file_exists(public_path('build-helpdesk/widget/main.js')))
        <script type="module" src="{{ asset('build-helpdesk/widget/main.js') }}"></script>
    @else
        <div class="widget-not-built">
            <strong>Helpdesk widget bundle not built yet.</strong>
            <p style="margin: 8px 0 0;">Run <code>cd modules/Helpdesk && npm install && npm run widget:build</code> to compile the React bundle.</p>
        </div>
    @endif
</body>
</html>
