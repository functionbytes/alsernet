...
@push('scripts')
    {{-- Datos puros (sin lógica) para el listener global movido a conversations-list.js
         y el auto-bind de conversación movido a conversations-thread.js: bandejas
         visibles del agente (para las suscripciones Echo por bandeja) y la
         conversación preseleccionada por query string (?selected=). Mismo patrón que
         window.BvLazyModalScripts en partials/modals.blade.php. --}}
    <script>
    ...
    {{-- conversations.js (7.138 líneas) se dividió por responsabilidad para que sea más
         mantenible. Ahora pasan por Vite (mismo pipeline que resources/js/app.js): se
         sirven minificados con hash de caché real en vez del filemtime() manual anterior.
         El ORDEN sí importa: conversations-core.js debe ir primero porque expone en
         window los helpers que usan los demás (openModal, escapeHtml, refreshInboxList,
         ...). @vite() conserva el orden del array y cada entrada se emite como
         <script type="module"> sin async, que el navegador ejecuta en orden de
         documento — igual que los <script defer> que sustituye. --}}
    @vite([
