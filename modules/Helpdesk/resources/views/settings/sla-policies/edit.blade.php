...
@section('content')
    ...
    <div class="row g-3">
        <div class="col-12 col-lg-8">
            <div class="card">
                <form action="{{ route('settings.helpdesk.sla-policies.update', $slaPolicy) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="card-header border-bottom p-3">
                        <h5 class="mb-0 fw-bold">Editar politica SLA</h5>
                        <small class="text-muted">{{ $slaPolicy->name }}</small>
                    </div>
