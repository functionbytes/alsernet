...
@push('scripts')
<script>
window.SurveysIndexConfig = {
    flash: { success: @json(session('success')), error: @json(session('error')) },
    bulkActionUrl: @json(route('settings.helpdesk.surveys.bulk-action')),
};
</script>
<script src="{{ asset('core/js/bulk.js?v=2') }}"></script>
<script>window.HdSettingsCommonSkipAutoInit = true;</script>
<script src="{{ asset('vendor/helpdesk/settings/settings-common.js') }}?v={{ @filemtime(public_path('vendor/helpdesk/settings/settings-common.js')) }}" defer></script>
<script src="{{ asset('vendor/helpdesk/settings/surveys-index.js') }}?v={{ @filemtime(public_path('vendor/helpdesk/settings/surveys-index.js')) }}" defer></script>
@endpush
