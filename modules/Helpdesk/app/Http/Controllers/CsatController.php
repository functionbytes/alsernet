...
class CsatController extends Controller
{
    ...
    public function show(string $token): View
    {
        ...
        if ($survey->answered_at !== null) {
            return view('helpdesk::public.csat.thanks', ['rating' => $survey]);
        }

        if ($survey->expires_at && $survey->expires_at->isPast()) {
            return view('helpdesk::public.csat.expired', ['rating' => $survey]);
        }

        return view('helpdesk::public.csat.show', ['rating' => $survey]);
    }
