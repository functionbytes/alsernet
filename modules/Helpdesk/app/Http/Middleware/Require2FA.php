        ...
        if (! $this->twoFactorService->isEnabled($user)) {
            if ($request->expectsJson()) {
                return response()->json([
                    ...
                    'redirect' => route('helpdesk.2fa.setup'),
                ], 403);
            }

            return redirect()->route('helpdesk.2fa.setup')
        ...
        if (! $request->session()->get('2fa_passed')) {
            if ($request->expectsJson()) {
                return response()->json([
                    ...
                    'redirect' => route('helpdesk.2fa.challenge'),
                ], 403);
            }

            return redirect()->route('helpdesk.2fa.challenge')
