...
return [
    ...
    */
    ...
    // Sigue siendo configurable por Settings ('helpdeskemailactivity.per_page'),
    ...
    */
    ...
    | apertura en el HTML de los envíos que lo soportan (ver
    | tracked_modules). No afecta al histórico ya registrado en
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Módulos con seguimiento de apertura y clic
    |--------------------------------------------------------------------------
    | El seguimiento NO se aplica a todo el correo saliente a propósito: los
    | resets de contraseña, los OTP o las notificaciones internas no tienen por
    | qué llevar píxel. Solo se sigue lo que aparece en esta lista.
    |
    | Antes era una comparación literal contra 'HelpdeskTickets' dentro de
    | LogEmailQueued, así que cualquier otro módulo que midiera aperturas
    | mostraba 0% para siempre sin ninguna pista de por qué.
    |
    | Añadir un módulo aquí implica que sus correos llevan píxel y enlaces
    | reescritos: hazlo solo para correos comerciales o informativos, nunca
    | para los que transportan credenciales.
    */
    'tracked_modules' => [
        'HelpdeskTickets',
        'HelpdeskBirthday',
    ...
    */
    ...
    /*
    | Inspector — pestaña "Compatibilidad": descargar las hojas de estilo
    | remotas (<link rel="stylesheet">) antes de analizar el HTML.
    |
    | Desactivado por defecto, y no por comodidad: analizar un correo pasaría a
    | hacer peticiones salientes a dominios de terceros, y el análisis se lanza
    | solo al abrir cada mensaje. A cambio se gana poco — ningún cliente de
    | correo real (Gmail, Outlook, Apple Mail) carga CSS externo, así que esas
    | reglas no llegan a aplicarse en la bandeja de nadie.
    |
    | Actívalo si envías correo a clientes de escritorio antiguos que sí lo
    | soportan y quieres que esas reglas cuenten en la puntuación. Las descargas
