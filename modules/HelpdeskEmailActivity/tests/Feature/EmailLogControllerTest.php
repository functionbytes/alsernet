...
class EmailLogControllerTest extends TestCase
{
    ...
    {
        return tap(User::factory()->create())->givePermissionTo(['helpdeskemailactivity.view', 'helpdeskemailactivity.manage']);
    }

    public function test_index_requires_authentication(): void
    {
        $this->get(route('helpdeskemailactivity.index'))->assertRedirect();
    }
    ...
    public function test_index_renders_with_logs_and_stats(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index'))
            ->assertOk()
            ->assertViewIs('helpdeskemailactivity::emails.index')
    ...
    public function test_index_filters_by_status_and_module(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['status' => 'failed']))
            ->assertOk()
            ->assertSee('Weekly digest')
            ->assertDontSee('Reset link');

        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['module' => 'Auth']))
            ->assertOk()
    ...
    {
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.show', Str::orderedUuid()))
            ->assertNotFound();
    }
    ...
    public function test_show_returns_only_the_detail_fragment_for_ajax_requests(): void
    {
        ...
        $response->assertOk()->assertViewIs('helpdeskemailactivity::emails.partials.detail-panel');
    ...
    public function test_show_returns_the_full_page_without_ajax_header(): void
    {
        ...
        $response = $this->actingAs($this->viewer())->get(route('helpdeskemailactivity.show', $log->uid));

        $response->assertOk()->assertViewIs('helpdeskemailactivity::emails.index');
    ...
    public function test_index_shows_engagement_counts_for_tracked_logs(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['search' => 'Tracked in list']))
            ->assertOk()
    ...
    public function test_index_filters_by_engagement(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['engagement' => 'opened', 'search' => 'one']))
            ->assertSee('Opened one')
            ->assertDontSee('Not opened one')
            ->assertDontSee('Untracked one');

        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['engagement' => 'not_opened', 'search' => 'one']))
            ->assertSee('Not opened one')
            ->assertDontSee('Opened one')
            ->assertDontSee('Untracked one');

        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['engagement' => 'clicked', 'search' => 'one']))
            ->assertSee('Opened one')
            ->assertDontSee('Not opened one');

        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['engagement' => 'not_clicked', 'search' => 'one']))
            ->assertSee('Not opened one')
            ->assertDontSee('Opened one');
    }
    ...
    public function test_destroy_requires_manage_permission(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->delete(route('helpdeskemailactivity.destroy', $log->uid))
            ->assertForbidden();
    ...
    public function test_manager_can_delete_a_log(): void
    {
        ...
        $this->actingAs($this->manager())
            ->delete(route('helpdeskemailactivity.destroy', $log->uid))
            ->assertRedirect(route('helpdeskemailactivity.index'))
    ...
    public function test_manager_can_bulk_delete_logs(): void
    {
        ...
        $this->actingAs($this->manager())
            ->delete(route('helpdeskemailactivity.bulk-destroy'), ['uids' => $logs->pluck('uid')->all()])
            ->assertRedirect()
            ->assertSessionHas('success');
    ...
    {
        $this->actingAs($this->manager())
            ->delete(route('helpdeskemailactivity.bulk-destroy'), ['uids' => []])
    ...
    public function test_export_returns_a_csv_download(): void
    {
        ...
        $response = $this->actingAs($this->viewer())->get(route('helpdeskemailactivity.export'));
    ...
    /**
     * CSV/formula injection: el asunto puede venir de un correo entrante o de
     * un formulario público, y Excel/LibreOffice ejecutan como fórmula toda
     * celda que empiece por = + - @. El export debe anteponer un apóstrofo.
     */
    public function test_export_neutralises_spreadsheet_formulas(): void
    {
        EmailLog::factory()->create([
            'subject' => '=HYPERLINK("http://evil.test","gana un premio")',
            'from_address' => '+34600000000@example.test',
        ]);

        $csv = $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.export'))
            ->assertOk()
            ->streamedContent();

        $this->assertStringContainsString('\'=HYPERLINK', $csv);
        $this->assertStringContainsString("'+34600000000@example.test", $csv);
        // El valor sigue ahí íntegro: se antepone el apóstrofo, no se recorta.
        $this->assertStringContainsString('gana un premio', $csv);
    }
    ...
    public function test_export_includes_opens_and_clicks_columns(): void
    {
        ...
        $response = $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.export', ['search' => 'Exported']));

        $rows = array_map('str_getcsv', array_filter(explode("\n", trim($response->streamedContent()))));
        $header = array_shift($rows);

        $opensCol = array_search(__('helpdeskemailactivity::emaillog.csv.opens'), $header, true);
        $clicksCol = array_search(__('helpdeskemailactivity::emaillog.csv.clicks'), $header, true);
        $subjectCol = array_search(__('helpdeskemailactivity::emaillog.csv.subject'), $header, true);
    ...
    public function test_resend_dispatches_job_for_manager(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.resend', $log->uid))
            ->assertRedirect()
            ->assertSessionHas('success');
    ...
    public function test_index_ignores_invalid_date_filters(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['date_from' => 'not-a-date', 'date_to' => '9999-99-99']))
            ->assertOk()
    ...
    public function test_index_ignores_array_date_filters(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index').'?date_from[]=2026-01-01&date_to[]=x')
            ->assertOk();
    }
    ...
    public function test_index_applies_valid_date_filters(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['date_from' => now()->subDay()->toDateString()]))
            ->assertOk()
    ...
    public function test_bulk_resend_skips_redacted_and_truncated_logs(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.bulk-resend'), [
    ...
    public function test_resend_creates_a_log_row_traceable_to_the_original(): void
    {
        ...
        $this->assertSame('HelpdeskEmailActivity', $resend->module);
    ...
    public function test_bulk_destroy_rejects_non_uuid_values(): void
    {
        ...
        $this->actingAs($this->manager())
            ->delete(route('helpdeskemailactivity.bulk-destroy'), ['uids' => ['not-a-uuid']])
    ...
    public function test_index_search_finds_by_recipient(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['search' => 'needle@example.test']))
            ->assertOk()
    ...
    public function test_resend_to_alternative_address_dispatches_job_with_override(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.resend', $log->uid), ['to' => 'alt@example.test'])
            ->assertRedirect()
            ->assertSessionHas('success');
    ...
    public function test_resend_rejects_invalid_alternative_address(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.resend', $log->uid), ['to' => 'not-an-email'])
    ...
    public function test_bulk_resend_requires_manage_permission(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->post(route('helpdeskemailactivity.bulk-resend'), ['uids' => [$log->uid]])
            ->assertForbidden();
    ...
    public function test_download_returns_html_attachment(): void
    {
        ...
        $response = $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.download', $log->uid));
    ...
    public function test_manager_can_purge_body(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.purge-body', $log->uid))
            ->assertRedirect()
            ->assertSessionHas('success');
    ...
    public function test_index_computes_stats_delta_for_active_date_filter(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['date_from' => '2020-01-10', 'date_to' => '2020-01-16']))
            ->assertOk()
            ->assertViewHas('statsDelta', function ($delta) {
                $total = $delta['total'];
    ...
    public function test_index_open_rate_delta_has_no_data_for_previous_period_without_tracking(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['date_from' => '2020-03-10', 'date_to' => '2020-03-16']))
            ->assertOk()
            ->assertViewHas('statsDelta', function ($delta) {
    ...
    public function test_export_selected_returns_csv_for_chosen_uids_only(): void
    {
        ...
        $response = $this->actingAs($this->viewer())
            ->post(route('helpdeskemailactivity.export-selected'), ['uids' => [$selected->uid]]);
    ...
    public function test_export_selected_requires_view_permission(): void
    {
        ...
        $this->actingAs(User::factory()->create())
            ->post(route('helpdeskemailactivity.export-selected'), ['uids' => [$log->uid]])
            ->assertForbidden();
    }
    ...
    {
        $this->actingAs($this->viewer())
            ->post(route('helpdeskemailactivity.export-selected'), ['uids' => []])
            ->assertSessionHasErrors('uids');
    }
    ...
    {
        $this->actingAs($this->viewer())
            ->post(route('helpdeskemailactivity.export-selected'), ['uids' => ['not-a-uuid']])
            ->assertSessionHasErrors('uids.0');
    }
    ...
    public function test_show_exposes_position_and_prev_next_navigation_independent_of_pagination(): void
    {
        ...
        $response = $this->actingAs($this->viewer())->get(route('helpdeskemailactivity.show', [
            'emailLog' => $target->uid,
            'module' => $module,
            'sort_by' => 'subject',
            'sort_dir' => 'asc',
    ...
    public function test_show_prev_and_next_are_null_when_it_is_the_only_matching_log(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.show', ['emailLog' => $log->uid, 'module' => $module]))
            ->assertOk()
    ...
    public function test_index_navigation_is_null_without_any_matching_log(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['module' => $module]))
            ->assertOk()
    ...
    public function test_index_exposes_a_plain_text_body_snippet_per_row(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['search' => 'Snippet source subject']))
            ->assertOk()
    ...
    public function test_index_body_snippet_is_empty_when_the_body_was_redacted(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['search' => 'Redacted snippet subject']))
            ->assertOk()
    ...
    public function test_show_computes_recipient_aggregates_for_the_sidebar_card(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.show', $target->uid))
            ->assertOk()
    ...
    {
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.trash.index'))
            ->assertForbidden();
    }
    ...
    public function test_trash_lists_only_soft_deleted_logs(): void
    {
        ...
        $this->actingAs($this->manager())
            ->get(route('helpdeskemailactivity.trash.index'))
            ->assertOk()
            ->assertViewIs('helpdeskemailactivity::emails.trash')
    ...
    public function test_manager_can_restore_a_log_from_trash(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.trash.restore', $log->uid))
            ->assertRedirect()
            ->assertSessionHas('success');
    ...
    public function test_manager_can_bulk_restore_logs(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.trash.bulk-restore'), ['uids' => $logs->pluck('uid')->all()])
            ->assertRedirect()
            ->assertSessionHas('success');
    ...
    public function test_bulk_restore_requires_manage_permission(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->post(route('helpdeskemailactivity.trash.bulk-restore'), ['uids' => [$log->uid]])
            ->assertForbidden();
    ...
    public function test_manager_can_force_destroy_a_log_from_trash(): void
    {
        ...
        $this->actingAs($this->manager())
            ->delete(route('helpdeskemailactivity.trash.force-destroy', $log->uid))
            ->assertRedirect(route('helpdeskemailactivity.trash.index'))
            ->assertSessionHas('success');
    ...
    public function test_resolve_bounce_dispatches_resend_to_corrected_address(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.resolve-bounce', $log->uid), ['to' => 'fixed@example.test'])
            ->assertRedirect()
            ->assertSessionHas('success');
    ...
    public function test_resolve_bounce_suppresses_old_address_when_requested(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.resolve-bounce', $log->uid), [
    ...
    public function test_resolve_bounce_rejects_the_same_address_as_a_fix(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.resolve-bounce', $log->uid), ['to' => 'bad@example.test'])
            ->assertSessionHasErrors('to');
    ...
    /**
     * La fila de auditoría se crea directamente sobre el modelo Activity en
     * vez de disparar download(): así el test comprueba solo la lectura y el
     * render de EmailLogController::activityLogFor(), sin depender de qué
     * evento concreto escriba cada acción.
     */
    public function test_show_displays_the_activity_log_for_this_email(): void
    {
        ...
        $this->actingAs($this->manager())
            ->get(route('helpdeskemailactivity.show', $log->uid))
            ->assertOk()
            ->assertSee(__('helpdeskemailactivity::emaillog.activity_log.events.downloaded'));
    }

    /**
     * Por el listado, no por show(): show() registra 'viewed' en la propia
     * petición (ver EmailLogController::show()), así que su bitácora nunca
     * llega vacía a la vista. index() autoselecciona la primera fila SIN
     * registrar nada, que es justo el caso que se quiere comprobar aquí.
     */
    public function test_activity_log_is_empty_for_an_email_with_no_recorded_actions(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index'))
            ->assertOk()
            ->assertSee(__('helpdeskemailactivity::emaillog.activity_log.empty'));
    }
    ...
    public function test_index_filters_by_causer_agent(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['causer_id' => $agent->id]))
            ->assertOk()
    ...
    public function test_index_filters_by_from_address(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['from_address' => 'marketing@example.test']))
            ->assertOk()
    ...
    public function test_index_filters_by_has_attachments(): void
    {
        ...
        $this->actingAs($this->viewer())
            ->get(route('helpdeskemailactivity.index', ['has_attachments' => 1]))
            ->assertOk()
    ...
    public function test_link_entity_links_the_email_to_a_ticket_and_logs_it(): void
    {
        ...
        $this->actingAs($this->manager())
            ->post(route('helpdeskemailactivity.link-entity', $log->uid), [
    ...
    public function test_search_tickets_returns_matching_tickets(): void
    {
        ...
        $response = $this->actingAs($this->manager())
            ->getJson(route('helpdeskemailactivity.tickets.search', ['q' => 'Needle in a haystack']))
            ->assertOk();
    ...
    {
        $this->actingAs($this->viewer())
            ->getJson(route('helpdeskemailactivity.tickets.search', ['q' => 'anything']))
            ->assertForbidden();
    }
}
