 ...
 * (settings.helpdeskemailactivity.webhook-events.*) y apuntando al controlador
 * real: el middleware `can:...` se resuelve desde el controlador
 * (constructor), no desde la ruta, y la propia vista genera enlaces con
 * route('settings.helpdeskemailactivity.webhook-events.index') — así que este
...
class WebhookEventsControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if (! Route::has('settings.helpdeskemailactivity.webhook-events.index')) {
            Route::middleware(['web', 'auth'])->group(function (): void {
                Route::get('/panel/settings/helpdeskemailactivity/webhook-events', [WebhookEventsController::class, 'index'])
                    ->name('settings.helpdeskemailactivity.webhook-events.index');
                Route::post('/panel/settings/helpdeskemailactivity/webhook-events/{event}/reprocess', [WebhookEventsController::class, 'reprocess'])
                    ->name('settings.helpdeskemailactivity.webhook-events.reprocess');
            });
        }
    ...
    {
        $this->get(route('settings.helpdeskemailactivity.webhook-events.index'))->assertRedirect();
    }
    ...
    public function test_index_requires_settings_view_permission(): void
    {
        ...
        $this->actingAs($outsider)
            ->get(route('settings.helpdeskemailactivity.webhook-events.index'))
            ->assertForbidden();
    }
    ...
    public function test_index_renders_health_panel_and_events_for_authorized_user(): void
    {
        ...
        $this->postJson(route('helpdeskemailactivity.webhooks.receive', ['provider' => 'mailrelay']), [
    ...
    public function test_reprocess_requires_settings_update_permission(): void
    {
        ...
        $this->actingAs($viewer)
            ->post(route('settings.helpdeskemailactivity.webhook-events.reprocess', $event))
            ->assertForbidden();
    }
