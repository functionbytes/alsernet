 ...
 * Postmark, ver Modules\HelpdeskEmailActivity\Http\Controllers\... si algún día
...
class BounceProcessorService
{
    ...
    public function process(int $limitPerMailbox = 50): array
    {
        if (! class_exists(ClientManager::class)) {
            Log::error('HelpdeskEmailActivity BounceProcessorService: webklex/php-imap no está instalado.');
    ...
    private function processMailbox(array $mailbox, int $limit): array
    {
        ...
        try {
            ...
            Log::warning('HelpdeskEmailActivity BounceProcessorService: fallo de conexión IMAP', [
        ...
        try {
            ...
            foreach ($messages as $message) {
                ...
                try {
                    ...
                    Log::warning('HelpdeskEmailActivity BounceProcessorService: fallo procesando un mensaje', [
                        'error' => $e->getMessage(),
                    ]);
                    $unmatched++;
                }
