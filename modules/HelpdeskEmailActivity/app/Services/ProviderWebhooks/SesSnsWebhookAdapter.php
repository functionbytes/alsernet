...
class SesSnsWebhookAdapter implements EmailProviderWebhookAdapter
{
    ...
    public function handleControlMessage(Request $request): ?Response
    {
        ...
        if (! $this->isAmazonHost($subscribeUrl) || ! OutboundUrlGuard::isSafe($subscribeUrl)) {
            Log::warning('helpdeskemailactivity: SubscribeURL de SNS con host no confiable, ignorado', ['url' => $subscribeUrl]);
        ...
        try {
            ...
            Log::warning('helpdeskemailactivity: fallo al confirmar suscripción SNS', ['error' => $e->getMessage()]);
    ...
    private function fetchCert(string $url): ?string
    {
        return Cache::remember(
            'helpdeskemailactivity:sns_cert:'.md5($url),
            now()->addHour(),
            function () use ($url) {
                try {
