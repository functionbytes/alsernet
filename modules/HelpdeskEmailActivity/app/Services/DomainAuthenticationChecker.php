 ...
 * helpdeskemailactivity.reputation_common_dkim_selectors), pero un selector no
...
class DomainAuthenticationChecker
{
    ...
    {
        $hours = (int) config('helpdeskemailactivity.reputation_dns_cache_hours', 24);

        return Cache::remember(
            "helpdeskemailactivity:domain-auth:{$domain}",
            now()->addHours($hours),
            fn () => [
    ...
    {
        $common = (array) config('helpdeskemailactivity.reputation_common_dkim_selectors', []);
