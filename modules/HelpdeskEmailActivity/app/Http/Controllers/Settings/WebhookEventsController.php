...
use Modules\HelpdeskEmailActivity\Services\ProviderWebhookEventProcessor;
...
class WebhookEventsController extends Controller
{
    ...
    public function __construct(
        ...
        $this->middleware('can:helpdeskemailactivity.settings.update')->only('reprocess');
    }
    ...
    public function index(Request $request): View
    {
        ...
        return view('helpdeskemailactivity::settings.webhook-events', [
            'events' => $events,
    ...
    public function reprocess(ProviderWebhookEvent $event): RedirectResponse
    {
        if ($event->email_log_id !== null) {
            return back()->with('error', __('helpdeskemailactivity::emaillog.webhook_events.already_correlated'));
        }

        $parsedEvent = $this->rebuildParsedEvent($event);

        if ($parsedEvent === null) {
            return back()->with('error', __('helpdeskemailactivity::emaillog.webhook_events.cannot_reprocess'));
        }

        $config = $this->settingsRepo->get();

        if (! $this->processor->shouldProcess($parsedEvent, $config)) {
            return back()->with('warning', __('helpdeskemailactivity::emaillog.webhook_events.type_disabled'));
        }
        ...
        return back()->with(
            ...
            __($matchedLog !== null
                ? 'helpdeskemailactivity::emaillog.webhook_events.reprocessed_matched'
                : 'helpdeskemailactivity::emaillog.webhook_events.reprocessed_unmatched')
        );
    }
