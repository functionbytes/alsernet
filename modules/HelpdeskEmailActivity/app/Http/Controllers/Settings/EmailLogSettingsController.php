...
{
    private const PREFIX = 'helpdeskemailactivity.';
    ...
    public function __construct(private readonly ProviderWebhookSettingsRepository $providerWebhookSettings)
    {
        ...
        $this->middleware('can:helpdeskemailactivity.settings.update')->only('update');
    }

    public function index(): View
    {

        $s = fn (string $key, mixed $default = null) => Setting::get(self::PREFIX.$key, $default ?? config('helpdeskemailactivity.'.$key));
        ...
        return view('helpdeskemailactivity::settings.index', [
            ...
            'perPageOptions' => config('helpdeskemailactivity.per_page_options', [10, 25, 50, 100]),
            ...
            'providerWebhookUrl' => $providerConfig['provider']
                ? route('helpdeskemailactivity.webhooks.receive', ['provider' => $providerConfig['provider']])
                : null,
        ]);
    }
