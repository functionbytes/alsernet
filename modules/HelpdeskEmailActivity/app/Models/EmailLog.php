...
use Modules\HelpdeskEmailActivity\Database\Factories\EmailLogFactory;
...
class EmailLog extends Model
{
     ...
     * `helpdeskemailactivity.trash_retention_days` días. El borrado GDPR
     ...
     */
    private const CACHE_KEY_STATS = 'helpdeskemailactivity:stats';

    private const CACHE_KEY_TREND = 'helpdeskemailactivity:trend';

    private const CACHE_KEY_STALE = 'helpdeskemailactivity:stale';

    private const CACHE_KEY_MODULES = 'helpdeskemailactivity:modules';
     ...
     */
    private const CACHE_KEY_CAUSERS = 'helpdeskemailactivity:causers';

    private const CACHE_KEY_FROM_ADDRESSES = 'helpdeskemailactivity:from-addresses';
    ...
    protected static function booting(): void
    {
        ...
        // En 'saving', no en 'creating': el índice desnormalizado que usa el
        // buscador tiene que seguir a las direcciones también cuando cambian.
        // Calculándolo solo al crear, cualquier actualización de
        // to/cc/bcc_addresses dejaba el índice apuntando a las direcciones
        // viejas — y el buscador seguía encontrando por ellas (era el caso de
        // la anonimización RGPD, que tenía que rehacerlo a mano).
        //
        // Recalcular en cada guardado es barato: concatenar tres arrays de
        // direcciones, sin consultas.
        static::saving(function (self $model) {
            if (! $model->exists
                || $model->isDirty(['to_addresses', 'cc_addresses', 'bcc_addresses'])) {
     ...
     * vigilado — ver Modules\HelpdeskEmailActivity\Services\BounceProcessorService
    ...
    /**
     * Emails cuyo cuerpo llevaba píxel de apertura.
     *
     * Filtra por la columna generada `open_tracking_enabled` en vez de por
     * `metadata->open_tracking_enabled`: son equivalentes (la columna se
     * deriva del propio metadata, ver la migración
     * ..._add_tracking_flag_columns_to_email_logs), pero la columna está
     * indexada y la expresión JSON obliga a recorrer la tabla entera.
     *
     * La fuente de verdad sigue siendo `metadata`, que es lo que escriben
     * LogEmailQueued/LogEmailSent; estas columnas solo existen para filtrar.
     */
    #[Scope]
    protected function openTracked(Builder $query): void
    {
        $query->where('open_tracking_enabled', true);
    }

    /** Igual que openTracked(), para la reescritura de enlaces. */
    #[Scope]
    protected function clickTracked(Builder $query): void
    {
        $query->where('click_tracking_enabled', true);
    }

    /** Emails SIN píxel de apertura (complemento de openTracked()). */
    #[Scope]
    protected function withoutOpenTracking(Builder $query): void
    {
        $query->where('open_tracking_enabled', false);
    }
    ...
    /**
     * El mensaje SÍ llevaba una parte HTML (o de texto), pero no se guardó:
     * el ajuste "Guardar cuerpo" estaba desactivado al enviarlo.
     *
     * La diferencia importa en el inspector: decir "este envío no tiene versión
     * HTML" cuando en realidad sí la tenía manda a buscar un fallo de plantilla
     * que no existe — el contenido está fuera por configuración, no ausente.
     */
    public function htmlBodyWasDiscarded(): bool
    {
        return ! $this->body_html
            && ! $this->isBodyRedacted()
            && (bool) ($this->metadata['has_html'] ?? false);
    }

    public function textBodyWasDiscarded(): bool
    {
        return ! $this->body_text
            && ! $this->isBodyRedacted()
            && (bool) ($this->metadata['has_text'] ?? false);
    }
    ...
    protected function entityLabel(): Attribute
    {
        return Attribute::make(get: function (): ?string {
            ...
            $labels = config('helpdeskemailactivity.entity_labels', []);
    ...
    protected function entityUrl(): Attribute
    {
        return Attribute::make(get: function (): ?string {
            ...
            $route = config('helpdeskemailactivity.entity_routes.'.$this->entity_type);
            ...
            try {
                ...
                Log::warning('HelpdeskEmailActivity: no se pudo generar entity_url.', [
