...
">
	...
	<script>
		...
		$(function() {
			...
			// Customers color scheme | menu layout wizard
			@if (false)
				$(function() {
			...
			@if (null !== Session::get('orig_admin_id') && Auth::user()->admin)
				notify({
					type: 'warning',
					message: `{!! trans('messages.current_login_as', ["name" =>Auth::user()->firsname]) !!}<br>{!! trans('messages.click_to_return_to_origin_user', ["link" => route("Settings\AdminController@loginBack")]) !!}`,
					timeout: false,
				});
			@endif
