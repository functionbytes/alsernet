        ...
        <div class="collapse navbar-collapse" id="mainAppNav">
            <ul class="navbar-nav me-auto mb-md-0 main-menu">
                ...
                <li class="nav-item dropdown {{ in_array($menu, [
                    ...
                    <a title="{{ trans('messages.sending') }}" href="{{ route('manager.templates') }}" class="leftbar-tooltip nav-link d-flex align-items-center py-3 lvl-1 dropdown-toggle {{ request()->session()->get('helpdesks-leftbar-state') != 'closed'  && request()->session()->get('customer-leftbar-state') != 'closed'  && in_array($menu, [
                        'sending_server',
                        'sending_domain',
                        'sender',
                        'tracking_domain',
                    ...
                    <ul class="dropdown-menu dropdown-menu-bottom {{ request()->session()->get('helpdesks-leftbar-state') != 'closed'  && request()->session()->get('customer-leftbar-state') != 'closed'  && in_array($menu, [
                        'sending_server',
                        'sending_domain',
                        'sender',
                        'tracking_domain',
                ...
                <li class="nav-item dropdown {{ in_array($menu, ['setting','helpdesks','admin_group','layout','language','plugin','product','source']) ? 'active' : '' }}">
					<a role="button" class="nav-link lvl-1 dropdown-toggle d-flex align-items-center {{ request()->session()->get('helpdesks-leftbar-state') != 'closed'  && request()->session()->get('customer-leftbar-state') != 'closed'  && in_array($menu, ['setting','helpdesks','admin_group','layout','language','plugin','product','source']) ? 'show' : '' }}" data-bs-toggle="dropdown">
					<i class="navbar-icon">
                    ...
                    <ul class="dropdown-menu dropdown-menu-bottom {{ request()->session()->get('helpdesks-leftbar-state') != 'closed'  && request()->session()->get('customer-leftbar-state') != 'closed'  && in_array($menu, ['setting','helpdesks','admin_group','layout','language','plugin','product','source']) ? 'show' : '' }}">
