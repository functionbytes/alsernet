...
<div class="container py-4" style="max-width:880px">
    ...
    <form method="post" action="{{ route('manager.campaigns.automations.update', $automation->uid) }}">
        @csrf @method('PUT')
        ...
        @if ($automation->status === 'active')
            <form method="post" action="{{ route('manager.campaigns.automations.disable', $automation->uid) }}" class="d-inline">@csrf<button class="btn btn-outline-warning">Pausar</button></form>
        @else
            <form method="post" action="{{ route('manager.campaigns.automations.enable', $automation->uid) }}" class="d-inline">@csrf<button class="btn btn-outline-success">Activar</button></form>
        @endif
