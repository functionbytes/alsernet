...
use Modules\Campaign\Models\Setting;
...
class Tool
{
    ...
    public static function cronjobUpdateController($request, $controller)
    {
        ...
        if (empty(Setting::get('remote_job_token'))) {
            Setting::set('remote_job_token', str_random(60));
        }
    ...
    /**
     * Ejecuta una tarea respetando rate limits y créditos. Reemplaza el helper
     * global Acelle\Helpers\execute_with_limits() de la versión SaaS.
     *
     * @param  array  $rateTrackers  Trackers que aplican siempre.
     * @param  mixed  $rateTrackersPool  Pool RouletteWheel opcional. null si no aplica.
     * @param  array  $creditTrackers  Vacío en este destino no-SaaS (firma se mantiene).
     * @param  \Closure|null  $task  Closure(?$selectedServer) que envía.
     * @return mixed
     *
     * @throws RateLimitExceeded
     */
    public static function executeWithLimits(
        array $rateTrackers,
        $rateTrackersPool,
        array $creditTrackers,
        ?\Closure $task = null,
        $rateExceedingCallback = null
    ) {
        $creditTrackers = array_values(array_filter($creditTrackers));
        $creditCounted = [];

        try {
            foreach ($creditTrackers as $tracker) {
                $tracker->count();
                $creditCounted[] = $tracker;
            }
        } catch (\Throwable $e) {
            foreach ($creditCounted as $tracker) {
                $tracker->rollback();
            }
            throw $e;
        }

        $selectedServer = self::withRateLimits($rateTrackers, $rateTrackersPool, $rateExceedingCallback, $creditCounted);
    ...
    /**
     * Cuenta los rate trackers y selecciona un servidor del pool si está presente.
     *
     * @throws RateLimitExceeded
     */
    public static function withRateLimits(
        array $rateTrackers,
        $rateTrackersPool,
        $rateExceedingCallback,
        array $creditCounted = [],
        bool $testMode = false
    ) {
        ...
        try {
            foreach ($rateTrackers as $tracker) {
                $testMode
                    ? $tracker->test(now(), $rateExceedingCallback)
            ...
            do {
                ...
                if (is_null($selectedServer)) {
                    throw new RateLimitExceeded($notes);
                }

                $serverTracker = $selectedServer->getRateLimitTracker();
                try {
                    $testMode
                        ? $serverTracker->test(now())
                        : $serverTracker->count(now());
                    $passed = true;
                } catch (RateLimitExceeded $ex) {
                    $notes .= ' | EXCEEDED '.$serverTracker->getLimitsDescription();
                    $passed = false;
                }
            } while (! $passed);
            ...
            if (! $testMode) {
                ...
                foreach ($rateCounted as $tracker) {
                    $tracker->rollback();
                }
            }
