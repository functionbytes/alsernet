...
class BaseCampaign extends Model
{
     ...
     * Associations
     *
     * Recursos globales: en este destino no SaaS, la propiedad por usuario fue retirada.
     * Si en el futuro hace falta saber QUIÉN creó la campaña, asociar a User vía Activity Log
     * (módulo Activity) en vez de añadir owner_id aquí.
     *
     * @deprecated En este destino no se asocia campaign↔customer.
     */
    public function customer()
    {
        return $this->belongsTo('App\Models\User'); // fallback compat
    }
    ...
    public function execute()
    {
        ...
        if (! is_null($this->run_at) && $this->run_at->gte($now)) {
            $scheduledAt = $this->run_at->timezone(config('app.timezone', 'UTC'));
    ...
    {
        if ($this->run_at) {
            return $this->run_at->timezone(config('app.timezone', 'UTC'))->diffForHumans();
        }
