...
class SystemUrl extends Notification
{
    ...
    {
        $title = trans('messages.helpdesks.notification.system_url_title');
        ...
        if ($current != $cached) {
            $warning = [
                'title' => $title,
                'message' => trans('messages.helpdesks.notification.system_url_not_match', ['cached' => $cached, 'current' => $current]),
            ];
