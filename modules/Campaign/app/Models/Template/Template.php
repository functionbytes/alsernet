...
use Modules\Campaign\Services\UrlService;
...
class Template extends Model
{
    ...
    // ── BuilderJS (page templates) — contrato portado de acellemail ──────────
    // JSON BuilderJS por defecto para una página en blanco. Satisface la
    // invariante de json no-vacío que espera el motor BuilderJS.
     ...
     */
    protected $fillable = [
        'uid', 'name', 'title', 'subject', 'content', 'html', 'plain', 'builder',
        'is_default', 'theme', 'type', 'shared', 'image', 'layout_id',
        'blocks', 'global_settings', 'json', 'source',
    ];
    ...
    {
        return $this->belongsToMany('Modules\Campaign\Models\Template\TemplateCategory', 'campaign_templates_categories', 'template_id', 'category_id');
    }
    ...
    // ════════════════════════════════════════════════════════════════════════
    // BuilderJS bridge (page templates) — portado/adaptado de acellemail.
    // Aislado del builder Alpine de email: usa columnas json/theme/source y la
    // columna content para el HTML renderizado por BuilderJS.
    // ════════════════════════════════════════════════════════════════════════

    /**
     * Estado BuilderJS decodificado (para `builder.load(themeJson, ...)`).
     */
    ...
    /** ¿Plantilla en modo custom HTML (one-way, fuera del builder)? */
    public function isBuilderUploaded(): bool
    {
        return $this->source === self::SOURCE_UPLOADED;
    }

    /** Alias compatible con las vistas portadas de acellemail. */
    public function isUploaded(): bool
    {
        return $this->isBuilderUploaded();
    }

    /**
     * URL del thumbnail para las vistas de page templates. Evita el
     * \App\Helpers\generatePublicPath de acellemail (no existe en este host):
     * sirve el fichero almacenado vía la ruta del módulo, con fallback a placeholder.
     */
    public function getThumbnailUrl(): string
    {
        if ($this->getThumbName()) {
            return route('manager.page_templates.thumb', ['uid' => $this->uid])
                .'?v='.optional($this->updated_at)->timestamp;
        }

        return asset('refactor/images/favicon.svg');
    }

    /** URL base de assets del tema (servida por la ruta theme_asset del módulo). */
    public function getThemeUrl($path = null): string
    {
        return app(UrlService::class)
            ->generateThemeAssetUrl($this->theme ?: 'default', $path);
    }

    /** Manifiesto del tema (resources/themes/{theme}/index.json del módulo). */
    public function getThemeConfigData(): array
    {
        $theme = $this->theme ?: 'default';
        $path = module_path('Campaign', 'resources/themes/'.$theme.'/index.json');
    ...
    /**
     * Guarda json (BuilderJS) + content (HTML render). Devuelve Validator.
     */
    public function updateBuilderContent($json, $content)
    {
        $validator = \Validator::make(
            ['json' => $json, 'content' => $content],
            ['json' => 'required', 'content' => 'required'],
        );
    ...
    /** Copia un thumbnail de origen al storage de la plantilla. */
    public function updateThumbnailFromPath($sourcePath): void
    {
        ...
        $ext = in_array($ext, ['svg', 'png', 'jpg', 'jpeg', 'gif', 'webp'], true) ? $ext : 'png';

        File::copy($sourcePath, $this->getStoragePath('thumbnail.'.$ext));
    }

    /** Crea un Template BuilderJS (page) con json + content + theme. */
    public static function createBuilderTemplate(string $theme, string $name, string $json, string $content): self
    {
        $template = new self;
        ...
        $template->source = self::SOURCE_BUILDER;
        $template->save();

        return $template;
    }

    /** Crea un Template en modo custom HTML (source=uploaded). */
    public static function makeUploadedBuilderTemplate(string $name, string $html): self
    {
        $template = new self;
        $template->theme = 'default';
        $template->name = $name;
        $template->json = self::DEFAULT_BUILDER_JSON;
