...
// use App; // legacy
// use App\Library\Traits\QueryHelper; // legacy
use Carbon\Carbon;
// use App\Models\Page; // legacy
// use App\Models\Subscriber\Subscriber; // legacy
// use App\Models\Subscriber\SubscriberList; // legacy
use Exception;
...
// use Modules\Campaign\Jobs\Subscribers\ImportSubscribers2;
// use Modules\Campaign\Jobs\Subscribers\ImportSubscribersJob;
// use Modules\Campaign\Jobs\Subscribers\ImportSubscribersListsJob;
// use Modules\Campaign\Library\ExtendedSwiftMessage; // legacy
...
// use Modules\Subscriber\Jobs\ExportSubscribersJob; // legacy
// use Modules\Subscriber\Jobs\VerifyMailListJob; // legacy
 ...
 * @property-read Collection<int, CampaignField> $fields
 * @property-read int|null $fields_count
 * @property-read Collection<int, JobMonitor> $jobMonitors
 * @property-read int|null $job_monitors_count
 * @property-read Collection<int, CampaignSegment> $segments
 * @property-read int|null $segments_count
 * @property-read Collection<int, CampaignMaillistsSubscriber> $subscribers
 ...
 * @mixin \Eloquent
 */
class CampaignMaillistLegacy extends Model
{
    use HasCache;
    use HasUid;
    use QueryHelper;
    ...
    {
        return $this->hasMany('Modules\Campaign\Models\CampaignField');
    }

    public function segments()
    {
        return $this->hasMany('Modules\Campaign\Models\CampaignSegment', 'maillist_id');
    }

    public function automations()
    {
        return $this->hasMany('Modules\Campaign\Models\Automation\Automation');
    }
    ...
    {
        return $this->hasMany('Modules\Campaign\Models\CampaignMailListsSubscriber', 'maillist_id');
    }

    public function campaigns()
    {
        return $this->belongsToMany('Modules\Campaign\Models\Campaign', 'campaigns_lists_segments', 'maillist_id', 'campaign_id');
    }
    ...
    public function sendSubscriptionConfirmationEmail($subscriber)
    {
        ...
        $send_page = Page::findPage($this, $layout);
    ...
    public function sendSubscriptionWelcomeEmail($subscriber)
    {
        ...
        $send_page = Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function subscribe($request, $source)
    {
        ...
        // Do not allow duplicate email if added by helpdesks (throw an exception)
    ...
    public function updateOrCreateFieldsFromRequest($request)
    {
        ...
        foreach ($request->fields as $key => $item) {
            ...
            if ($this->getEmailField()->uid != $item['uid']) {
                ...
                // Check tag exsit
                $tag = Modules\Campaign\Models\CampaignField::formatTag($item['tag']);
        ...
        foreach ($request->fields as $uid => $item) {
            $field = Modules\Campaign\Models\CampaignField::findByUid($item['uid']);
            if (! $field) {
                $field = new Modules\Campaign\Models\CampaignField;
            ...
            if ($this->getEmailField()->uid != $field->uid) {
                // save exsit field
                $item['tag'] = Modules\Campaign\Models\CampaignField::formatTag($item['tag']);
                $field->fill($item);
                $field->save();
                ...
                if (isset($item['options'])) {
                    foreach ($item['options'] as $key2 => $item2) {
                        $option = new Modules\Campaign\Models\CampaignFieldOption($item2);
