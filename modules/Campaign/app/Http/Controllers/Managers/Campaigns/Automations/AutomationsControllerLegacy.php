...
// use Acelle\Helpers\LicenseHelper; // removed: no es producto licenciado en este destino
...
class AutomationsController extends Controller
{
    ...
    public function wizardListFieldSelect(Request $request)
    {
        ...
        $list = ModelsList::findByUid($request->list_uid);
    ...
    public function prepareOptions($options, $customer)
    {
        if (isset($options['key']) && $options['key'] == 'specific-date') {
            if (isset($options['date'])) {
                $date = Carbon::createFromFormat('Y-m-d H:i', $options['date'].' '.$options['at'], $customer->timezone);
    ...
    public function actionSelect(Request $request, $uid)
    {
        ...
        if ($request->key == 'wait') {
            $delayOptions = Automation2::getDelayOptions();
    ...
    {
        // Japan + not license
        if (config('custom.japan') && ! LicenseHelper::hasActiveLicense()) {
            return response()->json([
                'status' => 'error',
    ...
    {
        $webhook = EmailWebhook::findByUid($request->webhook_uid);
