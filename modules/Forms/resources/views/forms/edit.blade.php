                ...
                <div class="card mb-3">
                    ...
                    <div class="config-card-body p-3">
                        ...
                        <div class="mb-4">
                            ...
                            {{-- Los valores tienen que ser los del enum de la columna
                                 (none, fade, checkmark, confetti). Antes ofrecía "" y
                                 "fireworks", que no existen, y no ofrecía "fade", que es
                                 el valor por defecto: un formulario con fade abría con el
                                 select vacío y guardar reventaba con
                                 "Column 'success_animation' cannot be null". --}}
                            <select id="configSuccessAnimation" class="form-select config-select2">
                                <option value="none"     {{ $form->success_animation === 'none'      ? 'selected' : '' }}>Ninguna</option>
                                <option value="fade"     {{ in_array($form->success_animation, ['fade', null, ''], true) ? 'selected' : '' }}>Desvanecido</option>
        ...
        <div class="col-12 col-lg-4">
            ...
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <button type="button" class="btn btn-primary w-100 mb-2" id="btnSaveConfig">
                        <span class="btn-save-text">Guardar configuración</span>
                    </button>
                    <a href="{{ route('settings.forms.preview', $form) }}" target="_blank" class="btn btn-outline-secondary w-100 mb-2" id="btnFooterPreview">
                        Vista previa
                    </a>
            ...
            <div class="card mb-3">
                <div class="card-header border-bottom p-3">
                    ...
                    <small class="text-muted">Enlaces y accesos del formulario</small>
                </div>
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        {{-- Aquí no hay CMS que interprete shortcodes [form ...],
                             así que se ofrece lo que sí funciona: el enlace
                             público directo y el iframe para incrustarlo. --}}
                        <button type="button"
                                ...
                                data-shortcode="{{ route('forms.public.show', $form->slug) }}">
                            <span class="small"><i class="fas fa-link me-2 text-muted"></i>Copiar enlace público</span>
                        </button>
                        <button type="button"
                                class="list-group-item list-group-item-action d-flex align-items-center justify-content-between px-3 py-2 btn-copy-shortcode"
                                data-shortcode='<iframe src="{{ route('forms.public.embed', $form->slug) }}" width="100%" height="600" frameborder="0"></iframe>'>
                            <span class="small"><i class="fas fa-code me-2 text-muted"></i>Copiar código de inserción</span>
                        </button>
            ...
            <div class="card mb-3">
                <div class="card-header border-bottom p-3">
                    <h6 class="mb-0 fw-bold">Sitio web</h6>
                    <small class="text-muted">Publicar este formulario en la tienda</small>
                </div>
                <div class="card-body p-3">

                    @if (! $publication['configured'])
                        <p class="small text-muted mb-0">
                            La conexión con la tienda no está configurada. Falta definir
                            <code>FORMS_STORE_API_URL</code>.
                        </p>

                    @elseif (! $publication['linked'])
                        {{-- La clave la define el catálogo de Helpdesk porque es quien
                             determina la categoría del ticket que abrirá cada envío. --}}
                        <p class="small text-muted mb-3">
                            Vincula el formulario a una clave del sitio para poder publicarlo.
                        </p>
                        <form method="POST" action="{{ route('settings.forms.publication.link', $form) }}">
                            @csrf
                            <div class="mb-3">
                                <label class="form-label fw-semibold" for="site-form-key">Clave en el sitio</label>
                                <select class="form-select" id="site-form-key" name="form_key" required>
                                    <option value="">Seleccionar…</option>
                                    @foreach ($siteFormKeys as $siteKey)
                                        <option value="{{ $siteKey->form_key }}">{{ $siteKey->name }} ({{ $siteKey->form_key }})</option>
                                    @endforeach
                                </select>
                                @if ($siteFormKeys->isEmpty())
                                    <small class="text-muted">No hay claves libres en el catálogo del sitio.</small>
                                @endif
                            </div>
                            <button type="submit" class="btn btn-primary w-100" @disabled($siteFormKeys->isEmpty())>
                                Vincular al sitio
                            </button>
                        </form>
                    ...
                    @else
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="fw-semibold font-monospace small">{{ $publication['form_key'] }}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span class="text-muted small">Estado</span>
                            <span class="badge {{ $publication['status'] === 'published' ? 'bg-primary-subtle text-primary' : 'bg-light text-muted' }}">
                                {{ [
                                    'published' => 'Publicado',
                                    'pending' => 'Publicando…',
                                    'failed' => 'Falló',
                        ...
                        @if ($publication['published_at'])
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <span class="text-muted small">Versión {{ $publication['published_version'] }}</span>
                                <span class="text-muted small">{{ $publication['published_at']->diffForHumans() }}</span>
                            </div>
                        @endif

                        @if ($publication['last_error'])
                            <div class="alert alert-warning small py-2 px-3">
                                {{ \Illuminate\Support\Str::limit($publication['last_error'], 160) }}
                            </div>
                        @endif

                        @if ($publication['has_pending_changes'])
                            <div class="alert alert-warning small py-2 px-3">
                                Hay cambios sin publicar. El sitio sigue sirviendo la versión anterior.
                            </div>
                        @else
                            <p class="small text-muted">El sitio sirve exactamente esta versión.</p>
                        @endif

                        {{-- Interruptor de migración. Publicar deja la definición lista en la
                             tienda, pero mientras esto esté apagado el cliente sigue viendo el
                             formulario antiguo: permite revisarlo en producción antes de
                             activarlo, y volver atrás al instante si algo falla. --}}
                        @if ($publication['status'] === 'published')
                            <form method="POST" action="{{ route('settings.forms.publication.override', $form) }}" class="mb-3">
                                @csrf
                                <label class="form-label fw-semibold" for="publication-override">¿Qué ve el cliente?</label>
                                <select class="form-select mb-2" id="publication-override" name="overrides_legacy"
                                        onchange="this.form.submit()">
                                    <option value="0" @selected(! $publication['overrides_legacy'])>El formulario antiguo del sitio</option>
                                    <option value="1" @selected($publication['overrides_legacy'])>Este formulario</option>
                                </select>
                                <small class="text-muted">
                                    {{ $publication['overrides_legacy']
                                        ? 'Este formulario sustituye al que el sitio tenía en código.'
                                        : 'La definición está publicada, pero el sitio aún muestra el formulario antiguo.' }}
                                </small>
                            </form>
                        @endif

                        <form method="POST" action="{{ route('settings.forms.publication.publish', $form) }}" class="mb-1">
                            @csrf
                            <button type="submit" class="btn btn-primary w-100">
                                {{ $publication['has_pending_changes'] ? 'Publicar cambios' : 'Publicar' }}
                            </button>
                        </form>
                        <form method="POST" action="{{ route('settings.forms.publication.republish', $form) }}" class="mb-1">
                            @csrf
                            {{-- Para cuando la tienda se restaura de una copia y se queda
                                 atrás sin que aquí conste. --}}
                            <button type="submit" class="btn btn-secondary w-100">Reenviar al sitio</button>
                        </form>
                        <form method="POST" action="{{ route('settings.forms.publication.unpublish', $form) }}">
                            @csrf
                            <button type="submit" class="btn btn-secondary w-100">Retirar del sitio</button>
                        </form>
                    @endif
                </div>
            </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                <div class="field-switches-bar d-flex flex-wrap gap-4 p-3">
                                    ...
                                    <div>
                                        <label class="form-label fw-semibold mb-1" for="fieldExcludeHelpdesk">Enviar al ticket</label>
                                        <div class="text-muted field-check-hint mb-1">Los datos de gestión (intereses, consentimiento comercial) no le sirven al agente</div>
                                        <select class="form-select form-select-sm" name="exclude_from_helpdesk" id="fieldExcludeHelpdesk">
