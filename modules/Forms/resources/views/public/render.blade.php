...
(function () {
    ...
    var jqvSrc   = '{{ asset('core/validate/jquery.validate.min.js') }}';
    var localeSrc = @json($validateLocale ? asset('core/validate/localization/messages_'.$validateLocale.'.min.js') : null);
    var select2Src = '{{ asset('core/select2/js/select2.min.js') }}';
    var formsSrc   = '{{ asset('modules/forms/js/forms.min.js') }}';

    waitForJQuery(function () {
        // jQuery Validate y Select2 se sirven self-hosted (public/core/validate,
        // public/core/select2) en vez de un CDN externo, para que la validación
        // de los formularios no dependa de la disponibilidad de un tercero.
        // No dependen entre sí, así que se cargan en paralelo; forms.min.js
        // necesita ambos, por eso espera a que los dos terminen.
        var pending = 2;
        function afterBothReady() {
            pending--;
            if (pending === 0) { loadScript(formsSrc); }
        }
        loadScript(jqvSrc, function () {
            if (localeSrc) { loadScript(localeSrc, afterBothReady); }
            else { afterBothReady(); }
        });
        loadScript(select2Src, afterBothReady);
    });
})();
</script>
@endonce
...
(function () {
    ...
    function setupLoadingStateOnSubmit(form) {
        ...
        form.addEventListener('submit', function () {
            // No mostrar el estado de carga si el formulario todavia tiene campos
            // invalidos: jquery-validate cancelara este submit, y sin este guard el
            // boton quedaba deshabilitado en "Sending..." varios segundos aunque
            // nunca se llego a enviar nada.
    ...
    function setupInlineValidation(form) {
        ...
        // select2 dispara `blur` sobre el <select> original al inicializarse:
        // sin esto, todos los desplegables obligatorios salian en rojo nada mas
        // cargar la pagina, sin que el visitante hubiera tocado nada. El aviso
        // solo aparece a partir de la primera interaccion real con el
        // formulario; el envio sigue validando todo igual.
        var userEngaged = false;
        ['pointerdown', 'keydown'].forEach(function (evt) {
            form.addEventListener(evt, function (e) {
                if (e.isTrusted) userEngaged = true;
            }, true);
        });
...
window.FormsConfig['{{ $formId }}'] = {
    ...
    successMessage: @json($form->localizedSuccessMessage(app()->getLocale())),
    ...
    {{-- Pedido explicito del usuario: en "Solicitar cotizacion" (fichas de
         ...
         guardado en el mismo navegador/sesion sin que el usuario lo pidiera
         (se notaba como "datos de ejemplo" ya cargados). Se apaga solo para
         este formulario compartido; el resto (contacto, booking) sigue
         guardando/restaurando borrador como antes. --}}
    abandonTrackingEnabled: {{ ($form->is_active && $form->slug !== 'product-quote-request') ? 'true' : 'false' }},
};
@if($display === 'popup')
