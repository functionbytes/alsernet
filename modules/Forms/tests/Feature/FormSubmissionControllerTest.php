...
class FormSubmissionControllerTest extends TestCase
{
    ...
    {
        $user = $this->createUser(['Forms.submissions.edit']);
        $form = $this->createForm();
    ...
    public function test_admin_can_bulk_delete_submissions(): void
    {
        ...
        foreach ($ids as $id) {
            $this->assertSoftDeleted('form_submissions', ['id' => $id]);
        }
    }
    ...
    public function test_admin_can_export_submissions(): void
    {
        ...
        Queue::assertPushed(ExportFormSubmissionsJob::class, fn ($job) => $job->form->is($form) && $job->user->is($user));
    }
