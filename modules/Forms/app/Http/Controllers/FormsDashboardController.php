...
class FormsDashboardController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $cacheKey = "forms:dashboard:{$range}:".now()->format('YmdHi');

        $data = Cache::remember($cacheKey, 300, function () use ($range) {
            ...
            $prevMonth = FormSubmission::query()
                ->whereBetween('created_at', [
                    Carbon::parse($startPrevMonth)->startOfDay(),
                    Carbon::parse($endPrevMonth)->endOfDay(),
                ])
                ->count();
            $prev30Days = FormSubmission::query()
                ->whereBetween('created_at', [$prevStart->copy()->startOfDay(), $prevEnd->copy()->endOfDay()])
                ->count();
            $curr30Days = FormSubmission::query()
                ->whereBetween('created_at', [$start->copy()->startOfDay(), $end->copy()->endOfDay()])
                ->count();
        ...
        return view('forms::inbox.dashboard', array_merge(['range' => $range], $data));
    }
