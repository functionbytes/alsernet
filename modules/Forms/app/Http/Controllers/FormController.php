...
class FormController extends Controller
{
    ...
    public function edit(Form $form): View
    {
        ...
        $publication = app(FormPublishingService::class)->status($form);
        $siteFormKeys = AlsernetForm::query()
            ->where(fn ($q) => $q->whereNull('form_id')->orWhere('form_id', $form->id))
            ->orderBy('name')
            ->get(['form_key', 'name']);

        return view('forms::forms.edit', compact(
            'form', 'categories', 'previewToken', 'fieldTypeSettings', 'publication', 'siteFormKeys'
        ));
    }
                ...
                foreach ($incomingFields as $sortOrder => $fieldData) {
                    ...
                    $form->fields()->create(
                        collect($fieldData)->only([
                            ...
                            'show_char_counter', 'likert_rows', 'exclude_from_helpdesk',
