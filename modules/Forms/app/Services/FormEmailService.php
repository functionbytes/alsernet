...
class FormEmailService
{
    ...
    public function sendConfirmation(Form $form, FormSubmission $submission, string $email): bool
    {
        try {
            ...
            // El asunto y el cuerpo configurados en el formulario mandan sobre
            // los genéricos: el de PQRSF, por ejemplo, no dice lo mismo que el
            // de una cotización.
            $subject = $form->confirmation_subject ?: "Hemos recibido tu mensaje - {$form->name}";

            Mail::send('forms::emails.confirmation', [
                'form' => $form,
                'submission' => $submission,
                'successMessage' => $form->confirmation_message
                    ?: $form->success_message
                    ?: config('forms.default_success_message'),
    ...
    protected function buildTemplateVariables(Form $form, FormSubmission $submission): array
    {
        ...
        // Los datos de contacto se leen de los ajustes del panel (los mismos que
        // alimentan el schema.org del sitio). Antes estaban escritos a mano con la
        // ficha de otra empresa, asi que cada correo de formulario salia con un
        // telefono y una direccion de Portugal.
        $logoUrl = function_exists('getLogo') ? (string) getLogo() : '';
        ...
        return [
            ...
            '{COMPANY_PHONE}' => (string) setting('seo.schema.local_business.phone', '+573228513175'),
            '{COMPANY_EMAIL}' => (string) setting('seo.schema.local_business.email', 'comercial@inoqualab.com'),
            '{COMPANY_ADDRESS}' => (string) setting('seo.schema.local_business.street', 'Cra 22 # 35 - 40 Int. 224, Edificio Apolo'),
            '{COMPANY_CITY}' => (string) setting('seo.schema.local_business.city', 'Bucaramanga'),
            '{COMPANY_POSTAL_CODE}' => (string) setting('seo.schema.local_business.postal_code', ''),
            '{COMPANY_COUNTRY}' => (string) setting('seo.schema.local_business.country_name', 'Colombia'),
            '{CURRENT_YEAR}' => date('Y'),
