...
| Operational routes use /panel/documents prefix
| Configuration routes use /panel/settings/documents prefix
...
Route::middleware(['web', 'auth'])->group(function () {
    ...
    Route::prefix('panel/documents')->name('documents.')->group(function () {
        ...
        Route::get('/upload/{document}', [DocumentsController::class, 'getUploadUrl'])->name('documents.upload-url');
        // Document views
    ...
    // ====================================================================
    // CONFIGURATION ROUTES - /panel/settings/documents (Admin only)
    // ====================================================================
    Route::prefix('panel/settings/documents')->name('settings.documents.')
        ...
        ->group(function () {
            ...
            Route::prefix('configurations')->name('configurations.')->group(function () {
                ...
                // Sync schedule
                Route::get('/sync-schedule', [DocumentConfigurationController::class, 'syncScheduleSettings'])->name('sync-schedule');
                Route::patch('/sync-schedule', [DocumentConfigurationController::class, 'updateSyncScheduleSettings'])->name('sync-schedule.update');

                // Endpoints / Integraciones
                Route::get('/endpoints', [DocumentConfigurationController::class, 'endpointsSettings'])->name('endpoints');
                Route::patch('/endpoints', [DocumentConfigurationController::class, 'updateEndpointsSettings'])->name('endpoints.update');
                Route::post('/endpoints/test', [DocumentConfigurationController::class, 'testEndpoint'])->name('endpoints.test');
    ...
    // Product Blockades - accesible por super-admin y supervisor
    Route::prefix('panel/settings/documents/blockades')->name('settings.documents.blockades.')
        ->middleware(['role:super-admin|supervisor'])
        ->group(function () {
            Route::get('/', [DocumentProductBlockadeController::class, 'index'])->name('index');
            Route::get('/products', [DocumentProductBlockadeController::class, 'products'])->name('products');
