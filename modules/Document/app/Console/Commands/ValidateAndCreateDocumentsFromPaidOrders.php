    ...
    private function fetchRealCustomerData(int $customerId): ?array
    {
        try {
            ...
            if ($isAnonymous) {
                ...
                if ($realData) {
                    // Corrige el email/nombre del cliente anónimo en PrestaShop vía
                    // el bridge (antes: UPDATE directo a aalv_customer). El bridge
                    // re-verifica server-side que el email actual sigue siendo
                    // anon_* antes de escribir — no hace falta duplicar ese chequeo
                    // aquí (ver alsernet_customer_fix_anonymous_profile).
                    app(PrestashopOrderLookupService::class)->fixAnonymousCustomer(
                        $customerId,
                        (string) $realData['firstname'],
                        (string) $realData['lastname'],
                        (string) $realData['email'],
                    );
