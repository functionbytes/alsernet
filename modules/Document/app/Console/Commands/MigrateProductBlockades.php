...
class MigrateProductBlockades extends Command
{
    ...
    public function handle(): int
    {
        ...
        Setting::set('documents.product_blockades_sync_count', (string) DocumentProductBlockade::count());
    ...
    private function migrateFromTable(string $tableName, string $origenColumn, string $productColumn, string $blockadeLabels, string $type): int
    {
        ...
        foreach ($labels as $label) {
            ...
            $documentTypeId = $this->blockadeTypeMapping[$blockadeType];

            // Build query
            $query = DB::connection('prestashop')->table($tableName);

            if ($type === 'combination') {
                $query->leftJoin('aalv_product_attribute as apa', 'apa.id_product_attribute', '=', "{$tableName}.{$productColumn}")
                    ->select(
                        "{$tableName}.{$origenColumn}",
                        "{$tableName}.{$productColumn}",
                        "{$tableName}.etiqueta",
                        'apa.id_product'
                    );
            } else {
                $query->select($origenColumn, $productColumn, 'etiqueta');
            }

            // ✅ Mejor filtro para etiquetas
            $products = $query->where("{$tableName}.etiqueta", 'LIKE', "%{$label}%")
                ->get();
            ...
            foreach ($products as $product) {
                ...
                $productAttributeId = $product->{$productColumn};
                ...
                if ($type === 'product') {
                    $data['product_id'] = $productAttributeId;
                ...
                // ✅ Verificación completa de duplicados (incluye source_id)
                $exists = DocumentProductBlockade::where('product_id', $data['product_id'])
                    ...
                    ->where('source_id', $data['source_id']) // ✅ Agregado
                    ->exists();
