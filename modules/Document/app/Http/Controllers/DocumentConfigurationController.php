...
class DocumentConfigurationController extends Controller
{
    ...
    /**
     * Mostrar panel de programación de sincronización de bloqueos
     */
    public function syncScheduleSettings(): View
    {
        $syncSettings = $this->getSyncScheduleSettings();

        return view('documents::settings.configurations.sync-schedule', [
            'syncSettings' => $syncSettings,
        ]);
    }

    /**
     * Actualizar configuración de programación de sincronización
     */
    public function updateSyncScheduleSettings(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            'blockade_sync_enabled' => 'boolean',
            'blockade_sync_frequency' => 'required|string|in:manual,hourly,every_2_hours,every_6_hours,every_12_hours,daily,weekly,custom',
            'blockade_sync_hour' => 'nullable|string|regex:/^\d{2}:\d{2}$/',
            'blockade_sync_cron' => 'nullable|string|max:100',
            'blockade_sync_fresh' => 'boolean',
        ]);

        if (($validated['blockade_sync_frequency'] === 'custom') && empty($request->input('blockade_sync_cron'))) {
            return back()->withInput()->with('error', 'Debes introducir una expresión cron cuando seleccionas frecuencia personalizada.');
        }

        try {
            Setting::set('documents.blockade_sync_enabled', $request->boolean('blockade_sync_enabled') ? 'yes' : 'no');
            Setting::set('documents.blockade_sync_frequency', $validated['blockade_sync_frequency']);
            Setting::set('documents.blockade_sync_hour', $validated['blockade_sync_hour'] ?? '08:00');
            Setting::set('documents.blockade_sync_cron', $validated['blockade_sync_cron'] ?? '');
            Setting::set('documents.blockade_sync_fresh', $request->boolean('blockade_sync_fresh') ? 'yes' : 'no');
    ...
    /**
     * Obtener configuración de programación de sincronización de bloqueos
     */
    public function getSyncScheduleSettings(): array
    {
        $lastSync = Setting::get('documents.product_blockades_last_sync');
        $syncCount = Setting::get('documents.product_blockades_sync_count');
        $frequency = Setting::get('documents.blockade_sync_frequency', 'manual');
        $hour = Setting::get('documents.blockade_sync_hour', '08:00');
        $enabled = Setting::get('documents.blockade_sync_enabled', 'no') === 'yes';

        $nextRun = 'No programado';
        ...
        return [
            ...
            'blockade_sync_frequency' => $frequency,
    ...
    /**
     * Calcular próxima ejecución estimada según la frecuencia configurada
     */
    private function calculateNextRun(string $frequency, string $hour, string $cron): string
    {
        try {
            ...
            return match ($frequency) {
                'hourly' => $now->copy()->addHour()->setMinute($m)->setSecond(0)->format('d/m/Y H:i'),
    ...
    /**
     * Mostrar panel de configuración de endpoints externos.
     */
    public function endpointsSettings(): View
    {
        return view('documents::settings.configurations.endpoints', [
            'endpoints' => $this->getEndpointsSettings(),
        ]);
    }

    /**
     * Actualizar URLs de endpoints externos.
     */
    public function updateEndpointsSettings(Request $request): RedirectResponse
    {
        $validated = $request->validate([
        ...
        try {
            ...
            return redirect()
                ->back()
                ->with('success', 'Configuración de endpoints actualizada correctamente.');
        } catch (\Exception $e) {
            return back()
                ->withInput()
    ...
    /**
     * Obtener valores actuales de endpoints.
     */
    public function getEndpointsSettings(): array
    {
        return [
