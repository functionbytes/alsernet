...
class AdminAlsernetRelacionadosController extends ModuleAdminController
{
    ...
    public function displayAjaxLoadFilters()
    {
        ...
        if (! $id) {
            exit(json_encode(['ok' => false, 'message' => $this->l('Referencia no encontrada.')]));
        }
        ...
        if (! $data) {
            exit(json_encode(['ok' => false, 'message' => $this->l('No se pudo obtener datos del producto.')]));
        }
    ...
    public function displayAjaxSearchRelated()
    {
        ...
        exit(json_encode(['ok' => true, 'payload' => $res]));
    }
}
