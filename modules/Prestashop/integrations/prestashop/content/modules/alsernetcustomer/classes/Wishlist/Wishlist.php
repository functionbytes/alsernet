...
class Wishlist extends ObjectModel
{
    ...
    public static function exists($id_wishlist, $id_customer, $return = false)
    {
        ...
        if (empty($result) === false and $result != false and count($result)) {
            if ($return === false) {
                return true;
            } else {
                return $result;
    ...
    public static function getSimpleProductByIdCustomer($id_customer, $id_lang, $id_shop)
    {
        ...
        if (empty($wishlists) === true or ! count($wishlists)) {
            return [];
        }
    ...
    public static function addProduct($id_wishlist, $id_product, $id_product_attribute)
    {
        ...
        if (empty($result) === false and count($result)) {
    ...
    public function setDefault()
    {
        if ($default = $this->getDefault($this->id_customer)) {
            Db::getInstance()->update('leofeature_wishlist', ['default' => '0'], 'id_wishlist = '.(int) $default[0]['id_wishlist']);
        }

        return Db::getInstance()->update('leofeature_wishlist', ['default' => '1'], 'id_wishlist = '.(int) $this->id);
    }
