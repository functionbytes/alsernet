    ...
    public function initContent()
    {
        ...
        if (Tools::isSubmit('submitLoad') && $this->isTokenValid()) {
            if ($id_product <= 0 || ! (new Product($id_product))->id) {
                ...
                [$groups, $attributes] = $this->loadProductAttributes($id_product, $languages);
        ...
        if ($groupIds) {
            ...
            foreach ($groups as $gid => &$gdata) {
                foreach ($id_langs as $id_lang) {
                    if (! isset($gdata['names'][$id_lang])) {
                        $gdata['names'][$id_lang] = '';
                    }
                    if (! isset($gdata['public_names'][$id_lang])) {
                        $gdata['public_names'][$id_lang] = '';
                    }
                }
            }
        }
