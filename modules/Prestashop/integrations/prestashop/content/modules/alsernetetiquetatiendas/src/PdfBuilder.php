...
class PdfBuilder
{
    ...
    public function __construct($arg1 = null, $arg2 = null)
    {
        ...
        $this->settings = is_array($arg1) ? $arg1 : (is_array($arg2) ? $arg2 : []);
    ...
    public function buildHorizontal(array $rows, $outputPdf)
    {
        ...
        $pdf = new TCPDF('L', 'mm', [$pageW, $pageH], true, 'UTF-8');
        $pdf->SetCreator('Alsernet');
        $pdf->SetAuthor('Alsernet');
        $pdf->SetMargins(0, 0, 0);
        $pdf->SetAutoPageBreak(false, 0);
    ...
    public function buildVertical(array $rows, $bgPath, $outputPdf)
    {
        ...
        foreach ($chunks as $chunkIdx => $chunk) {
            ...
            for ($slot = 1; $slot <= 4; $slot++) {
                ...
                // ================= LABEL =================
                [$fam, $sty] = $this->fontSpec('font_label_family', 'helvetica');
                ...
                // ================= REFERENCIA =================
                [$fam, $sty] = $this->fontSpec('font_referencia_family', 'helvetica');
                ...
                [$fam, $sty] = $this->fontSpec('font_descripcion_family', 'helvetica');
                ...
                // ================= PVPRP =================
                [$fam, $sty] = $this->fontSpec('font_pvprp_family', 'helvetica');
    ...
    {
        [$r, $g, $b] = $this->hexToRgb($hexColor ?: '#000000');
    ...
    {
        $s = trim(str_replace(['€', 'EUR', 'eur', 'Eur'], '', (string) $raw));
