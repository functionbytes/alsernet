        ...
        <div class="card">
            ...
            <div class="card-header p-4 border-bottom border-light">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="mb-1 fw-bold">Sedes de atención</h5>
                        <p class="small mb-0 text-muted">Puntos físicos donde se atienden solicitudes presenciales</p>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="{{ route('settings.attention.sedes.create') }}" class="btn btn-primary">
                    ...
                    <div class="col-md-3">
                        <div class="card bg-light-secondary h-100">
                            <div class="card-body">
                                ...
                                <small class="text-muted">Sedes activas</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card bg-light-secondary h-100">
                            <div class="card-body">
                                ...
                                <small class="text-muted">Sedes inactivas</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card bg-light-secondary h-100">
                            <div class="card-body">
                                <h6 class="card-title mb-2">Ciudades</h6>
                                <h4 class="mb-1 fw-bold">{{ $stats['cities'] }}</h4>
                                <small class="text-muted">Ciudades distintas</small>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {{-- Search --}}
            <div class="card-body border-bottom">
                <form method="GET" action="{{ route('settings.attention.sedes.index') }}">
                    <div class="d-flex flex-column flex-lg-row gap-3 align-items-stretch">
                        <div class="flex-fill">
                            <div class="input-group h-100">
                                ...
                                <input type="search" name="search" class="form-control border-start-0 ps-0"
                                       placeholder="Buscar por nombre, ciudad, dirección o teléfono..."
                        ...
                        <div class="d-flex gap-2 flex-shrink-0">
                            ...
                            @if(request('search') || request('status'))
                                <a href="{{ route('settings.attention.sedes.index') }}" class="btn btn-outline-secondary" title="Limpiar filtros">
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                @foreach($sedes as $sede)
                                    <tr>
                                        <td><input type="checkbox" class="form-check-input bulk-checkbox" value="{{ $sede->id }}"></td>
                                        <td><strong>{{ $sede->name }}</strong></td>
                                        <td><small class="text-muted">{{ $sede->city ?? '-' }}</small></td>
                                        ...
                                        <td>
                                            <span class="text-muted">{{ $sede->created_at->format('d/m/Y') }}</span>
                                        </td>
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    <li>
                                                        <a class="dropdown-item" href="{{ route('settings.attention.sedes.edit', $sede->id) }}">
                                                            Editar
                                                        </a>
                                                    </li>
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item delete-btn"
                                                           ...
                                                           data-url="{{ route('settings.attention.sedes.destroy', $sede->id) }}"
                                                           data-title="Eliminar: {{ $sede->name }}">
                                                            Eliminar
                                                        </a>
                                                    </li>
                                                </ul>
                    ...
                    <div class="text-center py-5">
                        <div class="d-flex flex-column align-items-center">
                            ...
                            <p class="text-muted mb-3">
                                ...
                                @else
                                    Crea la primera sede para empezar a gestionar atenciones presenciales
                                @endif
                            </p>
                            @if(!request('search') && !request('status'))
                                <a href="{{ route('settings.attention.sedes.create') }}" class="btn btn-sm btn-primary">
                                    <i class="fas fa-plus me-1"></i> Nueva sede
                                </a>
                            @endif
                        </div>
            ...
            @if($sedes->hasPages())
                <div class="card-footer">{{ $sedes->links() }}</div>
            @endif
...
$(document).ready(function () {
    ...
    $('#bulk-apply-btn').on('click', function () {
        ...
        $.ajax({
            url: '{{ route('settings.attention.sedes.bulk-action') }}',
            method: 'POST',
