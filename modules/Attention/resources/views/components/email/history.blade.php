...
<div class="card mb-3">
    <div class="card-header p-3 bg-white border-bottom d-flex justify-content-between align-items-center">
        <div>
            ...
            <p class="small mb-0 text-muted">Historial de correos electrónicos enviados al ciudadano</p>
        </div>
    </div>

    @php
        $recentMails = $attention->mails()->orderBy('created_at', 'desc')->take(3)->get();
        $totalMails = $attention->mails()->count();
    @endphp
        ...
        <div class="card-body p-4">
            ...
            <div class="email-history-list">
                @foreach($recentMails as $mail)
                    @php
                        $typeConfig = [
                            'confirmation' => ['icon' => 'fa-duotone fa-check-circle', 'bg' => 'success-subtle', 'text' => 'success'],
                            'in_process' => ['icon' => 'fa-duotone fa-clock', 'bg' => 'info-subtle', 'text' => 'info'],
                            'resolved' => ['icon' => 'fa-duotone fa-check', 'bg' => 'success-subtle', 'text' => 'success'],
                            'custom' => ['icon' => 'fa-duotone fa-envelope', 'bg' => 'secondary-subtle', 'text' => 'secondary'],
                        ];
                        $config = $typeConfig[$mail->email_type] ?? $typeConfig['custom'];
                    @endphp
                    <a href="{{ route('attention.emails.preview', [$attention->uid, $mail->uid]) }}" class="email-item d-block text-decoration-none" target="_blank">
                        <div class="d-flex gap-3 align-items-start">
                            <div class="flex-grow-1 min-width-0">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <span class="fw-semibold small text-dark fs-2">{{ $mail->email_type_label ?? 'Email' }}</span>
                                    @if($mail->status === 'sent')
            ...
            <div class="border-top mt-3 pt-3">
                <a href="{{ route('attention.emails.index', $attention->uid) }}" class="btn btn-primary w-100">
