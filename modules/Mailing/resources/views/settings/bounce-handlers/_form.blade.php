<form method="POST"
      action="{{ isset($handler) ? route('settings.mailing.bounce-handlers.update', $handler->uid) : route('settings.mailing.bounce-handlers.store') }}"
      id="formBounceHandler">
                ...
                <div class="col-12 col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="email"
                               ...
                               placeholder="rebotes@example.com">
                        @error('email')
                            <span class="field-validation-error"><i class="fa fa-circle-exclamation"></i> {{ $message }}</span>
                        @enderror
                    </div>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="row">
                ...
                {{-- Auto Unsubscribe Hard Bounce --}}
                <div class="col-12 col-md-6">
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input"
                               type="checkbox"
                               id="auto_unsubscribe_hard_bounce"
                               name="auto_unsubscribe_hard_bounce"
                               value="1"
                               {{ old('auto_unsubscribe_hard_bounce', $handler->auto_unsubscribe_hard_bounce ?? true) ? 'checked' : '' }}>
                        <label class="form-check-label" for="auto_unsubscribe_hard_bounce">
                            Desuscribir automáticamente en rebote duro
                        </label>
                    </div>
                </div>
                ...
                <div class="col-12 col-md-6">
                    <div class="mb-3">
                        <label for="soft_bounce_limit" class="form-label">
                            Límite de rebotes suaves
                        </label>
                        <input type="number"
                               class="form-control @error('soft_bounce_limit') is-invalid @enderror"
                               id="soft_bounce_limit"
                               name="soft_bounce_limit"
                               value="{{ old('soft_bounce_limit', $handler->soft_bounce_limit ?? 3) }}"
                               placeholder="3"
                               min="1">
                        @error('soft_bounce_limit')
                            <span class="field-validation-error"><i class="fa fa-circle-exclamation"></i> {{ $message }}</span>
                        @enderror
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleConfigCards() {
        ...
        } else if (typeSelect.value === 'webhook') {
