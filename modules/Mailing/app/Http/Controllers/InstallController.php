...
use Modules\Mailing\Models\Notification as AppNotification;
...
class InstallController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        $language = \Modules\Mailing\Models\Language::getFirstLanguageByCode($locale);

        $admin = new \Modules\Mailing\Models\Admin;
        $admin->user_id = 1;
        $admin->admin_group_id = 1;
        $admin->language_id = $language->id;
        $admin->timezone = config('app.timezone');
        $admin->status = 'active';
        $admin->menu_layout = \Modules\Mailing\Models\Setting::get('layout.menu_bar');
        $admin->save();
        ...
        \Modules\Mailing\Models\Setting::set('url_delivery_handler', action('DeliveryController@notify', ['stype' => '']));
        \Modules\Mailing\Models\Setting::set('url_unsubscribe', action('CampaignController@unsubscribe', ['message_id' => 'MESSAGE_ID', 'subscriber' => 'SUBSCRIBER']));
        \Modules\Mailing\Models\Setting::set('url_open_track', action('CampaignController@open', ['message_id' => 'MESSAGE_ID']));
        \Modules\Mailing\Models\Setting::set('url_web_view', action('CampaignController@webView', ['message_id' => 'MESSAGE_ID']));
        \Modules\Mailing\Models\Setting::set('url_click_track', action('CampaignController@click', ['message_id' => 'MESSAGE_ID', 'url' => 'URL']));
        \Modules\Mailing\Models\Setting::set('url_update_profile', action('PageController@profileUpdateForm', ['list_uid' => 'LIST_UID', 'uid' => 'SUBSCRIBER_UID', 'code' => 'SECURE_CODE']));

        // Site info settings
        \Modules\Mailing\Models\Setting::set('site_name', $site_info['site_name']);
        \Modules\Mailing\Models\Setting::set('site_keyword', $site_info['site_keyword']);
        \Modules\Mailing\Models\Setting::set('site_description', $site_info['site_description']);
        \Modules\Mailing\Models\Setting::set('license', $site_info['license']);
        // \Modules\Mailing\Models\Setting::set('license_type', $site_info['license_type']);

        // mail setting
        \Modules\Mailing\Models\Setting::set('mailer.mailer', $site_info['mail_mailer']);
        \Modules\Mailing\Models\Setting::set('mailer.host', $site_info['smtp_hostname']);
        \Modules\Mailing\Models\Setting::set('mailer.port', $site_info['smtp_port']);
        \Modules\Mailing\Models\Setting::set('mailer.encryption', $site_info['smtp_encryption']);
        \Modules\Mailing\Models\Setting::set('mailer.username', $site_info['smtp_username']);
        \Modules\Mailing\Models\Setting::set('mailer.password', $site_info['smtp_password']);
        \Modules\Mailing\Models\Setting::set('mailer.from.name', $site_info['mail_from_name']);
        \Modules\Mailing\Models\Setting::set('mailer.from.address', $site_info['mail_from_email']);
        \Modules\Mailing\Models\Setting::set('mailer.sendmail_path', $site_info['sendmail_path']);

        // Generate translation files if not exist
        // Cashier plugin for example
        \Modules\Mailing\Models\Language::dump();
    ...
    public function cronJobs(Request $request)
    {
        ...
        // $respone = \Modules\Mailing\Library\Tool::cronjobUpdateController($request, $this);
