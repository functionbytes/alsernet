...
use Modules\Mailing\Library\Downloader;
...
class SettingController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->user()->admin->getPermission('setting_general') == 'yes') {
            return redirect()->action('Settings\SettingController@general');
            // } elseif ($request->user()->admin->getPermission('setting_sending') == 'yes') {
            //   return redirect()->action('Settings\SettingController@sending');
        } elseif ($request->user()->admin->getPermission('setting_system_urls') == 'yes') {
            return redirect()->action('Settings\SettingController@urls');
        } elseif ($request->user()->admin->getPermission('setting_background_job') == 'yes') {
            return redirect()->action('Settings\SettingController@cronjob');
        }
    }
    ...
    public function cronjob(Request $request)
    {
        ...
        if ($respone == 'done' || $respone['valid'] == true) {
            $next = action('Settings\SettingController@cronjob').'#result_box';
    ...
    public function mailer(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            // Redirect to my lists page
            $next = action('Settings\SettingController@mailer');
            $request->session()->flash('alert-success', trans('messages.setting.updated'));
    ...
    public function updateUrls(Request $request)
    {
        ...
        \Modules\Mailing\Helpers\reset_app_url(true); // force update
        ...
        Setting::set(
            'url_update_profile',
            action('PageController@profileUpdateForm', [
                'list_uid' => 'LIST_UID',
                'uid' => 'SUBSCRIBER_UID',
                'code' => 'SECURE_CODE', ])
        );
    ...
    public function license(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                return redirect()->action('Settings\SettingController@license');
            } catch (\Throwable $ex) {
    ...
    public function doUpgrade(Request $request)
    {
        ...
        if (empty($failed)) {
            ...
            //     $pageUrl = action('Settings\Upgrade@migrate');
            // Use route()
    ...
    {
        if (is_null(\Modules\Mailing\Helpers\LicenseHelper::getCurrentLicense())) {
    ...
    public function advanced(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return redirect()->action('Settings\SettingController@advanced');
        }
