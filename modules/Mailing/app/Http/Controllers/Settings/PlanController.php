...
class PlanController extends Controller
{
    ...
    {
        // authorize
        if (\Gate::denies('read', new PlanGeneral)) {
            return $this->notAuthorized();
        }

        // If admin can view all sending domains
        if (! $request->user()->admin->can('readAll', new PlanGeneral)) {
    ...
    public function store(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return redirect()->action('Settings\PlanController@index');
        }
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (isset($request->old()['sending_servers'])) {
            ...
            foreach ($request->old()['sending_servers'] as $key => $param) {
                if ($param['check']) {
                    $server = \Modules\Mailing\Models\SendingServer::findByUid($key);
                    $row = new \Modules\Mailing\Models\PlansSendingServer;
                    $row->plan_id = $plan->id;
    ...
    {
        // Get current user
        $plan = PlanGeneral::findByUid($uid);
        ...
        if ($request->use_system_sending_server) {
            return redirect()->action('Settings\PlanController@sendingServers', $plan->uid);
        }
    ...
    {
        echo PlanGeneral::select2($request);
    }
    ...
    public function pieChart(Request $request)
    {
        ...
        $datas = [];
        foreach (PlanGeneral::active()->get() as $plan) {
    ...
    {
        $plan = PlanGeneral::findByUid($request->copy_plan_uid);
    ...
    public function sendingServer(Request $request, $uid)
    {
        ...
        if ($plan->getOption('sending_server_option') == PlanGeneral::SENDING_SERVER_OPTION_SYSTEM) {
            return redirect()->action('Settings\PlanController@sendingServers', $plan->uid);
        } elseif ($plan->getOption('sending_server_option') == PlanGeneral::SENDING_SERVER_OPTION_OWN) {
            return redirect()->action('Settings\PlanController@sendingServerOwn', $plan->uid);
        } elseif ($plan->getOption('sending_server_option') == PlanGeneral::SENDING_SERVER_OPTION_SUBACCOUNT) {
            return redirect()->action('Settings\PlanController@sendingServerSubaccount', $plan->uid);
        }
    }
    ...
    {
        $plan = PlanGeneral::findByUid($request->uid);
    ...
    {
        if (! $request->uid) {
            $plan = PlanGeneral::newDefaultPlan();
        } else {
            $plan = PlanGeneral::findByUid($request->uid);
        }
        ...
        if ($request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                return redirect()
                    ->action('Settings\PlanController@wizard')
            ...
            return redirect()->action('Settings\PlanController@wizardSendingServer', $plan->uid);
        }
    ...
    public function wizardSendingServer(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                return redirect()
                    ->action('Settings\PlanController@wizardSendingServer', $plan->uid)
                    ->withErrors($validator)
                    ->withInput();
            }
    ...
    public function sendingServerOption(Request $request, $uid)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return redirect()->action('Settings\PlanController@sendingServer', $plan->uid);
        }
