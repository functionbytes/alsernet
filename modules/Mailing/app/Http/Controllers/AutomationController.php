...
use Modules\Mailing\Jobs\ForceTriggerAutomation;
use Modules\Mailing\Models\Attachment;
use Modules\Mailing\Models\Automation;
use Modules\Mailing\Models\Email;
...
class Automation2Controller extends Controller
{
    ...
    {
        // authorize
        if (\Gate::denies('list', Automation::class)) {
    ...
    {
        $types = Automation::getTriggerTypes();
    ...
    public function update(Request $request, $uid)
    {
        ...
        // find automation
        $automation = Automation::findByUid($uid);
    ...
    public function actionSelect(Request $request, $uid)
    {
        ...
        if ($request->key == 'wait') {
            $delayOptions = \Modules\Mailing\Models\Automation::getDelayOptions();
    ...
    {
        $automation = Automation::findByUid($request->uid);
        $email = Email::findByUid($request->email_uid);
    ...
    {
        // Japan + not license
        if (config('custom.japan') && ! \Modules\Mailing\Helpers\LicenseHelper::hasActiveLicense()) {
            return response()->json([
                'status' => 'error',
        ...
        $automations = Automation::whereIn(
            'uid',
    ...
    public function debug(Request $request, $uid)
    {
        ...
        switch ($type) {
            case Automation::TRIGGER_TYPE_WELCOME_NEW_SUBSCRIBER:
            ...
            case Automation::TRIGGER_TYPE_SAY_GOODBYE_TO_SUBSCRIBER:
            ...
            case Automation::TRIGGER_TYPE_SAY_HAPPY_BIRTHDAY:
    ...
    {
        $automation = Automation::findByUid($request->automation);
    ...
    {
        $webhook = \Modules\Mailing\Models\EmailWebhook::findByUid($request->webhook_uid);
