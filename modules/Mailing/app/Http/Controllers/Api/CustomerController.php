...
use Modules\Mailing\Library\TransactionResult;
use Modules\Mailing\Models\Customer;
use Modules\Mailing\Models\SubscriptionLog;
...
class CustomerController extends Controller
{
    ...
    {
        $customers = \Modules\Mailing\Models\Customer::select('customers.*')
            ->get();
    ...
    public function assignPlan(Request $request, $uid, $plan_uid)
    {
        $user = \Auth::guard('api')->user();
        $customer = \Modules\Mailing\Models\Customer::findByUid($uid);
        $offlineGateway = \Modules\Mailing\Library\Facades\Billing::getGateway('offline');
        ...
        $plan = \Modules\Mailing\Models\PlanGeneral::findByUid($plan_uid);
        ...
        if ($request->disable_billing && $request->disable_billing !== 'false') {
            ...
            if (! $offlineGateway->isActive() || ! \Modules\Mailing\Library\Facades\Billing::isGatewayEnabled($offlineGateway)) {
        ...
        \DB::transaction(function () use ($plan, $customer, $request, $offlineGateway) {
            ...
            if ($request->disable_billing && $request->disable_billing !== 'false') {
                $subscription->getItsOnlyUnpaidInitInvoice()->checkout($offlineGateway, function () {
                    return new \Modules\Mailing\Library\TransactionResult(\Modules\Mailing\Library\TransactionResult::RESULT_DONE);
                });
            }
        });
