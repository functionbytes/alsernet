...
class SubscriberController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // authorize
        if (! $list) {
            return \Response::json(['message' => trans('List not found')], 400);
        }
        // authorize
        if (! $user->can('read', $list)) {
            return \Response::json(['message' => 'You\'re not authorized to access this resources'], 401);
        }
        if (isset($request->per_page)) {
            $per_page = $request->per_page;
        } else {
            $per_page = \Modules\Mailing\Models\Subscriber::$itemsPerPage;
        }

        $items = \Modules\Mailing\Models\Subscriber::search($request, $user->customer)
    ...
    public function store(Request $request)
    {
        try {
            ...
            [$validator, $subscriber] = $list->subscribe($request, MailList::SOURCE_API);
    ...
    public function show($uid)
    {
        ...
        $item = \Modules\Mailing\Models\Subscriber::where('uid', '=', $uid)
            ->first();
    ...
    {
        $user = \Auth::guard('api')->user();
        $subscriber = \Modules\Mailing\Models\Subscriber::findByUid($uid);
        ...
        if ($subscriber->status == 'subscribed') {
            return \Response::json(['status' => 0, 'message' => 'Already subscribed'], 409);
        }
        ...
        return \Response::json(['status' => 1, 'message' => 'Subscribed'], 200);
    }
    ...
    public function unsubscribe($list_uid, $uid)
    {
        ...
        if ($subscriber->status == 'unsubscribed') {
            return \Response::json(['status' => 0, 'message' => 'Already unsubscribed'], 409);
        }
        ...
        return \Response::json(['status' => 1, 'message' => 'Unsubscribed'], 200);
    }
    ...
    public function unsubscribeEmail($list_uid, $email)
    {
        ...
        if (! filter_var($email, FILTER_VALIDATE_EMAIL)) {
            return \Response::json(['status' => 0, 'message' => 'Email is not valid'], 400);
        }

        $user = \Auth::guard('api')->user();
        $list = \Modules\Mailing\Models\MailList::findByUid($list_uid);
        ...
        return \Response::json(['status' => 1, 'message' => "{$email} was unsubscribed"], 200);
    }
    ...
    public function showByEmail($email)
    {
        ...
        $subscribers = \Modules\Mailing\Models\Subscriber::where('email', '=', $email)->get();
    ...
    public function update(Request $request, $uid)
    {
        ...
        // update MailList cache
        event(new \Modules\Mailing\Events\MailListUpdated($subscriber->mailList));
