...
class UserController extends Controller
{
    ...
    {
        $userActivation = \App\Models\UserActivation::where('token', '=', $token)->first();
        ...
        if (! $userActivation) {
            ...
            // assignPlan
            if ($request->plan_uid) {
                $userActivation->user->customer->assignGeneralPlan(\Modules\Mailing\Models\PlanGeneral::findByUid($request->plan_uid));
            }
    ...
    {
        if (\Modules\Mailing\Models\Setting::get('enable_user_registration') == 'no') {
        ...
        if ($request->isMethod('post')) {
            ...
            // Captcha check
            if (\Modules\Mailing\Models\Setting::get('registration_recaptcha') == 'yes') {
                // @hCaptcha
