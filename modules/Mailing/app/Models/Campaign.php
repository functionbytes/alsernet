...
use Modules\Mailing\Jobs\ExecuteCampaignCallback;
use Modules\Mailing\Jobs\SendMessage;
use Modules\Mailing\Library\BaseCampaign;
use Modules\Mailing\Library\Contracts\CampaignInterface;
use Modules\Mailing\Library\Contracts\HasTemplateInterface;
use Modules\Mailing\Library\HtmlHandler\InjectTrackingPixel;
use Modules\Mailing\Library\HtmlHandler\TransformUrl;
use Modules\Mailing\Library\RouletteWheel;
use Modules\Mailing\Library\StringHelper;
use Modules\Mailing\Library\Traits\HasTemplate;
use Carbon\Carbon;
use Closure;
use DB;
use Exception;
...
class Campaign extends BaseCampaign implements CampaignInterface, HasTemplateInterface
{
    ...
    public function log($name, $customer, $add_datas = [])
    {
        ...
        \Modules\Mailing\Models\Log::create([
    ...
    public function fillAttributes($params)
    {
        ...
        if (isset($params['custom_tracking_domain']) && $params['custom_tracking_domain'] && isset($params['tracking_domain_uid'])) {
            $tracking_domain = \Modules\Mailing\Models\TrackingDomain::findByUid($params['tracking_domain_uid']);
            if ($tracking_domain) {
    ...
    {
        $webhook = new \Modules\Mailing\Models\CampaignWebhook;
    ...
    public function saveFromArray($params)
    {
        ...
        if (isset($params['mail_list_uid'])) {
            $this->default_mail_list_id = \Modules\Mailing\Models\MailList::findByUid($params['mail_list_uid'])->id;
        }
    ...
    public function makeSampleData($params = [])
    {
        ...
        foreach ($this->trackingLogs()->whereIn('id', $openLogs)->get() as $r) {
            $log = new \Modules\Mailing\Models\OpenLog;
            $log->message_id = $r->message_id;
            $location = \Modules\Mailing\Models\IpLocation::add(StringHelper::getRandomUSIpAddresses());
        ...
        foreach ($this->trackingLogs()->whereIn('id', $clickLogs)->get() as $r) {
            $log = new \Modules\Mailing\Models\ClickLog;
            $log->message_id = $r->message_id;
            $location = \Modules\Mailing\Models\IpLocation::add(StringHelper::getRandomUSIpAddresses());
            $log->ip_address = $location->ip_address;
            $log->user_agent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.36';
        ...
        foreach ($this->trackingLogs()->whereIn('id', $unsubscribeLogs)->get() as $r) {
            // Unsubscribe log
            $log = new \Modules\Mailing\Models\UnsubscribeLog;
            $log->message_id = $r->message_id;
