...
use Modules\Mailing\Library\Automation\Send;
use Modules\Mailing\Library\Automation\Trigger;
use Modules\Mailing\Library\Automation\Wait;
...
class Automation2 extends Model
{
    ...
    public function fillRequest($request)
    {
        ...
        if (! empty($request->segment_uid)) {
            foreach ($request->segment_uid as $segmentUid) {
                $segments[] = \Modules\Mailing\Models\Segment::findByUid($segmentUid)->id;
            }
    ...
    public function getSegments()
    {
        ...
        $segments = \Modules\Mailing\Models\Segment::whereIn('id', explode(',', $this->segment_id))->get();
    ...
    public function getAbandonedCartEmail()
    {
        ...
        return \Modules\Mailing\Models\Email::findByUid($this->getElements()[1]->options->email_uid);
    }
