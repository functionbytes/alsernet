...
class Plan extends Model
{
    ...
    public function mapType()
    {
        //
        switch ($this->type) {
            case \Modules\Mailing\Models\PlanGeneral::TYPE_GENERAL:
                return \Modules\Mailing\Models\PlanGeneral::find($this->id);
                break;
            case \Modules\Mailing\Models\PlanNumber::TYPE_NUMBER:
                return \Modules\Mailing\Models\PlanNumber::find($this->id);
                break;
            case \Modules\Mailing\Models\PlanSenderId::TYPE_SENDER_ID:
                return \Modules\Mailing\Models\PlanSenderId::find($this->id);
                break;
            case \Modules\Mailing\Models\PlanKeyword::TYPE_KEYWORD:
                return \Modules\Mailing\Models\PlanKeyword::find($this->id);
                break;
            default:
    ...
    public static function select2($request)
    {
        ...
        // Read all check
        if ($request->user()->admin && ! $request->user()->admin->can('readAll', \Modules\Mailing\Models\PlanGeneral::newGeneral())) {
        ...
        if ($request->change_from_uid) {
            $plan = \Modules\Mailing\Models\PlanGeneral::findByUid($request->change_from_uid);
