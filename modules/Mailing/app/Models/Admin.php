...
class Admin extends Model
{
    ...
    public function getColorScheme()
    {
        ...
        if (! empty($this->color_scheme)) {
            ...
            return \Modules\Mailing\Models\Setting::get('backend_scheme');
        }
    }
    ...
    {
        $query = \Modules\Mailing\Models\Customer::select('customers.*');

        if (! $this->user->can('readAll', new \Modules\Mailing\Models\Customer)) {
    ...
    {
        $query = \Modules\Mailing\Models\Log::select('logs.*')->join('customers', 'customers.id', '=', 'logs.customer_id')
    ...
    {
        $customer = \Modules\Mailing\Models\Customer::newCustomer();
    ...
    {
        return \Modules\Mailing\Models\PlanGeneral::active();
    }
    ...
    {
        $query = \Modules\Mailing\Models\Admin::getAll()
            ->where('admins.status', '=', \Modules\Mailing\Models\Admin::STATUS_ACTIVE);

        if (! $this->can('readAll', new \Modules\Mailing\Models\Admin)) {
    ...
    {
        $query = \Modules\Mailing\Models\AdminGroup::select('*');

        if (! $this->can('readAll', new \Modules\Mailing\Models\AdminGroup)) {
    ...
    {
        $query = \Modules\Mailing\Models\SendingServer::active();

        if (! $this->can('readAll', new \Modules\Mailing\Models\SendingServer)) {
    ...
    {
        $query = \Modules\Mailing\Models\Campaign::getAll();

        if (! $this->can('readAll', new \Modules\Mailing\Models\Customer)) {
    ...
    {
        $query = \Modules\Mailing\Models\MailList::getAll();
    ...
    public function getSubaccountSendingServers()
    {
        ...
        $query = $query->whereIn('type', \Modules\Mailing\Models\SendingServer::getSubAccountTypes());
