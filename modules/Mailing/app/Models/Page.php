...
class Page extends Model
{
    ...
    {
        return $this->belongsTo('Modules\Mailing\Models\MailList');
    }
    ...
    public static function findPage($list, $layout)
    {
        ...
        if (! isset($page)) {
            $page = new \Modules\Mailing\Models\Page;
    ...
    public function renderContent($values = null, $subscriber = null)
    {
        ...
        if (isset($subscriber)) {
            ...
            $this->content = str_replace('{SUBSCRIBE_CONFIRM_URL}', config('app.url').action('PageController@signUpConfirmationThankyou', ['list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('subscribe-confirm')], false), $this->content);
            ...
            // UBSUBSCRIBE URL
            $this->content = str_replace('{UNSUBSCRIBE_URL}', config('app.url').action('PageController@unsubscribeForm', ['list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('unsubscribe')], false), $this->content);

            $this->content = str_replace('{UPDATE_PROFILE_URL}', config('app.url').action('PageController@profileUpdateForm', ['list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('update-profile')], false), $this->content);
    ...
    public function getTransformedSubject($subscriber)
    {
        ...
        if (isset($subscriber)) {
            // [SUBSCRIBE_CONFIRM_URL]
            $subject = str_replace('{SUBSCRIBE_CONFIRM_URL}', config('app.url').action('PageController@signUpConfirmationThankyou', ['list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('subscribe-confirm')], false), $subject);
            ...
            // UBSUBSCRIBE URL
            $subject = str_replace('{UNSUBSCRIBE_URL}', config('app.url').action('PageController@unsubscribeForm', ['list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('unsubscribe')], false), $subject);

            $subject = str_replace('{UPDATE_PROFILE_URL}', config('app.url').action('PageController@profileUpdateForm', ['list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('update-profile')], false), $subject);
    ...
    {
        return in_array($this->layout->alias, [
