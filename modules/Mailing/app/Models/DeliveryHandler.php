...
class DeliveryHandler extends Model
{
    ...
    {
        return storage_path('logs/'.php_sapi_name().'/'.$this->logfile.'-'.$this->uid.'.log');
    }
    ...
    public function start($debug = false)
    {
        ...
        try {
            ...
            $count = 1;
            if (! empty($emails)) {
                $this->logger()->info(sprintf('%s unread emails found', count($emails)));

                foreach ($emails as $message) {
                    $this->logger()->info(sprintf('Processing message %s/%s', $count, count($emails)));
    ...
    public function getAcxMessageId()
    {
        preg_match('/(?<=X-Acelle-Message-Id:)\s{0,1}<{0,1}(?<id>[a-zA-Z0-9-\.]+[a-zA-Z0-9-]+@[a-zA-Z0-9\.\-]+[a-zA-Z0-9]+)/', $message, $matched);
    ...
    {
        preg_match('/(?<=X-Client-Message-Id:)\s{0,1}<{0,1}(?<id>[a-zA-Z0-9-\.]+[a-zA-Z0-9-]+@[a-zA-Z0-9\.\-]+[a-zA-Z0-9]+)/', $message, $matched);
