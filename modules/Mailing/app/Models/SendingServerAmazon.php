...
use Modules\Mailing\Library\Log as MailLog;
use Modules\Mailing\Library\SendingServer\DomainVerificationInterface;
...
class SendingServerAmazon extends SendingServer implements DomainVerificationInterface
{
    ...
    public $notificationTypes = ['Bounce', 'Complaint'];
    ...
    {
        if (! $this->snsClient) {
            $this->snsClient = \Aws\Sns\SnsClient::factory([
                'credentials' => [
    ...
    {
        if (! $this->sesClient) {
            $this->sesClient = \Aws\Ses\SesClient::factory([
                'credentials' => [
                    'key' => trim($this->aws_access_key_id),
                    'secret' => trim($this->aws_secret_access_key),
                ],
    ...
    {
        // See: https://aws.amazon.com/premiumsupport/knowledge-center/ses-bounce-notifications-sns/
        ...
        $topicResponse = $this->snsClient()->createTopic(['Name' => self::SNS_TOPIC]);
    ...
    public function syncIdentities()
    {
        ...
        foreach ($sendersByUsers as $sender) {
            if (array_key_exists($sender->email, $identities)) {
                ...
                $identities[$sender->email]['UserName'] = $sender->customer->displayName();
            }
        }
    ...
    private function generateCustomVerificationEmailTemplateName()
    {
        ...
        [$selected, $next] = $this->pickAvailableSlot($from, $max, $numbers);
