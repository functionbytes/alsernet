...
use Modules\Mailing\Library\Contracts\PlanInterface;
use Modules\Mailing\Library\QuotaManager;
use Validator;
...
class PlanGeneral extends Plan implements PlanInterface
{
    ...
    public function rules()
    {
        ...
        if ($this->getOption('sending_server_option') == \Modules\Mailing\Models\PlanGeneral::SENDING_SERVER_OPTION_SUBACCOUNT) {
    ...
    public static function defaultOptions()
    {
        ...
        foreach (\Modules\Mailing\Models\SendingServer::types() as $key => $type) {
    ...
    {
        if ($this->getOption('sending_server_option') != \Modules\Mailing\Models\PlanGeneral::SENDING_SERVER_OPTION_OWN) {
    ...
    public function fillPlansEmailVerificationServers($params)
    {
        ...
        foreach ($params as $key => $param) {
            if ($param['check']) {
                $server = \Modules\Mailing\Models\EmailVerificationServer::findByUid($key);
                $row = new \Modules\Mailing\Models\PlansEmailVerificationServer;
                $row->plan_id = $this->id;
    ...
    public function updateFitnesses($hash)
    {
        foreach ($hash as $uid => $value) {
            ...
            \Modules\Mailing\Models\PlansSendingServer::where('sending_server_id', '=', $sendingServer->id)
    ...
    public function getEmailVerificationServers()
    {
        if ($this->getOption('all_email_verification_servers') == 'yes') {
            $result = \Modules\Mailing\Models\EmailVerificationServer::getAllAdminActive()->get()->map(function ($server) {
                return $server;
            });
        } else {
    ...
    {
        return $this->getOption('sending_server_option') == \Modules\Mailing\Models\PlanGeneral::SENDING_SERVER_OPTION_SYSTEM;
    }

    public function useOwnSendingServer()
    {
        return $this->getOption('sending_server_option') == \Modules\Mailing\Models\PlanGeneral::SENDING_SERVER_OPTION_OWN;
    }
    ...
    public function updateTos($params)
    {
        ...
        //
        \Modules\Mailing\Models\Setting::set('terms_of_service.enabled', $params['terms_of_service']['enabled']);
        \Modules\Mailing\Models\Setting::set('terms_of_service.content', $params['terms_of_service']['content']);
