...
use Modules\Mailing\Library\CreditTracker;
use Modules\Mailing\Library\Facades\SubscriptionFacade;
...
class Subscription extends Model
{
    ...
    public function plan()
    {
        ...
        return $this->belongsTo('\Modules\Mailing\Models\Plan');
    }

    public function planGeneral()
    {
        // @todo dependency injection
        return $this->belongsTo('\Modules\Mailing\Models\PlanGeneral', 'plan_id')
            ->where('type', \Modules\Mailing\Models\PlanGeneral::TYPE_GENERAL);
    }
    ...
    {
        // @todo dependency injection
        return $this->belongsTo('\Modules\Mailing\Models\Customer');
    }
    ...
    public function createNewSubscriptionInvoiceWithoutTrial()
    {
        ...
        // create init invoice
        $invoice = \Modules\Mailing\Models\InvoiceNewSubscription::createInvoice(
            $type = \Modules\Mailing\Models\InvoiceNewSubscription::TYPE_NEW_SUBSCRIPTION,
    ...
    public function createRenewInvoice()
    {
        ...
        // create init invoice
        $invoice = \Modules\Mailing\Models\InvoiceRenewSubscription::createInvoice(
            $type = \Modules\Mailing\Models\InvoiceRenewSubscription::TYPE_RENEW_SUBSCRIPTION,
    ...
    public function createChangePlanInvoice($newPlan)
    {
        ...
        // create init invoice
        $invoice = \Modules\Mailing\Models\InvoiceChangePlan::createInvoice(
            $type = \Modules\Mailing\Models\InvoiceChangePlan::TYPE_CHANGE_PLAN,
    ...
    {
        return $this->current_period_ends_at->copy()->subDays(\Modules\Mailing\Models\Setting::get('subscription.auto_billing_period'));
    }
    ...
    {
        // @todo dependency injection
        return $this->hasMany('\Modules\Mailing\Models\SubscriptionLog');
    }
    ...
    {
        return \Modules\Mailing\Models\Transaction::whereIn('invoice_id', $this->invoices()->select('id'))
            ->orderBy('created_at', 'desc')
            ->orderBy('id', 'desc');
    }
    ...
    public function endIfExpired()
    {
        if ($this->isActive() &&
            ...
            \Modules\Mailing\Models\Setting::get('allowed_due_subscription') == 'no'
        ) {
            $this->end();
        }
    }
