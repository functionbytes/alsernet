...
use Modules\Mailing\Events\MailListSubscription;
use Modules\Mailing\Events\MailListUpdated;
use Modules\Mailing\Jobs\ExportSubscribersJob;
use Modules\Mailing\Jobs\ImportSubscribers2;
use Modules\Mailing\Jobs\ImportSubscribersJob;
use Modules\Mailing\Jobs\VerifyMailListJob;
use Modules\Mailing\Library\ExtendedSwiftMessage;
use Modules\Mailing\Library\MailListFieldMapping;
use Modules\Mailing\Library\RouletteWheel;
...
use Modules\Mailing\Library\Traits\QueryHelper;
use Modules\Mailing\Library\Traits\TrackJobs;
use App;
use DB;
use Exception;
...
class MailList extends Model
{
    ...
    public function sendSubscriptionConfirmationEmail($subscriber)
    {
        ...
        $layout = \Modules\Mailing\Models\Layout::where('alias', 'sign_up_confirmation_email')->first();
        $send_page = \Modules\Mailing\Models\Page::findPage($this, $layout);
    ...
    public function sendSubscriptionWelcomeEmail($subscriber)
    {
        ...
        $layout = \Modules\Mailing\Models\Layout::where('alias', 'sign_up_welcome_email')->first();
        $send_page = \Modules\Mailing\Models\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function sendUnsubscriptionNotificationEmail($subscriber)
    {
        ...
        $layout = \Modules\Mailing\Models\Layout::where('alias', 'unsubscribe_goodbye_email')->first();
        $send_page = \Modules\Mailing\Models\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function sendUnsubscriptionNotificationEmailToListOwner($subscriber)
    {
        ...
        $template = \Modules\Mailing\Models\Layout::where('alias', 'unsubscribe_notification_for_list_owner')->first();
    ...
    public function sendProfileUpdateEmail($subscriber)
    {
        ...
        $layout = \Modules\Mailing\Models\Layout::where('alias', 'profile_update_email')->first();
        $send_page = \Modules\Mailing\Models\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function subscribe($request, $source)
    {
        ...
        //     if ($subscriber && $subscriber->status == \Modules\Mailing\Models\Subscriber::STATUS_SUBSCRIBED) {
    ...
    public function updateOrCreateFieldsFromRequest($request)
    {
        ...
        foreach ($request->fields as $key => $item) {
            ...
            if ($this->getEmailField()->uid != $item['uid']) {
                ...
                // Check tag exsit
                $tag = \Modules\Mailing\Models\Field::formatTag($item['tag']);
        ...
        foreach ($request->fields as $uid => $item) {
            $field = \Modules\Mailing\Models\Field::findByUid($item['uid']);
            if (! $field) {
                $field = new \Modules\Mailing\Models\Field;
            ...
            if ($this->getEmailField()->uid != $field->uid) {
                // save exsit field
                $item['tag'] = \Modules\Mailing\Models\Field::formatTag($item['tag']);
                $field->fill($item);
                $field->save();
                ...
                if (isset($item['options'])) {
                    foreach ($item['options'] as $key2 => $item2) {
                        $option = new \Modules\Mailing\Models\FieldOption($item2);
