...
use Modules\Mailing\Library\Tool;
use Modules\Mailing\Library\Traits\HasUid;
use Modules\Mailing\Notifications\ResetPassword;
use App;
use AppUrl;
...
class User extends Authenticatable
{
    ...
    {
        return $this->hasOne('Modules\Mailing\Models\Admin');
    }
    ...
    {
        $layout = \Modules\Mailing\Models\Layout::where('alias', 'registration_confirmation_email')->first();
        $token = $this->getToken();
    ...
    {
        $token = \Modules\Mailing\Models\UserActivation::getToken();

        $userActivation = $this->userActivation;

        if (! $userActivation) {
            $userActivation = new \Modules\Mailing\Models\UserActivation;
