...
use Modules\Mailing\Exceptions\MailingException;
...
class MailingService
{
    ...
    public function __construct(?Client $client = null)
    {
        ...
        $this->apiKey = config('mailing.api_key');
        $this->apiUrl = config('mailing.api_url');
        $this->timeout = config('mailing.timeout', 30);
        $this->connectTimeout = config('mailing.connect_timeout', 10);
        $this->retryConfig = config('mailing.retry', [
            'max_attempts' => 3,
            'delay' => 100,
            'multiplier' => 2,
        ]);
        ...
        while ($attempt < $this->retryConfig['max_attempts']) {
            try {
                ...
                if ($statusCode >= 200 && $statusCode < 300) {
                    if (config('mailing.logging.enabled')) {
                        Log::channel(config('mailing.logging.channel', 'stack'))
                ...
                if ($attempt >= $this->retryConfig['max_attempts']) {
                    ...
                    throw MailingException::connectionFailed($message);
                }
                ...
                throw MailingException::connectionFailed($e->getMessage());
            }
        }

        throw MailingException::connectionFailed('Max retry attempts reached');
    }
    ...
    public function syncSubscriber(Subscriber $subscriber, array $groups = []): array
    {
        try {
            ...
            throw MailingException::syncFailed($subscriber->email, $e->getMessage());
        }
    }
    ...
    {
        $cacheKey = config('mailing.cache.prefix').':groups:'.md5(json_encode($filters));
        $cacheTtl = config('mailing.cache.ttl.groups', 3600);

        if (config('mailing.cache.enabled')) {
    ...
    public function syncGroups(): int
    {
        try {
            ...
            if (config('mailing.cache.enabled') && config('mailing.cache.auto_invalidate')) {
                Cache::forget(config('mailing.cache.prefix').':groups:'.md5(json_encode([])));
            }
    ...
    public function createCampaign(Campaign $campaign, array $options = []): array
    {
        try {
            $data = [
                ...
                'sender_id' => $campaign->sender_id ?? config('mailing.campaign.default_sender_id'),
            ];
            ...
            throw MailingException::campaignFailed($e->getMessage());
        }
    }
    ...
    public function sendCampaign(Campaign $campaign, array $options = []): array
    {
        if (! $campaign->mailrelay_campaign_id) {
            throw MailingException::campaignFailed('Campaign must be created in Mailrelay first');
        }
        ...
        try {
            ...
            if (config('mailing.campaign.track_analytics')) {
    ...
    public function getCampaignAnalytics(Campaign $campaign): array
    {
        ...
        $cacheKey = config('mailing.cache.prefix').":analytics:{$campaign->mailrelay_campaign_id}";
        $cacheTtl = config('mailing.cache.ttl.analytics', 300);
    ...
    {
        if (! $subscriber->mailrelay_id) {
            throw MailingException::subscriberNotFound(0);
        }
    ...
    {
        if (empty($events)) {
            $events = config('mailing.webhook.events', [
                'email.opened',
                'email.clicked',
        ...
        try {
            ...
            if ($secret = config('mailing.webhook.secret')) {
                $data['secret'] = $secret;
            }
            ...
            throw MailingException::connectionFailed('Failed to create webhook: '.$e->getMessage());
        }
    }
