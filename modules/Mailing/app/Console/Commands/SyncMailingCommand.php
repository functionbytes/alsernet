...
use Modules\Mailing\Jobs\SyncSubscriberJob;
 ...
 */
class SyncMailingCommand extends Command
{
    /**
     ...
     */
    protected MailingService $mailrelayService;
     ...
     * @return void
     */
    public function __construct(MailingService $mailrelayService)
    {
        parent::__construct();
    ...
    protected function syncGroup(array $groupData, bool $isDryRun): void
    {
        ...
        $group = MailingGroup::where('mailrelay_group_id', $mailrelayGroupId)->first();
    ...
    protected function dispatchSubscriberSyncJobs(bool $isForce, bool $isDryRun): bool
    {
        ...
        try {
            ...
            $useQueue = config('mailing.sync.use_queue', true);
            $queueName = config('mailing.sync.queue_name', 'mailing');
