            ...
            <div class="col-lg-8">
                    ...
                    <div class="card-body">
                        <div class="mb-0">
                            ...
                            <input type="text" name="name" id="option-name"
                                ...
                                value="{{ old('name', $globalOption->name) }}" required>
                            @error('name')
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div id="option-values-placeholder" class="text-center py-4 text-muted" style="display:none">
                            <i class="fas fa-list fa-2x opacity-50 mb-2 d-block"></i>
                            <small>¡Elija el tipo de opcion!</small>
                        </div>

                        {{-- Tabla de valores --}}
                        <div id="option-values-section">
                            <div class="table-responsive">
                                <table class="table table-sm align-middle mb-0" id="values-table">
                                    ...
                                    <tbody id="values-tbody">
                                        @if(!old('values'))
                                            @foreach($globalOption->values as $i => $value)
                                                <tr class="value-row">
                                                    <td><i class="fas fa-grip-vertical text-muted" style="cursor:grab"></i></td>
                                                    <td><input type="text" name="values[{{ $i }}][option_value]" class="form-control form-control-sm" placeholder="Etiqueta" value="{{ $value->option_value }}" required></td>
                                                    <td><input type="number" name="values[{{ $i }}][affect_price]" class="form-control form-control-sm" placeholder="0.00" step="0.01" min="0" value="{{ $value->affect_price }}"></td>
                                                    <td>
                                                        <select name="values[{{ $i }}][affect_type]" class="form-select form-select-sm">
                                                            <option value="fixed" {{ $value->affect_type === 'fixed' ? 'selected' : '' }}>Fijado</option>
                                                            <option value="percentage" {{ $value->affect_type === 'percentage' ? 'selected' : '' }}>Porcentaje</option>
                                                        </select>
                                                    </td>
                        ...
                        <div class="card-body">
                            <select name="option_type" id="option_type"
                                ...
                                <optgroup label="Texto">
                                    <option value="text" {{ old('option_type', $globalOption->option_type) === 'text' ? 'selected' : '' }}>Field</option>
                                </optgroup>
                                <optgroup label="Seleccion">
                                    <option value="dropdown" {{ old('option_type', $globalOption->option_type) === 'dropdown' ? 'selected' : '' }}>Dropdown</option>
                                    <option value="checkbox" {{ old('option_type', $globalOption->option_type) === 'checkbox' ? 'selected' : '' }}>Checkbox</option>
                                    <option value="radio"    {{ old('option_type', $globalOption->option_type) === 'radio'    ? 'selected' : '' }}>RadioButton</option>
                                </optgroup>
                            </select>
                            @error('option_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="card-body">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="required" value="1"
                                    id="option-required" {{ old('required', $globalOption->required) ? 'checked' : '' }}>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <input type="number" name="order" min="0"
                                ...
                                value="{{ old('order', $globalOption->order) }}">
                            @error('order')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="card-body p-0">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item px-3 py-2 d-flex justify-content-between small">
                                    ...
                                    <span>{{ $globalOption->created_at->translatedFormat('j M, Y') }}</span>
                                </li>
                                <li class="list-group-item px-3 py-2 d-flex justify-content-between small">
                                    <span class="text-muted">Actualizado</span>
                                    <span>{{ $globalOption->updated_at->translatedFormat('j M, Y') }}</span>
                                </li>
                            </ul>
                        </div>
                    </div>
...
<script>
(function () {
    var valueIndex = {{ $globalOption->values->count() }};
    ...
    @if(old('values'))
        document.getElementById('values-tbody').innerHTML = '';
