                            ...
                            <tbody>
                                @foreach($tags as $tag)
                                    <tr>
                                        ...
                                        <td><span class="badge bg-{{ $tag->status === 'published' ? 'success' : 'secondary' }}">{{ $tag->status }}</span></td>
                                        <td class="text-center">
                                            <a href="{{ route('ecommerce.tags.edit', $tag) }}" class="btn btn-sm btn-outline-primary"><i class="fas fa-edit"></i></a>
                                            <form action="{{ route('ecommerce.tags.destroy', $tag) }}" method="POST" class="d-inline" onsubmit="return confirm('Eliminar esta etiqueta?')">
                                                @csrf @method('DELETE')
