                            ...
                            <tbody>
                                @foreach($collections as $collection)
                                    <tr>
                                        ...
                                        <td><span class="badge bg-{{ $collection->status === 'published' ? 'success' : 'secondary' }}">{{ $collection->status }}</span></td>
                                        <td class="text-center">
                                            <a href="{{ route('ecommerce.collections.edit', $collection) }}" class="btn btn-sm btn-outline-primary"><i class="fas fa-edit"></i></a>
                                            <form action="{{ route('ecommerce.collections.destroy', $collection) }}" method="POST" class="d-inline" onsubmit="return confirm('Eliminar esta coleccion?')">
                                                @csrf @method('DELETE')
