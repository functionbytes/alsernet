                            ...
                            <tbody>
                                @foreach($taxes as $tax)
                                    <tr>
                                        ...
                                        <td><span class="badge bg-{{ $tax->status === 'published' ? 'success' : 'secondary' }}">{{ $tax->status }}</span></td>
                                        <td class="text-center">
                                            <a href="{{ route('ecommerce.taxes.edit', $tax) }}" class="btn btn-sm btn-outline-primary"><i class="fas fa-edit"></i></a>
                                            <form action="{{ route('ecommerce.taxes.destroy', $tax) }}" method="POST" class="d-inline" onsubmit="return confirm('Eliminar este impuesto?')">
                                                @csrf @method('DELETE')
