...
class EmailSuppressionControllerTest extends TestCase
{
    ...
    {
        $this->get(route('settings.helpdeskemaillog.suppressions.index'))->assertRedirect();
    }
    ...
    public function test_index_requires_settings_view_permission(): void
    {
        ...
        $this->actingAs($outsider)
            ->get(route('settings.helpdeskemaillog.suppressions.index'))
            ->assertForbidden();
    }
    ...
    {
        $this->actingAs($this->manager)
            ->post(route('settings.helpdeskemaillog.suppressions.store'), [
                'email' => 'Manual@Example.Test',
                'module' => '',
                'reason' => 'manual',
                'notes' => 'Pidió baja por teléfono',
            ])
            ->assertRedirect(route('settings.helpdeskemaillog.suppressions.index'));
    ...
    public function test_manager_can_remove_a_suppression(): void
    {
        ...
        $this->actingAs($this->manager)
            ->delete(route('settings.helpdeskemaillog.suppressions.destroy', $suppression))
            ->assertRedirect(route('settings.helpdeskemaillog.suppressions.index'));
