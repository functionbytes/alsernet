...
class BounceMailboxesControllerTest extends TestCase
{
    ...
    {
        $this->get(route('settings.helpdeskemaillog.bounce-mailboxes.index'))->assertRedirect();
    }
    ...
    public function test_index_requires_settings_view_permission(): void
    {
        ...
        $this->actingAs($outsider)
            ->get(route('settings.helpdeskemaillog.bounce-mailboxes.index'))
            ->assertForbidden();
    }
    ...
    {
        $this->actingAs($this->manager)
            ->post(route('settings.helpdeskemaillog.bounce-mailboxes.store'), [
            ...
            ->assertRedirect(route('settings.helpdeskemaillog.bounce-mailboxes.index'));
    ...
    public function test_manager_can_update_a_mailbox_and_blank_password_keeps_the_existing_one(): void
    {
        ...
        $this->actingAs($this->manager)
            ->put(route('settings.helpdeskemaillog.bounce-mailboxes.update', $mailbox['id']), [
                'label' => 'Actualizado',
    ...
    {
        $this->actingAs($this->manager)
            ->put(route('settings.helpdeskemaillog.bounce-mailboxes.update', 'no-existe'), [
                'label' => 'x', 'host' => 'x', 'username' => 'x',
            ])
            ->assertNotFound();
    }
    ...
    public function test_manager_can_delete_a_mailbox(): void
    {
        ...
        $this->actingAs($this->manager)
            ->delete(route('settings.helpdeskemaillog.bounce-mailboxes.destroy', $mailbox['id']))
            ->assertRedirect(route('settings.helpdeskemaillog.bounce-mailboxes.index'));
