...
class UpdateEmailLogSettingsRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'bounce_rate_critical_pct' => ['required', 'numeric', 'min:0', 'max:100'],
            'complaint_rate_warning_pct' => ['required', 'numeric', 'min:0', 'max:100', 'lte:complaint_rate_critical_pct'],
            'complaint_rate_critical_pct' => ['required', 'numeric', 'min:0', 'max:100'],
            'provider_webhook_provider' => ['nullable', 'in:mailrelay,ses,postmark,mailgun'],
            'provider_webhook_secret' => ['nullable', 'string', 'max:500'],
            'provider_webhook_process_bounces' => ['required', 'in:0,1'],
            'provider_webhook_process_complaints' => ['required', 'in:0,1'],
            'provider_webhook_process_deliveries' => ['required', 'in:0,1'],
            'provider_webhook_process_opens' => ['required', 'in:0,1'],
        ];
    }
