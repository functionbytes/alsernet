...
class CheckEmailReputationCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $bounceCritical = (float) Setting::get('helpdeskemaillog.bounce_rate_critical_pct', config('helpdeskemaillog.bounce_rate_critical_pct'));
        $complaintCritical = (float) Setting::get('helpdeskemaillog.complaint_rate_critical_pct', config('helpdeskemaillog.complaint_rate_critical_pct'));
    ...
    {
        $wasBreached = Setting::get("helpdeskemaillog.reputation_breached_{$key}", '0') === '1';
        $isBreached = $rate >= $threshold;

        Setting::set("helpdeskemaillog.reputation_breached_{$key}", $isBreached ? '1' : '0');
