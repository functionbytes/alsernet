...
class EmailLogAnalyticsService
{
     ...
     * EmailLogController::computeStats()) — reproducida aquí porque esta
     * consulta necesita el flag dentro de un SUM condicional agrupado por
     * mailable_class, no como WHERE de una query completa.
     */
    private const OPEN_TRACKING_JSON_PATH = 'json_extract(email_logs.metadata, \'$."open_tracking_enabled"\') = true';
    ...
    private function computeMailablePerformance(Carbon $from, Carbon $to): Collection
    {
        ...
        $rows = EmailLog::query()
            ...
            ->selectRaw('SUM('.self::OPEN_TRACKING_JSON_PATH.') as open_tracked')
            ->selectRaw('SUM('.self::OPEN_TRACKING_JSON_PATH.' AND eo.email_log_id IS NOT NULL) as opened')
    ...
    {
        return "helpdeskemaillog:analytics:{$suffix}:{$from->toDateString()}:{$to->toDateString()}";
    }
}
