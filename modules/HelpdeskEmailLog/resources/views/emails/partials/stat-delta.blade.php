...
@if($diff !== null)
    @php
        ...
        $label = match (true) {
            ...
            $isRate => $sign.number_format(abs($diff), 1).' '.__('helpdeskemaillog::emaillog.stats.delta_points_suffix'),
            default => $sign.number_format(abs($diff)),
        };
    @endphp
    <span class="evx-stat-delta {{ $polarityClass }}" title="{{ __('helpdeskemaillog::emaillog.stats.delta_vs_previous') }}">
