            ...
            <div class="col-lg-8 mx-auto">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">
                            <i class="fas fa-file-code me-2"></i>Editar Plantilla: {{ $template->name }}
                        </h5>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.social.templates.update', $template) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <small class="text-muted">
                                    ...
                                    Usa variables entre llaves como <code>{nombre}</code>, <code>{precio}</code>
                                </small>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text"
                                       ...
                                       id="hashtags"
                                       name="hashtags"
                                       value="{{ old('hashtags', is_array($template->hashtags) ? implode(' ', $template->hashtags) : '') }}">
                                @error('hashtags')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-check">
                                                <input class="form-check-input"
                                                       ...
                                                       value="{{ $network }}"
                                                       id="network_{{ $network }}"
                                                       {{ (is_array(old('networks')) && in_array($network, old('networks'))) ||
                                                          (is_array($template->networks) && in_array($network, $template->networks)) ? 'checked' : '' }}>
                            ...
                            <div class="alert alert-light border" role="alert">
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <small class="text-muted">
                                            Usada <strong>{{ $template->usage_count }}</strong> veces
                                            @if($template->last_used_at)
                                                - Última vez: {{ $template->last_used_at->diffForHumans() }}
                                            @endif
                                        </small>
                                    </div>
                                </div>
