...
class SyncCategoriesFromErpJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function handle(ErpCategorySyncService $syncService, SyncStatusService $statusService): void
    {
        $this->batch->refresh();
        if ($this->batch->isTerminal()) {
            Log::warning('SyncCategoriesFromErpJob skipped — batch already in terminal state', [
        ...
        try {
            Log::info('SyncCategoriesFromErpJob started', ['batch_id' => $this->batch->id]);

            $status = $statusService->startSync($this->batch, 0, $this->batch->triggered_by ?? 'manual', $this->batch->filter_criteria);

            $result = $syncService->syncAllHierarchy($this->batch);

            if ($result['success']) {
                $statusService->completeSync($status, [
                    'sports_synced' => $result['sports']['synced'] ?? 0,
                    'categories_synced' => $result['categories']['synced'] ?? 0,
                ]);
                Log::info('SyncCategoriesFromErpJob completed', [
                    'batch_id' => $this->batch->id,
                    'stats' => $result,
                ]);
            } else {
                $statusService->failSync($status, implode(', ', $result['errors'] ?? ['Unknown error']));
                Log::error('SyncCategoriesFromErpJob failed', [
                    'batch_id' => $this->batch->id,
            ...
            Log::error('SyncCategoriesFromErpJob exception', [
                'batch_id' => $this->batch->id,
                'error' => $e->getMessage(),
            ]);
    ...
    public function failed(Exception $exception): void
    {
        ...
        Log::error('SyncCategoriesFromErpJob permanently failed', [
            'batch_id' => $this->batch->id,
