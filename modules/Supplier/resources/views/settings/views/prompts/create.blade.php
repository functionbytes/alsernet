            ...
            <div class="col-12 col-lg-8">
                <div class="card">
                    ...
                    <div class="card-header d-flex align-items-center gap-2">
                        ...
                        <span class="pb-badge"><i class="fas fa-code-branch"></i> v1.0</span>
                    </div>
                                ...
                                <div class="card pb-editor-card">
                                    ...
                                    <div class="card-body p-0">
                                        <textarea class="form-control pb-editor @error('prompt_template') is-invalid @enderror"
                                                  ...
                                                  rows="16" required
                                                  placeholder="Escribe las instrucciones para la IA...">{{ old('prompt_template') }}</textarea>
                                    </div>
                    ...
                    <div class="card-body border-bottom">
                        <div class="mb-3">
                            ...
                            <select class="form-control select2 @error('scope') is-invalid @enderror"
                                ...
                                @php $selectedScope = old('scope', $defaults['scope'] ?? 'global'); @endphp
                        ...
                        <div class="mb-3" id="supplierContainer" style="display: none;">
                            ...
                            <select class="form-control select2" name="supplier_id" id="supplierSelect">
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" {{ old('supplier_id', $defaults['supplier_id'] ?? '') == $supplier->id ? 'selected' : '' }}>
                                        {{ $supplier->label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3" id="categoryContainer" style="display: none;">
                            @php
                                $selectedSubIds = old('subfamily_ids',
                                    isset($defaults['subfamily_id']) && $defaults['subfamily_id']
                                        ? [$defaults['subfamily_id']] : []
                                );
                                $isMultiMode = count((array)$selectedSubIds) > 1
                                    || old('subfamily_mode', 'single') === 'multi';
                            @endphp
                            ...
                            <div id="mode-single-fields" style="{{ $isMultiMode ? 'display:none' : '' }}">
                                ...
                                <select class="form-control select2" id="erpCatFilterSelect">
                                    ...
                                    @foreach($erpcategorias as $ec)
                                        <option value="{{ $ec->erp_categoria_id }}" data-sport-id="{{ $ec->sport_id }}">
                                ...
                                <select class="form-control select2" id="familyFilterSelect">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" data-sport-id="{{ $category->sport_id }}" data-erp-cat-id="{{ $category->erp_categoria_id }}">
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-0">
                            ...
                            <select class="form-control select2 @error('content_type') is-invalid @enderror" name="content_type" required>
                                ...
                                <option value="description" {{ old('content_type', 'description') === 'description' ? 'selected' : '' }}>Descripción completa</option>
                                <option value="short_description" {{ old('content_type') === 'short_description' ? 'selected' : '' }}>Descripción corta</option>
                                <option value="title" {{ old('content_type') === 'title' ? 'selected' : '' }}>Título del producto</option>
                                <option value="seo_title" {{ old('content_type') === 'seo_title' ? 'selected' : '' }}>Título SEO</option>
                                <option value="seo_description" {{ old('content_type') === 'seo_description' ? 'selected' : '' }}>Meta descripción SEO</option>
                                <option value="seo_keywords" {{ old('content_type') === 'seo_keywords' ? 'selected' : '' }}>Palabras clave SEO</option>
                                <option value="features" {{ old('content_type') === 'features' ? 'selected' : '' }}>Características destacadas</option>
                                <option value="specifications" {{ old('content_type') === 'specifications' ? 'selected' : '' }}>Especificaciones técnicas</option>
                                <option value="benefits" {{ old('content_type') === 'benefits' ? 'selected' : '' }}>Beneficios del producto</option>
                                <option value="metadata" {{ old('content_type') === 'metadata' ? 'selected' : '' }}>Metadata personalizada</option>
                            </select>
                        ...
                        <div class="pb-switch-card">
                            ...
                            <div class="form-check form-switch mb-0">
                                <input class="form-check-input" type="checkbox" role="switch"
                                       ...
                                       @if(old('seo_focus', '0') === '1') checked @endif>
                            </div>
                        </div>
                        ...
                        <div class="pb-switch-card align-items-start flex-column gap-2 pb-3">
                            ...
                            <select class="form-select select2 w-100" name="ai_model" id="ai_model">
                                ...
                                <optgroup label="OpenAI">
                                    <option value="gpt-4o-mini" @selected(old('ai_model') === 'gpt-4o-mini')>GPT-4o Mini — rápido y económico</option>
                                    <option value="gpt-4o" @selected(old('ai_model') === 'gpt-4o')>GPT-4o — calidad superior</option>
                                </optgroup>
                                <optgroup label="Anthropic">
                                    <option value="claude-3-5-haiku" @selected(old('ai_model') === 'claude-3-5-haiku')>Claude 3.5 Haiku</option>
                                    <option value="claude-3-5-sonnet" @selected(old('ai_model') === 'claude-3-5-sonnet')>Claude 3.5 Sonnet</option>
                                </optgroup>
                                <optgroup label="Google Gemini (búsqueda web incluida)">
                                    <option value="gemini-2.5-flash" @selected(in_array(old('ai_model'), ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-2.0-flash-lite']))>Gemini 2.5 Flash — recomendado</option>
                                    <option value="gemini-2.5-pro" @selected(old('ai_model') === 'gemini-2.5-pro')>Gemini 2.5 Pro — máxima calidad</option>
                                </optgroup>
                            </select>
                        </div>
...
$(document).ready(function () {
    ...
    // ── Cascade deporte → categoría → familia → subfamilia ──────────────────
    @php
    ...
    const initialSubfamilyIds = @json(old('subfamily_ids',
        isset($defaults['subfamily_id']) && $defaults['subfamily_id'] ? [$defaults['subfamily_id']] : []
    ));
    ...
    function s2HighlightResult(data) {
        ...
        const highlighted = safe.replace(new RegExp('(' + escaped + ')', 'gi'), '<span style="text-decoration:underline;font-weight:700;color:inherit;">$1</span>');
        return $('<span>').html(highlighted);
    }

    const subfamiliaS2Opts = { placeholder: 'Buscar subfamilia(s)...', allowClear: false, templateResult: s2HighlightResult };
    const subfamiliaS2OptsSingle = { placeholder: 'Seleccionar subfamilia...', allowClear: true, templateResult: s2HighlightResult };
    ...
    $('#erpCatFilterSelect').on('change', function () {
        if (_preloading) return;
        rebuildFamilySelect($('#sportFilterSelect').val(), $(this).val(), null);
        rebuildSubfamilySingle(null, null);
    });
    ...
    // Pre-seleccionar si viene con old() o defaults
    ...
    $promptForm.on('submit', function (e) {
        ...
        if (!ids.length) { $promptForm[0].submit(); return; }

        $.post(conflictCheckUrl, {
            _token: $('meta[name="csrf-token"]').attr('content'),
            subfamily_ids: ids,
        }, function (res) {
            if (!res.conflicts || !res.conflicts.length) { $promptForm[0].submit(); return; }
