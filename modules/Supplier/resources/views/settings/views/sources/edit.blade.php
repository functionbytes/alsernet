...
(function () {
    ...
    // ============================================================
    // Pre-fill URLs de extracción desde la fuente existente
    // ============================================================
    const existingExtractionUrls = @json(collect($connectionConfig['urls'] ?? [])->map(fn ($u) => ['url' => $u['url'] ?? ''])->values());

    if (existingExtractionUrls && existingExtractionUrls.length) {
        existingExtractionUrls.forEach(function (item, i) {
            const $row = $(buildExtractionUrlRow(i));
            $row.find('input[type="url"]').val(item.url || '');
            $('#extractionUrlsContainer').append($row);
        });
        extractionUrlIndex = existingExtractionUrls.length - 1;
        updateExtractionRemoveButtons();
    } else {
