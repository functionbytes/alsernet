...
class CustomerContextTest extends TestCase
{
    ...
    public function test_authenticated_user_with_permission_can_get_context(): void
    {
        ...
        $response->assertJsonPath('customer.orders_count', 5);
    }
    ...
    public function test_start_return_with_valid_items_succeeds(): void
    {
        ...
        $response->assertOk();
        $response->assertJsonPath('success', true);
        $response->assertJsonPath('data.returnId', 42);
    }
    ...
    public function test_start_return_forwards_idempotency_header_to_prestashop(): void
    {
        $this->fakeStartReturnApiResponse();
        $user = $this->userWithPermission('helpdeskprestashop.orders.return');

        $clientKey = 'agent-order-999-attempt-1';
        ...
        $forwarded = $sentRequest->header('X-Alsernet-Idempotency-Key')[0] ?? null;
        $this->assertSame($clientKey, $forwarded);
    }

    public function test_start_return_generates_deterministic_idempotency_key_without_header(): void
    {
        Http::fake([
            $this->apiUrl => Http::response([
                'ok' => true,
                'data' => ['return_id' => 1, 'status' => 'created'],
            ], 200),
        ]);
        ...
        $this->actingAs($user)
            ->postJson('/api/helpdeskprestashop/orders/999/start-return', $payload)
            ->assertOk();

        $firstKey = Http::recorded()[0][0]->header('X-Alsernet-Idempotency-Key')[0] ?? null;
        $this->assertNotEmpty($firstKey);

        // Repeating the request with the same payload from the same user must
        // produce the same deterministic key, so PS dedupes the action.
        Http::fake([
            $this->apiUrl => Http::response([
                'ok' => true,
                'data' => ['return_id' => 1, 'status' => 'created'],
        ...
        $secondKey = Http::recorded()[0][0]->header('X-Alsernet-Idempotency-Key')[0] ?? null;
        $this->assertSame($firstKey, $secondKey, 'Deterministic idempotency key must be stable across retries.');
    }
}
