...
Las acciones de escritura (actualmente sólo `order.start_return`; `customer.add_message` y `order.add_note` están reservadas para futuras versiones) envían además:
- Header `X-Alsernet-Idempotency-Key: {key}` — la key la genera el controller a partir del header `Idempotency-Key` enviado por el cliente, o de forma determinística con `sha1(user_id:order_id:items_json)` si no viene en la request.

Esto asegura que si la petición se reintenta (timeout, retry, doble-click del agente), PS no procesa dos veces la misma acción. PrestaShop cachea el resultado por idempotency key durante 1 hora.
...
**Validación** (delegada al middleware `VerifyAlsernetHmac` que usa el helper `Support\HmacSigner`):
1. Verifica que `|time() - timestamp| ≤ 300 segundos` (ventana de ±5 minutos)
2. Recalcula el HMAC con `"$timestamp:$rawBody"` y lo compara con `hash_equals` (timing-safe)
3. Si la firma no coincide, rechaza con `401 Unauthorized`

### Idempotencia "two-phase"

El controller usa caché de dos fases para evitar tanto duplicación como reintentos perdidos:
- Al recibir el webhook se intenta `Cache::add(key, 'processing', 60s)`
- Si la entrada ya existe con valor `'done'` → respuesta 200 `deduplicated:true`
- Si otra request ya cogió el lock → respuesta 200 `deduplicated:true`
- Tras despachar el evento con éxito → `Cache::put(key, 'done', 600s)`
- Si la lógica lanza excepción → `Cache::forget(key)` para que el siguiente reintento del lado PS sí se procese
...
- Respuestas "no encontrado" (`customer.found=false`) usan `miss_ttl` (60 s por defecto)
- Stale-while-revalidate: si la entrada está dentro de los últimos `stale_grace` segundos (30 s por defecto) la respuesta sigue siendo servida desde caché y se dispara `RefreshPsContextJob` en background, que llama a `PrestashopContextService::revalidate()` y hace **un solo `Cache::put` atómico** sólo si la API responde con éxito (no se sobrescribe la caché con respuestas fallidas)
- Errores HTTP (timeout, 5xx, conexión rechazada) **no se cachean** — devuelven `EMPTY_CONTEXT` en memoria pero la siguiente petición vuelve a intentarlo
- Para invalidar manualmente: `php artisan cache:clear`

### Circuit breaker

Si se acumulan `circuit_failure_threshold` (5 por defecto) fallos consecutivos de red, el circuit breaker abre durante `circuit_open_seconds` (30 s) y todas las llamadas a la API se cortocircuitan devolviendo `null` sin tocar la red. Las respuestas con `ok=false` (errores semánticos) **no** cuentan como fallo de circuito.
...
HELPDESK_PS_HTTP_TIMEOUT=10
HELPDESK_PS_CONNECT_TIMEOUT=2
HELPDESK_PS_CIRCUIT_THRESHOLD=5
HELPDESK_PS_CIRCUIT_OPEN_SECONDS=30
```
