...
@if (! empty($children))
    <section class="{{ $sectionClass }}"
             @if ($background === 'parallax' && $backgroundImage)
                 data-image-src="{{ htmlspecialchars($backgroundImage) }}"
             @endif>
        <div class="container">
            @if ($title)
                <h3 class="testimonial-title text-center">{{ $title }}</h3>
            @endif

            <div class="{{ $carousel ? $owlClasses : 'row cols-' . $cols }}"
                 @if ($carousel) data-owl-options="{{ $owlOptions }}" @endif>
