...
<div class="{{ $wrapperClasses }}">
    ...
    @if ($layout === 'inline-banner')
        <div class="countdown-content-left d-flex align-items-center gap-3">
            @if ($dealImage)
                <figure class="deal-image mb-0">
                    <img src="{{ $dealImage }}" alt="" width="37" height="46" loading="lazy">
                </figure>
            @endif
            <div>
                @if ($title)
                    <h2 class="banner-title mb-0 fw-bold ls-m">{{ $title }}</h2>
                @endif
                @if ($subtitle)
                    <h3 class="banner-subtitle mb-0 fw-normal text-muted">{{ $subtitle }}</h3>
                @endif
            </div>
        </div>
        ...
        <div class="countdown-content-center">
            <div id="{{ $countdownId }}"
                 ...
                 data-format="{{ $format }}"
                 data-labels="{{ $labelsJson }}"></div>
        </div>

        @if ($buttonText)
            <div class="countdown-content-right">
                <a href="{{ $buttonHref }}" class="btn btn-link btn-icon-right text-uppercase">
    ...
    <noscript>
        <p class="text-muted small">
            {{ __('shortcode::shortcode.countdown_expires') }}:
            {{ date('d/m/Y H:i', $untilTs) }}
        </p>
    </noscript>
</div>
...
(function () {
    ...
    function initCountdown(el) {
        ...
        if (isNaN(seconds) || seconds <= 0) {
            $el.html('<span class="countdown-expired">{{ __("shortcode::shortcode.countdown_expired") }}</span>');
            return;
        }
