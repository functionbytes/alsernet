...
@section('title', 'Nueva plantilla SEO')

@section('page_header')
    @include('core::components.card', ['title' => 'Nueva plantilla SEO'])
@endsection
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="model_type" class="form-select select2 @error('model_type') is-invalid @enderror">
                                ...
                                <option value="Page" {{ old('model_type') === 'Page' ? 'selected' : '' }}>Page</option>
                                <option value="BlogPost" {{ old('model_type') === 'BlogPost' ? 'selected' : '' }}>BlogPost</option>
                                <option value="{{ old('model_type') }}"
                                        {{ !in_array(old('model_type'), ['', 'Page', 'BlogPost']) ? 'selected' : '' }}>
                                    {{ !in_array(old('model_type'), ['', 'Page', 'BlogPost']) ? old('model_type') : 'Otro...' }}
                                </option>
                            </select>
                            @error('model_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description_pattern"
                                      ...
                                      maxlength="500">{{ old('description_pattern') }}</textarea>
                        ...
                        <div class="col-12 mb-4">
                            ...
                            <select name="og_type" class="form-select select2 @error('og_type') is-invalid @enderror">
                                @foreach(['website' => 'Website', 'article' => 'Article', 'product' => 'Product', 'profile' => 'Profile', 'book' => 'Book'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('og_type', 'website') === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('og_type')
                                <div class="invalid-feedback">{{ $message }}</div>
