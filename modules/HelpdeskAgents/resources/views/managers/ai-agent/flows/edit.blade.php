        ...
        <div class="tab-pane fade show active" id="pane-meta" role="tabpanel">
            <div class="row g-3">
                ...
                <div class="col-12 col-lg-8">
                    <div class="card">
                        <form action="{{ route('manager.helpdesk.ai.flows.update', $flow) }}" method="POST">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="row g-3 mb-4">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <select name="trigger_type" class="form-select @error('trigger_type') is-invalid @enderror" required>
                                            <option value="message" {{ old('trigger_type', $flow->trigger) === 'message' ? 'selected' : '' }}>
                                                Mensaje — se activa al recibir un mensaje especifico
                                            </option>
                                            <option value="intent" {{ old('trigger_type', $flow->trigger) === 'intent' ? 'selected' : '' }}>
                                                Intencion — se activa al detectar una intencion del usuario
                                            </option>
                                            <option value="keyword" {{ old('trigger_type', $flow->trigger) === 'keyword' ? 'selected' : '' }}>
                                                Palabra clave — se activa cuando se menciona una palabra clave
                                            </option>
                                            <option value="conversation_start" {{ old('trigger_type', $flow->trigger) === 'conversation_start' ? 'selected' : '' }}>
                                ...
                                <table class="table table-hover align-middle mb-0" id="edges-table">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th>Hasta</th>
                                            <th>Label</th>
                                            <th class="text-end" style="width: 60px">Accion</th>
                                        </tr>
                                    </thead>
                                    <tbody id="edges-body">
...
$(document).ready(function () {
    ...
    const STRUCTURE_URL = '{{ route('manager.helpdesk.ai.flows.structure', $flow) }}';
