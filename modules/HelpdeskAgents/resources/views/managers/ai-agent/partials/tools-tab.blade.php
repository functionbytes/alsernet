...
$(function () {
    ...
    $('#tools-container').off('click.ajaxpage').on('click.ajaxpage', '[data-ajax-pagination] a', function (e) {
        e.preventDefault();
    ...
    $(document).on('click', '.tool-edit-btn', function (e) {
        ...
        $.get('{{ route("helpdesk.ai.tools.index") }}/' + id, function (data) {
            $('#tool_id').val(data.id);
    ...
    $(document).on('click', '.tool-toggle-btn', function (e) {
        ...
        $.ajax({
            url: '{{ route("helpdesk.ai.tools.toggle", "__ID__") }}'.replace('__ID__', id),
            method: 'POST',
    ...
    $(document).on('click', '.tool-delete-btn', function (e) {
        ...
        $('#delete-form').attr('action', '#').off('submit').on('submit', function (ev) {
            ev.preventDefault();
            $.ajax({
                url: '{{ route("helpdesk.ai.tools.destroy", "__ID__") }}'.replace('__ID__', id),
                method: 'DELETE',
                headers: { 'X-CSRF-TOKEN': CSRF }
            }).done(function () {
    ...
    $('#toolForm').off('submit').on('submit', function (e) {
        ...
        const url = id
            ? '{{ route("helpdesk.ai.tools.update", "__ID__") }}'.replace('__ID__', id)
            : '{{ route("helpdesk.ai.tools.store") }}';
    ...
    function reloadToolsTab() {
        $.get('{{ route("helpdesk.ai.tools.index") }}', function (html) {
            $('#tools-container').html(html);
