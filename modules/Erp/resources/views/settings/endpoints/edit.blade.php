...
@section('title', 'Editar Endpoint ERP')

@section('page_header')
    @include('core::components.card', ['title' => 'Editar Endpoint ERP'])
@endsection
...
@section('content')
    ...
    <div class="row">
        ...
        <form method="POST" action="{{ route('settings.erp.endpoints.update', $endpoint) }}">
            ...
            <div class="card-body">
                <div class="d-flex no-block align-items-center">
                    <h5 class="mb-0">Editar endpoint: {{ $endpoint->name }}</h5>
                </div>
                <p class="card-subtitle mb-3 mt-1">
                    Modifica la configuración del endpoint de integración con servicios externos.
                </p>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                   value="{{ old('name', $endpoint->name) }}" required placeholder="Ej: API Gestión Central">
                    ...
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror"
                                      rows="2" placeholder="Descripción opcional del endpoint">{{ old('description', $endpoint->description) }}</textarea>
                    ...
                    <div class="col-12 col-md-8">
                        <div class="mb-3">
                            ...
                            <input type="url" name="url" class="form-control @error('url') is-invalid @enderror"
                                   value="{{ old('url', $endpoint->url) }}" required placeholder="https://api.example.com/endpoint">
                    ...
                    <div class="col-12 col-md-4">
                        <div class="mb-3">
                            ...
                            <select name="method" class="form-control select2 @error('method') is-invalid @enderror" required>
                                <option value="GET" {{ old('method', $endpoint->method) === 'GET' ? 'selected' : '' }}>GET</option>
                                <option value="POST" {{ old('method', $endpoint->method) === 'POST' ? 'selected' : '' }}>POST</option>
                                <option value="PUT" {{ old('method', $endpoint->method) === 'PUT' ? 'selected' : '' }}>PUT</option>
                                <option value="PATCH" {{ old('method', $endpoint->method) === 'PATCH' ? 'selected' : '' }}>PATCH</option>
                                <option value="DELETE" {{ old('method', $endpoint->method) === 'DELETE' ? 'selected' : '' }}>DELETE</option>
                            </select>
                            @error('method')
                                <span class="field-validation-error"><i class="fa fa-circle-exclamation"></i> {{ $message }}</span>
                            @enderror
                    ...
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <input type="text" name="content_type" class="form-control @error('content_type') is-invalid @enderror"
                                   value="{{ old('content_type', $endpoint->content_type) }}" placeholder="application/json">
                            ...
                            <select name="credential_id" class="form-control select2 @error('credential_id') is-invalid @enderror">
                                ...
                                @foreach($credentials as $credential)
                                    <option value="{{ $credential->id }}"
                                            {{ old('credential_id', $endpoint->credential_id) == $credential->id ? 'selected' : '' }}>
                                        {{ $credential->name }}
                    ...
                    <div class="col-12 col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" name="timeout" class="form-control @error('timeout') is-invalid @enderror"
                                   value="{{ old('timeout', $endpoint->timeout) }}" min="1" max="300" placeholder="30">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" name="retry_attempts" class="form-control @error('retry_attempts') is-invalid @enderror"
                                   value="{{ old('retry_attempts', $endpoint->retry_attempts) }}" min="0" max="10" placeholder="3">
            ...
            <div class="card-footer bg-light border-top">
                ...
                <a href="{{ route('settings.erp.endpoints.show', $endpoint) }}" class="btn btn-secondary w-100">
                    Cancelar
                </a>
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <ol class="mb-3 ps-3">
                        <li class="mb-2">Actualiza el <strong>nombre</strong> o <strong>URL</strong> del endpoint si es necesario</li>
                        <li class="mb-2">Modifica el <strong>método HTTP</strong> o <strong>Content-Type</strong> según los cambios en la API externa</li>
                        <li class="mb-2">Cambia la <strong>credencial asignada</strong> si necesitas usar otra autenticación</li>
                        <li class="mb-2">Ajusta <strong>timeout</strong> y <strong>reintentos</strong> para optimizar el rendimiento</li>
                        <li class="mb-2">Guarda los cambios y verifica la conexión desde la página de detalle</li>
                    </ol>

                    <div class="alert alert-info mb-0">
                        <div class="d-flex">
                            <div>
                                <strong>Importante:</strong> Cualquier cambio en la URL o autenticación puede afectar
                                las integraciones existentes. Verifica que el endpoint siga funcionando correctamente
                                después de guardar los cambios usando el botón "Probar conexión".
                            </div>
                        </div>
                    </div>
                </div>
