...
@section('page_header')
    @include('core::components.card', ['title' => 'Crear Credencial'])
@endsection
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title mb-0">Crear nueva credencial para: {{ $endpoint->name }}</h4>
                </div>

                <form action="{{ route('settings.erp.endpoints.credentials.store', $endpoint) }}" method="POST">
                    @csrf
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-control select2 @error('auth_type') is-invalid @enderror"
                                    ...
                                    <option value="basic" {{ old('auth_type', 'basic') === 'basic' ? 'selected' : '' }}>Basic Auth</option>
                            ...
                            <div class="col-md-12 mb-3 auth-field auth-bearer" style="display: none;">
                                ...
                                <textarea class="form-control @error('token') is-invalid @enderror"
                                          id="token" name="token" rows="3">{{ old('token') }}</textarea>
                                @error('token')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3 auth-field auth-api_key" style="display: none;">
                                ...
                                <input type="text" class="form-control @error('api_key_header') is-invalid @enderror"
                                       id="api_key_header" name="api_key_header" value="{{ old('api_key_header', 'X-API-Key') }}"
                            ...
                            <div class="col-md-12 mb-3 auth-field auth-custom" style="display: none;">
                                ...
                                <textarea class="form-control @error('custom_headers') is-invalid @enderror"
                                          id="custom_headers" name="custom_headers" rows="5">{{ old('custom_headers', '{}') }}</textarea>
