                ...
                <div class="card-body">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <div class="text-center py-4 border border-dashed rounded bg-light mb-2" id="image-placeholder"
                                 style="{{ ($isEdit && $post->image) || old('image') ? 'display:none' : '' }}">
                                <i class="fas fa-image fa-2x text-muted"></i>
                                <p class="text-muted mb-0 mt-2 small">Sin imagen destacada</p>
                            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row" id="uiBlocksGrid">
                    ...
                    @empty
                        <div class="col-12">
                            <p class="text-muted text-center py-4">No hay shortcodes configurados.</p>
                        </div>
                    @endforelse
                </div>
            </div>
...
$(document).ready(function () {
    ...
    // UI BLOCKS
    // =========================================================
    @php $uiBlocksJson = \Modules\Template\Models\Shortcode::active()->get(['key', 'name', 'shortcode_template', 'config_fields']); @endphp
    var uiBlocksData = @json($uiBlocksJson);
    ...
    function openBlockConfig(key, name) {
        ...
        if (!block || !block.config_fields || !block.config_fields.length) {
            // No config needed — insert directly
            insertBlockDirect(key);
            return;
        }
        var html = '';
    ...
    function insertBlockDirect(key) {
        var block = uiBlocksData.find(function (b) { return b.key === key; });
        var shortcode = block ? (block.shortcode_template || '[' + key + '][/' + key + ']') : '[' + key + '][/' + key + ']';
    ...
    function insertBlock(key) {
        ...
        if (block && block.config_fields) {
            block.config_fields.forEach(function (field) {
                var val = $('#' + field.id).val() || '';
                shortcode = shortcode.replace(new RegExp('\\{' + field.id + '\\}', 'g'), val);
            });
        }
