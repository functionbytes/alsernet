        ...
        <div class="card-body px-4 py-3">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('manager.chats.campaigns.index') }}">Campañas</a></li>
        ...
        <div class="card-header p-4 border-bottom">
            <div class="d-flex justify-content-between align-items-start">
                ...
                <div class="d-flex gap-2">
                    @can('update', $campaign)
                        <a href="{{ route('manager.chats.campaigns.edit', $campaign) }}" class="btn btn-primary">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            @can('update', $campaign)
                                @if($campaign->status === 'draft')
                                    <li>
                                        <form method="POST" action="{{ route('manager.chats.campaigns.publish', $campaign) }}">
                                            @csrf
                                ...
                                @if($campaign->status === 'active')
                                    <li>
                                        <form method="POST" action="{{ route('manager.chats.campaigns.pause', $campaign) }}">
                                            @csrf
                                ...
                                @if($campaign->status === 'paused')
                                    <li>
                                        <form method="POST" action="{{ route('manager.chats.campaigns.resume', $campaign) }}">
                                            @csrf
                                            <button type="submit" class="dropdown-item text-success">
                                ...
                                @if(in_array($campaign->status, ['active', 'paused']))
                                    <li>
                                        <form method="POST" action="{{ route('manager.chats.campaigns.end', $campaign) }}">
                                            @csrf
                            ...
                            @can('create', Modules\Chat\Models\Campaigns\Campaign::class)
                                <li>
                                    <form method="POST" action="{{ route('manager.chats.campaigns.duplicate', $campaign) }}">
                                        @csrf
                            ...
                            @can('delete', $campaign)
                                <li><hr class="dropdown-divider"></li>
                                <li>
                                    <form method="POST" action="{{ route('manager.chats.campaigns.destroy', $campaign) }}"
