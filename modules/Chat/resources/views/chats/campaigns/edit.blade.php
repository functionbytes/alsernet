        ...
        <div class="card-body px-4 py-3">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item">
                                <a href="{{ route('manager.chats.campaigns.index') }}">Campañas</a>
                            </li>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('manager.chats.campaigns.show', $campaign) }}" class="btn btn-light btn-sm">
            ...
            <div class="card">
                    ...
                    <ul class="nav nav-tabs card-header-tabs" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link {{ $currentTab === 'general' ? 'active' : '' }}"
                               href="{{ route('manager.chats.campaigns.edit', ['campaign' => $campaign, 'tab' => 'general']) }}">
                                <i class="fas fa-info-circle me-1"></i> General
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link {{ $currentTab === 'content' ? 'active' : '' }}"
                               href="{{ route('manager.chats.campaigns.edit', ['campaign' => $campaign, 'tab' => 'content']) }}">
                                <i class="fas fa-layer-group me-1"></i> Contenido
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link {{ $currentTab === 'appearance' ? 'active' : '' }}"
                               href="{{ route('manager.chats.campaigns.edit', ['campaign' => $campaign, 'tab' => 'appearance']) }}">
                                <i class="fas fa-palette me-1"></i> Apariencia
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link {{ $currentTab === 'conditions' ? 'active' : '' }}"
                               href="{{ route('manager.chats.campaigns.edit', ['campaign' => $campaign, 'tab' => 'conditions']) }}">
                ...
                <div class="card-body">
                    ...
                    @if($currentTab === 'general')
                        @include('settings.views.chats.campaigns.tabs.general', ['campaign' => $campaign])
                    @endif

                    {{-- Content Tab --}}
                    @if($currentTab === 'content')
                        @include('settings.views.chats.campaigns.tabs.content', ['campaign' => $campaign])
                    @endif

                    {{-- Appearance Tab --}}
                    @if($currentTab === 'appearance')
                        @include('settings.views.chats.campaigns.tabs.appearance', ['campaign' => $campaign])
                    @endif

                    {{-- Conditions Tab --}}
                    @if($currentTab === 'conditions')
                        @include('settings.views.chats.campaigns.tabs.conditions', ['campaign' => $campaign])
                    @endif
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        // For now, just trigger preview update logic
    }
