...
<div class="container-fluid">
    <!-- Header -->
    @include('Chat::settings.includes.card', ['title' => 'Centro de Ayuda'])
...
$(document).ready(function() {
    ...
    function loadContent() {
        ...
        let url = '{{ route("manager.chats.helpcenter.api.categories") }}';

        if (currentLevel === 'sections' && currentParentId) {
            url = `{{ url('manager/chat/helpcenter/api/categories') }}/${currentParentId}/sections`;
        } else if (currentLevel === 'articles' && currentParentId) {
            url = `{{ url('manager/chat/helpcenter/api/sections') }}/${currentParentId}/articles`;
        }
    ...
    function createCategoryItem(item) {
        ...
        // Create element safely without XSS risk
        const $item = $('<div>')
            .addClass('hc-item list-group-item border mb-2 rounded p-3')
            .attr('data-id', item.id);

        const $flex = $('<div>').addClass('d-flex align-items-center gap-3');

        // Drag handle
        const $dragHandle = $('<div>').addClass('drag-handle').append($('<i>').addClass('fa fa-grip-vertical'));

        // Icon
        const $iconDiv = $('<div>').addClass('hc-item-icon').append($('<i>').addClass('fa ' + icon));

        // Content
        const $content = $('<div>').addClass('flex-grow-1').attr('data-action', 'navigate');
        const $title = $('<h6>').addClass('mb-0 fw-semibold').text(item.name);
        $content.append($title);

        if (item.description) {
            const $description = $('<p>').addClass('text-muted small mb-0 mt-1').text(item.description);
            $content.append($description);
        }

        const $count = $('<small>').addClass('text-muted').text(countText);
        $content.append($count);

        // Actions
        const $actions = $('<div>').addClass('hc-item-actions d-flex gap-2');
        const $editBtn = $('<button>').addClass('btn btn-sm btn-outline-primary').attr({
            'data-action': 'edit',
            'title': 'Editar'
        }).append($('<i>').addClass('fa fa-edit'));

        const $deleteBtn = $('<button>').addClass('btn btn-sm btn-outline-danger').attr({
            'data-action': 'delete',
            'title': 'Eliminar'
        ...
        $flex.append($dragHandle).append($iconDiv).append($content).append($actions);
        $item.append($flex);
    ...
    function createArticleItem(item) {
        ...
        // Icon
        const $iconDiv = $('<div>').addClass('hc-item-icon').append($('<i>').addClass('fa fa-file-alt'));

        // Content
        const $content = $('<div>').addClass('flex-grow-1');
        const $title = $('<h6>').addClass('mb-0 fw-semibold').text(item.title);
        $content.append($title);

        const $meta = $('<div>').addClass('mt-1');

        // Status badge
        const statusClass = item.draft ? 'bg-warning-subtle text-warning' : 'bg-success-subtle text-success';
        const statusText = item.draft ? 'Borrador' : 'Publicado';
        const $badge = $('<span>').addClass('badge ' + statusClass).text(statusText);
        $meta.append($badge);

        const $views = $('<small>').addClass('text-muted ms-2').text(item.views + ' vistas');
        $meta.append($views);
    ...
    function updateUI() {
        if (currentLevel === 'categories') {
            ...
            const $li = $('<li>').addClass('breadcrumb-item active');
            $li.append($('<i>').addClass('fa fa-home me-1')).append(document.createTextNode(' Categorías'));
    ...
    function handleReorder() {
        ...
        if (currentLevel === 'articles') {
            url = `{{ url('manager/chat/helpcenter/api/sections') }}/${currentParentId}/articles/reorder`;
            data = { ids: ids };
        } else {
            url = '{{ route("manager.chats.helpcenter.api.categories.reorder") }}';
    ...
    function handleDelete(itemId) {
        ...
        const url = currentLevel === 'articles'
            ? `{{ url('manager/chat/helpcenter/api/articles') }}/${itemId}`
            : `{{ url('manager/chat/helpcenter/api/categories') }}/${itemId}`;
    ...
    function saveItem() {
        ...
        const url = currentLevel === 'articles'
            ? '{{ route("manager.chats.helpcenter.api.articles.create") }}'
            : '{{ route("manager.chats.helpcenter.api.categories.create") }}';
