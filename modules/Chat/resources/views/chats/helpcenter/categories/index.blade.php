...
@section('content')
  ...
  @include('Chat::settings.includes.card', ['title' => 'Centro de Ayuda - Categorías'])
        ...
        <div class="col-md-12 col-xl-12">
          <form class="position-relative form-search" action="{{ request()->fullUrl() }}" method="GET">
            <div class="row justify-content-between g-2">
              ...
              <div class="col-auto">
                <a href="{{ route('manager.chats.helpcenter.categories.create') }}" class="btn btn-primary">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                      <li>
                        <a class="dropdown-item d-flex align-items-center gap-3"
                           href="{{ route('manager.chats.helpcenter.categories.show', $category->id) }}">
                          <i class="fa-duotone fa-eye"></i> Ver Categoría
                        </a>
                      </li>
                      <li>
                        <a class="dropdown-item d-flex align-items-center gap-3"
                           href="{{ route('manager.chats.helpcenter.sections.create', ['parent_id' => $category->id]) }}">
                      ...
                      <li>
                        <a class="dropdown-item d-flex align-items-center gap-3"
                           href="{{ route('manager.chats.helpcenter.categories.edit', $category->id) }}">
                          <i class="fa-duotone fa-edit"></i> Editar
                        </a>
                      </li>
                      <li>
                        <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                           onclick="deleteItem('{{ route('manager.chats.helpcenter.categories.destroy', $category->id) }}')">
                ...
                @foreach($category->sections as $section)
                  <tr class="search-items bg-light-subtle">
                    ...
                    <td class="ps-3">
                      ...
                      <a href="{{ route('manager.chats.helpcenter.sections.show', $section->id) }}"
                    ...
                    <td>
                      <div class="dropdown dropstart">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $section->id }}">
                          <li>
                            <a class="dropdown-item d-flex align-items-center gap-3"
                               href="{{ route('manager.chats.helpcenter.sections.show', $section->id) }}">
                              <i class="fa-duotone fa-eye"></i> Ver Sección
                            </a>
                          </li>
                          <li>
                            <a class="dropdown-item d-flex align-items-center gap-3"
                               href="{{ route('manager.chats.helpcenter.sections.articles.create', $section->id) }}">
                          ...
                          <li>
                            <a class="dropdown-item d-flex align-items-center gap-3"
                               href="{{ route('manager.chats.helpcenter.sections.edit', $section->id) }}">
                              <i class="fa-duotone fa-edit"></i> Editar
                            </a>
                          </li>
                          <li>
                            <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                               onclick="deleteItem('{{ route('manager.chats.helpcenter.sections.destroy', $section->id) }}')">
                              <i class="fa-duotone fa-trash"></i> Eliminar
                            </button>
                          </li>
                        </ul>
...
function deleteItem(url) {
    ...
    $.ajax({
        ...
        success: function(response) {
            toastr.success(response.message || 'Elemento eliminado correctamente');
            location.reload();
        },
        error: function(xhr) {
            const message = xhr.responseJSON?.error || xhr.responseJSON?.message || 'Error al eliminar';
