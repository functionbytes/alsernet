<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h5 class="mb-1"><i class="fas fa-brain me-2"></i>Base de conocimiento</h5>
    ...
    <div class="btn-group">
        <button type="button" class="btn btn-outline-warning" onclick="importFromHelpCenter()">
            <i class="fas fa-download me-2"></i>Importar desde centro de ayuda
        </button>
                ...
                <div class="card h-100 {{ $item->is_active ? '' : 'bg-light' }}">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center text-muted small mb-2">
                            <span><i class="fas fa-eye me-1"></i>{{ $item->usage_count }} usos</span>
                            @if($item->embedding_model)
                                <span class="badge bg-success-subtle text-success"><i class="fas fa-project-diagram me-1"></i>Embedding</span>
                            @endif
                        </div>
...
function openKnowledgeModal(id = null) {
    ...
    if (id) {
        $.get(`/dashboard/chat/ai/knowledge/${id}`, function(data) {
            ...
            const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al cargar el documento';
...
function toggleKnowledge(id, isActive) {
    $.post(`{{ route('manager.chats.ai.knowledge.toggle', '') }}/${id}`, {
        _token: '{{ csrf_token() }}',
        is_active: isActive
    }).done(function() {
        showSuccess('Documento ' + (isActive ? 'activado' : 'desactivado') + ' correctamente');
    }).fail(function(xhr) {
        const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al actualizar el documento';
        showError(message);
    });
}
...
function generateEmbedding(id) {
    if (confirm('¿Generar embedding para este documento? Esto puede tardar unos momentos.')) {
        $.post(`{{ route('manager.chats.ai.knowledge.generate-embedding', '') }}/${id}`, {
            _token: '{{ csrf_token() }}'
        }).done(function() {
            ...
            const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al generar el embedding';
            showError(message);
        });
    }
}
...
function deleteKnowledge(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este documento?')) {
        $.ajax({
            url: `{{ route('manager.chats.ai.knowledge.destroy', '') }}/${id}`,
            method: 'DELETE',
            ...
            const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al eliminar el documento';
            showError(message);
        });
    }
}
...
$('#knowledgeForm').on('submit', function(e) {
    ...
    const url = knowledgeId
        ? `{{ route('manager.chats.ai.knowledge.update', '') }}/${knowledgeId}`
        : `{{ route('manager.chats.ai.knowledge.store') }}`;
