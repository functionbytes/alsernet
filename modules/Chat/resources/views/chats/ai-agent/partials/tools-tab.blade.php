<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h5 class="mb-1"><i class="fas fa-tools me-2"></i>Herramientas del agente</h5>
...
function openToolModal(id = null) {
    ...
    if (id) {
        $.get(`/dashboard/chat/ai/tools/${id}`, function(data) {
            $('#tool_id').val(data.id);
            ...
            const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al cargar la herramienta';
...
function toggleTool(id, isActive) {
    $.post(`{{ route('manager.chats.ai.tools.toggle', '') }}/${id}`, {
        _token: '{{ csrf_token() }}',
        is_active: isActive
    }).done(function() {
        showSuccess('Herramienta ' + (isActive ? 'activada' : 'desactivada') + ' correctamente');
    }).fail(function(xhr) {
        const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al actualizar la herramienta';
        showError(message);
    });
}
...
function deleteTool(id) {
    if (confirm('¿Estás seguro de que deseas eliminar esta herramienta?')) {
        $.ajax({
            url: `{{ route('manager.chats.ai.tools.destroy', '') }}/${id}`,
            method: 'DELETE',
            ...
            const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al eliminar la herramienta';
            showError(message);
        });
    }
}
...
$('#toolForm').on('submit', function(e) {
    ...
    const url = toolId
        ? `{{ route('manager.chats.ai.tools.update', '') }}/${toolId}`
        : `{{ route('manager.chats.ai.tools.store') }}`;
