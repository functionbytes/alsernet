<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h5 class="mb-1"><i class="fas fa-tags me-2"></i>Gestión de tags</h5>
    ...
    <button type="button" class="btn btn-primary" onclick="openTagModal()">
        <i class="fas fa-plus me-2"></i>Nuevo tag
    </button>
</div>
                    ...
                    <div class="card-body">
                        ...
                        @if($tag->system_prompt_addition)
                            <div class="alert alert-light py-2 px-2 mb-2">
                                <small class="text-muted"><i class="fas fa-code me-1"></i>{{ Str::limit($tag->system_prompt_addition, 60) }}</small>
                            </div>
                        @endif
...
function openTagModal(id = null) {
    ...
    if (id) {
        // Load tag data
        $.get(`/dashboard/chat/ai/tags/${id}`, function(data) {
            $('#tag_id').val(data.id);
            ...
            const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al cargar el tag';
...
function toggleTag(id, isActive) {
    $.post(`{{ route('manager.chats.ai.tags.toggle', '') }}/${id}`, {
        _token: '{{ csrf_token() }}',
        is_active: isActive
    }).done(function() {
        showSuccess('Tag ' + (isActive ? 'activado' : 'desactivado') + ' correctamente');
    }).fail(function(xhr) {
        const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al actualizar el tag';
        showError(message);
    });
}
...
function deleteTag(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este tag?')) {
        $.ajax({
            url: `{{ route('manager.chats.ai.tags.destroy', '') }}/${id}`,
            method: 'DELETE',
            ...
            const message = xhr.responseJSON?.message || xhr.responseJSON?.error || 'Error al eliminar el tag';
            showError(message);
        });
    }
}
...
$('#tagForm').on('submit', function(e) {
    ...
    const url = tagId
        ? `{{ route('manager.chats.ai.tags.update', '') }}/${tagId}`
        : `{{ route('manager.chats.ai.tags.store') }}`;
