...
@section('content')
    ...
    <div class="card w-100">
        ...
        <form id="cannedForm" method="POST" action="{{ route('settings.chat.canneds.update', $canned) }}">
            ...
            <div class="card-body">
                ...
                <p class="card-subtitle mb-3 mt-1">
                    Modifica el contenido y configuración de la respuesta predefinida.
                </p>

                <!-- Usage Statistics Alert -->
                @if($canned->usage_count > 0)
                    <div class="alert alert-info d-flex align-items-center mb-3" role="alert">
                        <i class="fa fa-chart-line me-2"></i>
                        <div>
                            Esta respuesta se ha usado <strong>{{ $canned->usage_count }}</strong> {{ $canned->usage_count === 1 ? 'vez' : 'veces' }}.
                            @if($canned->last_used_at)
                                Último uso: {{ $canned->last_used_at->diffForHumans() }}.
                            @endif
                        </div>
                    </div>
                @endif
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $canned->title) }}" placeholder="Ej: Saludo de bienvenida">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="short_code" class="form-control @error('short_code') is-invalid @enderror" value="{{ old('short_code', $canned->short_code) }}" placeholder="saludo" required>
                            </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="mb-3">
                            ...
                            <select name="visibility" id="visibility" class="form-control select2 @error('visibility') is-invalid @enderror" required>
                                <option value="personal" {{ old('visibility', $canned->visibility) === 'personal' ? 'selected' : '' }}>Personal (solo tú)</option>
                                <option value="team" {{ old('visibility', $canned->visibility) === 'team' ? 'selected' : '' }}>Equipo</option>
                                <option value="everyone" {{ old('visibility', $canned->visibility) === 'everyone' ? 'selected' : '' }}>Todos (toda la cuenta)</option>
                            </select>
                            ...
                            <select name="team_ids[]" id="team_ids" class="form-control select2 select2" multiple>
                                @foreach($teams as $team)
                                    <option value="{{ $team->id }}"
                                        {{ in_array($team->id, old('team_ids', $canned->team_ids ?? [])) ? 'selected' : '' }}>
                                        {{ $team->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="user_ids[]" id="user_ids" class="form-control select2 select2" multiple>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ in_array($user->id, old('user_ids', $canned->user_ids ?? [])) ? 'selected' : '' }}>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="description" class="form-control @error('description') is-invalid @enderror" value="{{ old('description', $canned->description) }}" placeholder="Descripción breve (opcional)">
                    ...
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <textarea name="content" id="content" class="form-control @error('content') is-invalid @enderror" rows="6" placeholder="Escribe el contenido de la respuesta predefinida..." required>{{ old('content', $canned->content) }}</textarea>
