...
@section('content')
    ...
    <div class="card w-100">
        ...
        <form action="{{ route('settings.chat.automation-rules.update', $Automation->id) }}" method="POST" id="automation-form">
            ...
            <div class="card-body">
                <div class="d-flex no-block align-items-center">
                    <h5 class="mb-0">Editar regla de automatización</h5>
                </div>
                <p class="card-subtitle mb-3 mt-1">
                    Modifica las condiciones y acciones de la regla de automatización.
                </p>
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-control select2 @error('event_name') is-invalid @enderror"
                                ...
                                <option value="conversation_created" {{ old('event_name', $Automation->event_name) == 'conversation_created' ? 'selected' : '' }}>
                                    Conversación creada
                                </option>
                                <option value="conversation_updated" {{ old('event_name', $Automation->event_name) == 'conversation_updated' ? 'selected' : '' }}>
                                    Conversación actualizada
                                </option>
                                <option value="conversation_status_changed" {{ old('event_name', $Automation->event_name) == 'conversation_status_changed' ? 'selected' : '' }}>
                                    Estado de conversación cambiado
                                </option>
                                <option value="message_created" {{ old('event_name', $Automation->event_name) == 'message_created' ? 'selected' : '' }}>
                                    Mensaje creado
                                </option>
                            </select>
                            @error('event_name')
                    ...
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror"
                                      ...
                                      name="description"
                                      rows="2"
                                      placeholder="Describe qué hace esta regla...">{{ old('description', $Automation->description) }}</textarea>
                            @error('description')
                                <span class="field-validation-error">
                                    <i class="fas fa-circle-exclamation"></i> {{ $message }}
                                </span>
                    ...
                    <div class="col-12">
                        ...
                        <div id="conditions-container">
                            @foreach(old('conditions', $Automation->conditions ?? []) as $index => $condition)
                                        ...
                                        <div class="row g-2 align-items-start">
                                            <div class="col-md-3">
                                                ...
                                                <select class="form-control select2 form-select-sm condition-attribute"
                                                        name="conditions[{{ $index }}][attribute]" required>
                                                    <option value="">Selecciona...</option>
                                                    <option value="status" {{ ($condition['attribute'] ?? '') == 'status' ? 'selected' : '' }}>Estado</option>
                                                    <option value="priority" {{ ($condition['attribute'] ?? '') == 'priority' ? 'selected' : '' }}>Prioridad</option>
                                                    <option value="assignee_id" {{ ($condition['attribute'] ?? '') == 'assignee_id' ? 'selected' : '' }}>Agente asignado</option>
                                                    <option value="team_id" {{ ($condition['attribute'] ?? '') == 'team_id' ? 'selected' : '' }}>Equipo</option>
                                                    <option value="message_content" {{ ($condition['attribute'] ?? '') == 'message_content' ? 'selected' : '' }}>Contenido del mensaje</option>
                                                </select>
                                            </div>
                                            <div class="col-md-2">
                                                <label class="form-label small">Operador</label>
                                                <select class="form-control select2 form-select-sm"
                                                        name="conditions[{{ $index }}][operator]" required>
                                                    <option value="equal_to" {{ ($condition['operator'] ?? '') == 'equal_to' ? 'selected' : '' }}>Igual a</option>
                                                    <option value="not_equal_to" {{ ($condition['operator'] ?? '') == 'not_equal_to' ? 'selected' : '' }}>Diferente de</option>
                                                    <option value="contains" {{ ($condition['operator'] ?? '') == 'contains' ? 'selected' : '' }}>Contiene</option>
                                                    <option value="does_not_contain" {{ ($condition['operator'] ?? '') == 'does_not_contain' ? 'selected' : '' }}>No contiene</option>
                                                </select>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label small">Valor</label>
                                                <div class="condition-value-container" data-value="{{ $condition['value'] ?? '' }}"></div>
                                            </div>
                                            <div class="col-md-1">
                    ...
                    <div class="col-12">
                        ...
                        <div id="actions-container">
                            @foreach(old('actions', $Automation->actions ?? []) as $index => $action)
                                        ...
                                        <div class="row g-2 align-items-start">
                                            <div class="col-md-4">
                                                ...
                                                <select class="form-control select2 form-select-sm action-type"
                                                    ...
                                                    <option value="">Selecciona...</option>
                                                    <option value="assign_agent" {{ ($action['action'] ?? '') == 'assign_agent' ? 'selected' : '' }}>Asignar a agente</option>
                                                    <option value="assign_team" {{ ($action['action'] ?? '') == 'assign_team' ? 'selected' : '' }}>Asignar a equipo</option>
                                                    <option value="add_label" {{ ($action['action'] ?? '') == 'add_label' ? 'selected' : '' }}>Añadir etiqueta</option>
                                                    <option value="remove_label" {{ ($action['action'] ?? '') == 'remove_label' ? 'selected' : '' }}>Quitar etiqueta</option>
                                                    <option value="change_status" {{ ($action['action'] ?? '') == 'change_status' ? 'selected' : '' }}>Cambiar estado</option>
                                                    <option value="change_priority" {{ ($action['action'] ?? '') == 'change_priority' ? 'selected' : '' }}>Cambiar prioridad</option>
                                                    <option value="send_message" {{ ($action['action'] ?? '') == 'send_message' ? 'selected' : '' }}>Enviar mensaje</option>
                                                    <option value="add_private_note" {{ ($action['action'] ?? '') == 'add_private_note' ? 'selected' : '' }}>Añadir nota privada</option>
                                                    <option value="resolve_conversation" {{ ($action['action'] ?? '') == 'resolve_conversation' ? 'selected' : '' }}>Resolver conversación</option>
                                                    <option value="reopen_conversation" {{ ($action['action'] ?? '') == 'reopen_conversation' ? 'selected' : '' }}>Reabrir conversación</option>
                                                </select>
                                            </div>
                                            <div class="col-md-7">
                                                <label class="form-label small">Valor</label>
                                                <div class="action-value-container" data-value="{{ $action['value'] ?? '' }}"></div>
                                            </div>
                                            <div class="col-md-1">
                                                <label class="form-label small">&nbsp;</label>
...
@php
    ...
    $existingConditions = old('conditions', $Automation->conditions ?? []);
    $existingActions = old('actions', $Automation->actions ?? []);
@endphp
...
$(document).ready(function() {
    let conditionIndex = existingConditions.length;
    let actionIndex = existingActions.length;
    ...
    // Initialize existing conditions with their value inputs
    $('.condition-item').each(function() {
        const conditionItem = $(this);
        const conditionType = conditionItem.find('.condition-attribute').val();
        const index = conditionItem.data('index');
        const container = conditionItem.find('.condition-value-container');
        const currentValue = container.data('value') || '';
        container.html(createConditionValueInput(conditionType, index, currentValue));
    });

    // Initialize existing actions with their value inputs
    $('.action-item').each(function() {
        ...
        const currentValue = container.data('value') || '';
        container.html(createActionValueInput(actionType, index, currentValue));
    });
