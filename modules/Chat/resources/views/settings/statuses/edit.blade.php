...
<div class="container-fluid">
    ...
    <div class="row mb-4 align-items-center">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="mb-1 fw-bold">Edit status</h4>
                    <p class="text-muted small mb-0">Update the status configuration</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="mb-0 fw-bold">Status information</h5>
        </div>

        <form id="statusForm" method="POST" action="{{ route('manager.chat.settings.tickets.statuses.update', $status->id) }}" novalidate>
            @csrf
            @method('PUT')
                ...
                <div class="row g-4">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $status->name) }}" placeholder="e.g., In progress">
                            @error('name')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                            <small class="text-muted d-block mt-1">Visible name of the status</small>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            @error('slug')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                            <small class="text-muted d-block mt-1">Slug cannot be modified once created</small>
                        </div>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control" rows="3" placeholder="Optional description of the status">{{ old('description', $status->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                            <small class="text-muted d-block mt-1">Provide additional context about this status</small>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <input type="color" name="color" class="form-control form-control-color" value="{{ old('color', $status->color) }}" id="colorPicker" style="width: 60px; height: 40px;">
                                <input type="text" id="colorHex" class="form-control" value="{{ old('color', $status->color) }}" readonly style="max-width: 120px;">
                                <div id="colorPreview" class="rounded border" style="width: 40px; height: 40px; background-color: {{ old('color', $status->color) }};"></div>
                            </div>
                            <small class="text-muted d-block mt-1">Choose a color to identify the status visually</small>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-check form-switch">
                            ...
                            <small class="text-muted d-block mt-1">When enabled, this status is available for assignment to conversations</small>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-check form-switch">
                            ...
                            <small class="text-muted d-block mt-1">Automatically assigned to all new conversations. Only one default status can be active</small>
                        </div>
                    </div>
            ...
            <div class="card-footer bg-light border-top d-flex gap-2">
                ...
                <a href="{{ route('manager.chat.settings.tickets.statuses.index') }}" class="btn btn-secondary">
                    Cancel
                </a>
            </div>
