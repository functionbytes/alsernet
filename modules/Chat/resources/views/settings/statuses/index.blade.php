    ...
    <div class="row mb-4 align-items-center">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="{{ route('manager.chat.settings.tickets.statuses.create') }}" class="btn btn-primary">
        ...
        <div class="card-header border-bottom p-3">
            <div class="row g-3 align-items-end">
                <div class="col-12 col-md-5">
                    ...
                    <form method="GET" action="{{ route('manager.chat.settings.tickets.statuses.index') }}" class="d-flex gap-2">
                ...
                <div class="col-12 col-md-7 text-end">
                    @if(request('search'))
                        <a href="{{ route('manager.chat.settings.tickets.statuses.index') }}" class="btn btn-secondary btn-sm">
                            Clear filters
                        </a>
                    @endif
                </div>
                            ...
                            @foreach($statuses as $status)
                                <tr data-id="{{ $status->id }}" class="sortable-row">
                                    ...
                                    <td class="text-center">
                                        <form method="POST" action="{{ route('manager.chat.settings.tickets.statuses.toggle', $status->id) }}" class="toggle-form">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="{{ route('manager.chat.settings.tickets.statuses.edit', $status->id) }}"
            ...
            @else
                <div class="text-center py-5">
                    <div class="d-flex flex-column align-items-center">
                        ...
                        @if(!request('search'))
                            <a href="{{ route('manager.chat.settings.tickets.statuses.create') }}" class="btn btn-primary btn-sm">
...
$(document).ready(function() {
    ...
    $('.delete-status').on('click', function() {
        ...
        const deleteRoute = '{{ route("manager.chat.settings.tickets.statuses.destroy", ":id") }}'.replace(':id', statusId);
    ...
    $('#statusesList').sortable({
        ...
        update: function(event, ui) {
            ...
            // Save new order
            $.ajax({
                url: '{{ route('manager.chat.settings.tickets.statuses.reorder') }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    ids: ids
