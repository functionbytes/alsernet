...
@section('content')
    ...
    @include('Chat::settings.includes.card', ['title' => 'Grupos de Equipo'])
            ...
            <div class="card-header p-4 border-bottom border-light">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="small mb-0 text-muted">Organiza a tus agentes en grupos para asignación eficiente de
                            tickets</p>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="{{ route('manager.chat.settings.tickets.team.members') }}"
                           class="btn btn-secondary">
                            Ver miembros
                        </a>
                        <a href="{{ route('manager.chat.settings.tickets.team.group.create') }}"
                                                ...
                                                <div class="d-flex flex-wrap gap-1">
                                                    @foreach($group->users->take(4) as $user)
                                                        <div class="rounded-circle d-flex align-items-center justify-content-center"
                                                             style="width: 32px; height: 32px; background-color: {{ $user->pivot->conversation_priority === 'primary' ? '#f5f6f8' : '#f5f6f8' }}; color: {{ $user->pivot->conversation_priority === 'primary' ? '#081A28' : '#081A28' }}; font-size: 0.7rem; font-weight: 600;"
                                                             title="{{ $user->full_name }} ({{ $user->pivot->conversation_priority === 'primary' ? 'Primario' : 'Backup' }})">
                                                            {{ strtoupper(groups.blade.phpsubstr($user->firstname, 0, 1) . substr($user->lastname, 0, 1)) }}
                                                        </div>
                                                    @endforeach
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @if($group->users->count() > 0)
                                            <div class="d-flex justify-content-center gap-2">
                                                ...
                                                <span class="badge bg-info-subtle text-info">{{ $backupCount }} B</span>
                                            </div>
                                        @else
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li>
                                                    <a class="dropdown-item"
                                                       href="{{ route('manager.chat.settings.tickets.team.group.edit', $group->id) }}">
                                                        Editar
                                                    </a>
                                                </li>
                                                <li>
                                                ...
                                                <li>
                                                    <form method="POST"
                                                          action="{{ route('manager.chat.settings.tickets.team.group.destroy', $group->id) }}"
                ...
                <div class="card-footer bg-white border-top">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="text-muted small">
                            Mostrando <strong>{{ $groups->firstItem() }}</strong> a
                            <strong>{{ $groups->lastItem() }}</strong>
