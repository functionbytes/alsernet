...
class CustomerImportService
{
    ...
    public function previewImport(string $filePath, array $fieldMapping, int $accountId, array $options = []): array
    {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            $existing = $this->findExistingCustomer($accountId, $mappedData);
    ...
    protected function mapFields(array $data, array $fieldMapping): array
    {
        ...
        foreach ($fieldMapping as $csvColumn => $customerField) {
            if ($customerField && isset($data[$csvColumn])) {
                $value = trim($data[$csvColumn]);

                if ($value !== '') {
                    $mapped[$customerField] = $value;
                }
            }
        }
     ...
     */
    protected function findExistingCustomer(int $accountId, array $data): ?Customer
    {
    ...
    public function generateTemplate(): string
    {
        ...
        $filename = storage_path('app/temp/customer_import_template.csv');
