...
namespace Modules\Chat\Services\Channels\Facebook;
...
class ApiClient
{
    ...
    {
        $callbackUrl = route('settings.chat.channels.facebook-pages.callback');
        $scope = 'pages_messaging,pages_manage_metadata,pages_read_engagement';

        return 'https://www.facebook.com/v24.0/dialog/oauth?'.http_build_query([
    ...
    /**
     * Send an attachment (image, file, video, audio) to a user by URL.
     *
     * @param  string  $pageAccessToken  Facebook page access token
     * @param  string  $recipientId  Recipient user ID
     * @param  string  $type  Attachment type (image, video, audio, file)
     * @param  string  $url  Public URL of the file (accessible from internet)
     */
    public function sendAttachment(string $pageAccessToken, string $recipientId, string $type, string $url): array
    {
        $response = Http::post($this->graphApiUrl.'/me/messages', [
            'access_token' => $pageAccessToken,
            'recipient' => ['id' => $recipientId],
        ...
        if ($response->failed()) {
            throw new \Exception('Failed to send attachment: '.$response->body());
        }
