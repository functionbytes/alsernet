        ...
        <div class="ve-inspector-section">
            ...
            <div id="ve-sect-gradient" class="ve-hidden ve-sect-body">
                ...
                <div class="ve-gradient-row">
                    <input type="color" class="ve-color-picker" id="ve-gradient-color1" value="#90bb13">
                    <label>Color 1</label>
                </div>
                <div class="ve-gradient-row">
...
.ve-btn-primary {
    background: var(--ve-primary, #90bb13);
    border-color: var(--ve-primary, #90bb13);
    color: #fff;
    font-size: 12px;
}
...
.ve-field-required-error label { color: #90bb13 !important; }
.ve-field-required-error input,
.ve-field-required-error textarea { border-color: #90bb13 !important; }
...
.ve-sc-preset-delete:hover { color:#90bb13; }
</style>
...
(function ($) {
            ...
            error: function (xhr) {
                ...
                if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                    ...
                    $.each(xhr.responseJSON.errors, function (key, msgs) {
                        ...
                        if ($el.length) {
                            $el.after('<div class="ve-ff-error" style="color:#90bb13;font-size:10px;margin-top:2px;">' + msgs[0] + '</div>');
                        }
                    });
                    $('#ve-ff-status').html('<span style="color:#90bb13">Corrige los errores</span>');
                } else {
                    $('#ve-ff-status').html('<span style="color:#90bb13">Error al guardar</span>');
                }
            },
        });
    });
    ...
    function veBuildAttrsUI(parsedSc) {
        ...
        metaAttrs.forEach(function (ma) {
            ...
            const reqMark  = ma.required ? ' <span style="color:#90bb13">*</span>' : '';
    ...
    function showScEditor(encodedSc, scNodeId) {
        ...
        if (!parsed) {
            ...
            $('#ve-sc-status').html('<span style="color:#90bb13">Shortcode inválido — edita en modo Raw</span>');
    ...
    $(document).on('click', '#btn-sc-apply', function () {
        ...
        if (activeTab === 'raw') {
            ...
            if (missing.length) {
                $('#ve-sc-status').html('<span style="color:#90bb13"><i class="fa-solid fa-triangle-exclamation me-1"></i>Requeridos: ' + missing.join(', ') + '</span>');
                return;
            }
        ...
        $.ajax({
            ...
            error: function (xhr) {
                ...
                $('#ve-sc-status').html('<span style="color:#90bb13">' + msg + '</span>');
            },
        });
    });
    ...
    $(document).on('click', '#btn-sc-undo', function () {
        ...
        $.ajax({
            ...
            error: function () {
                $('#ve-sc-status').html('<span style="color:#90bb13">Error al revertir</span>');
            },
        });
    });
    ...
    $(document).on('click', '#btn-sc-reset', function () {
        ...
        $.ajax({
            ...
            error: function () {
                $('#ve-sc-status').html('<span style="color:#90bb13">Error al restablecer</span>');
            },
        });
    });
