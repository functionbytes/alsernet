...
<div class="preview-bar" style="position: sticky; top: 0; z-index: 1030; background: #fff; border-bottom: 3px solid #90bb13; box-shadow: 0 1px 6px rgba(0,0,0,.08); margin: -2rem 0 0 0; padding: 0;">
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 1rem; gap: 1rem;">
        <div style="display: flex; align-items: center; gap: 0.5rem; min-width: 0;">
            <span style="background: #90bb13; color: #fff; font-size: 0.65rem; font-weight: 700; letter-spacing: 0.06em; padding: 3px 8px; border-radius: 4px; flex-shrink: 0; white-space: nowrap;">VISTA PREVIA</span>
            <span style="font-weight: 600; text-overflow: ellipsis; overflow: hidden; white-space: nowrap; font-size: 0.9rem;">{{ $page->title }}</span>
            @if(!$page->isPublished())
                <span style="background: #fff3cd; color: #856404; border: 1px solid #ffc107; font-size: 0.65rem; padding: 3px 8px; border-radius: 4px; flex-shrink: 0; white-space: nowrap;">No publicada</span>
            @endif
        </div>
        <div style="display: flex; align-items: center; gap: 1rem; flex-shrink: 0; color: #6c757d; font-size: 0.8rem; white-space: nowrap;">
...
<div class="preview-page-content" style="padding: 2rem 0;">
    @if($page->content)
        {!! $page->content !!}
    @else
        <p class="text-muted text-center">Esta página no tiene contenido.</p>
    @endif
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Prevenir navegación en links internos (opcional)
    // document.querySelectorAll('a').forEach(function (link) {
    //     link.addEventListener('click', function (e) {
    //         if (this.href && !this.href.startsWith('javascript:')) {
    //             e.preventDefault();
    //             alert('La navegación está deshabilitada en modo vista previa.');
    //         }
    //     });
    // });
});
