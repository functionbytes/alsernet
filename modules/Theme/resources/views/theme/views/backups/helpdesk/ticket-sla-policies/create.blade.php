...
@section('content')
    ...
    <div class="row g-3">
        ...
        <div class="col-12 col-lg-8">
            <div class="card">
                <form id="slaForm" action="{{ route('manager.helpdesk.settings.ticket-sla-policies.store') }}" method="POST">
                    @csrf
                            ...
                            @foreach($multipliers as $key => $cfg)
                                <div class="col-6 col-md-3">
                                    ...
                                    <input type="number" name="priority_multipliers[{{ $key }}]"
                                           class="form-control"
                                           value="{{ old('priority_multipliers.' . $key, $cfg['default']) }}"
                                           step="0.25" min="0.25">
                                </div>
                            @endforeach
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-select @error('enable_escalation') is-invalid @enderror"
                                    ...
                                    <option value="0" {{ old('enable_escalation', '0') == '0' ? 'selected' : '' }}>Desactivado — no se envian alertas de escalamiento</option>
                                    <option value="1" {{ old('enable_escalation', '0') == '1' ? 'selected' : '' }}>Activo — enviar alertas al alcanzar el umbral</option>
                                </select>
                            ...
                            <div class="col-12 col-md-6" id="escalation-threshold-wrap" style="{{ old('enable_escalation') == '1' ? '' : 'display:none;' }}">
                                ...
                                <input type="number" name="escalation_threshold_percent"
                                       ...
                                       value="{{ old('escalation_threshold_percent', 80) }}"
                                       min="1" max="100">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-select @error('active') is-invalid @enderror" name="active" id="active">
                                    <option value="1" {{ old('active', '1') == '1' ? 'selected' : '' }}>Activa — disponible para aplicar a tickets</option>
                                    <option value="0" {{ old('active', '1') == '0' ? 'selected' : '' }}>Inactiva — no se aplica a nuevos tickets</option>
                                </select>
                                @error('active')
                                    <span class="field-validation-error"><i class="fas fa-circle-exclamation"></i> {{ $message }}</span>
                                @enderror
