            ...
            <div class="card">
                <form id="viewForm" action="{{ route('manager.helpdesk.settings.ticket-views.update', $view) }}" method="POST">
                    ...
                    <div class="card-header border-bottom p-3">
                        ...
                        <small class="text-muted">Modifica las propiedades de la vista guardada</small>
                    </div>
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="description"
                                          ...
                                          rows="2"
                                          placeholder="Describe que tickets muestra esta vista">{{ old('description', $view->description) }}</textarea>
                                @error('description')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="filters_json" id="filtersJson"
                                          ...
                                          rows="6"
                                          placeholder='{"status":"open","priority":"high"}'>{{ json_encode($view->filters ?? [], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</textarea>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select name="is_shared" class="form-select @error('is_shared') is-invalid @enderror">
                                    <option value="0" {{ old('is_shared', $view->is_shared ? '1' : '0') === '0' ? 'selected' : '' }}>Privada — solo visible para mi</option>
                                    <option value="1" {{ old('is_shared', $view->is_shared ? '1' : '0') === '1' ? 'selected' : '' }}>Compartida — visible para todo el equipo</option>
                                </select>
                                @error('is_shared')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select name="is_default" class="form-select @error('is_default') is-invalid @enderror">
                                    <option value="0" {{ old('is_default', $view->is_default ? '1' : '0') === '0' ? 'selected' : '' }}>No — usar otra vista por defecto</option>
                                    <option value="1" {{ old('is_default', $view->is_default ? '1' : '0') === '1' ? 'selected' : '' }}>Si — aplicar como vista predeterminada</option>
                                </select>
                                @error('is_default')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
