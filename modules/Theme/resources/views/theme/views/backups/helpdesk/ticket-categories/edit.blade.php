...
<div class="container-fluid">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
            <h4 class="mb-0 fw-bold">
                @if($category->icon)
                    <i class="{{ $category->icon }} me-2" style="color:{{ $category->color ?? '#90bb13' }}"></i>
                @endif
                Editar: {{ $category->name }}
            </h4>
            <p class="text-muted mb-0">Modifica las propiedades de la categoría</p>
        </div>
    </div>
    ...
    <div class="card">
        <form method="POST" action="{{ route('manager.helpdesk.backups.tickets.categories.update', $category->id) }}">
            @csrf
            @method('PUT')
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div id="colorPreview" class="border rounded"
                                 style="width:40px;height:40px;background-color:{{ old('color', $category->color ?? '#90bb13') }};"></div>
                        </div>
                        @error('color')
