...
class ChatFlowEngine
{
    ...
    {
        $conditions = $branchItem['data']['conditions'] ?? [];

        // Opt-in OR grouping: a branchItem may declare `data.match` = "any" to
        // require only one matching condition. Absent (legacy) or "all" keeps the
        // historical implicit-AND behaviour untouched.
        $match = strtolower((string) ($branchItem['data']['match'] ?? 'all'));
     ...
     * opt-in operators selected by the node data and never reached by legacy
     * branchItems. Unknown operators fall through to `false` as before.
     *
