...
<script async src="{$apiUrl}/eng/api/assets/engagement-sdk"></script>
<script>
...
(function(w, d, apiUrl, token) {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            try { w.HELPDESK_WIDGET_CONFIG = JSON.parse(xhr.responseText); } catch (e) {}
        }
        loadWidget();
    };
    xhr.onerror = loadWidget;
    xhr.send();

    function loadWidget() {
        // Font Awesome (widget icons)
        ...
        // Widget bundle — served via Laravel proxy to add CORS headers
...
HTML;
    ...
    /**
     * True when API URL points to a local/testing domain — disables TLS verify
     * to avoid self-signed certificate failures in dev environments.
     */
    private function isTestingEndpoint(string $url): bool
    {
        return (bool) preg_match('/\.test|localhost|127\.0\.0\.1|host\.docker\.internal/i', $url);
    }
    ...
    public function syncCatalog(int $page = 1, int $perPage = 250): array
    {
        ...
        $url = rtrim($apiUrl, '/').'/eng/api/sdk/catalog/sync';
        $ch = curl_init($url);
        $opts = [
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => $payload,
