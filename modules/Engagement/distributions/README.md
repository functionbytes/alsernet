...
El secret se almacena en `helpdesk_livechat_platform_integrations.webhook_secret` (oculto en API responses).
...
3. El `PlatformWebhookController` recibe webhooks → `PlatformWebhookHandler` los traduce a eventos `LivechatEvent` → `ScoringService` actualiza score → Reverb avisa al SDK.
