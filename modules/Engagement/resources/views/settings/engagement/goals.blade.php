...
<style>
    ...
    .funnel-step { padding: .75rem 1rem; border-left: 4px solid #90bb13; background: #f9f9f9; margin-bottom: .25rem; border-radius: 0 6px 6px 0; }
...
@section('content')
    <div class="lc-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            <a href="{{ route('settings.engagement.index') }}" class="btn btn-light btn-sm"><i class="fas fa-arrow-left me-1"></i>Volver</a>
        </div>
    </div>
    ...
    <div class="card mb-3">
        <div class="card-body d-flex align-items-center gap-3 flex-wrap">
            ...
            <div class="ms-auto d-flex gap-2">
                ...
                <button class="btn btn-primary" id="btnNew"><i class="fas fa-plus me-1"></i>Nuevo objetivo</button>
                @endcan
            </div>
        </div>
    </div>
...
$(function () {
    const routes = {
        ...
        trashed: '{{ route('settings.engagement.goals.trashed') }}',
        restore: (id) => '{{ route('settings.engagement.goals.restore', ['id' => '__ID__']) }}'.replace('__ID__', id),
    };
    ...
    const grid = $('#goalsGrid').dxDataGrid({
        dataSource: {
            load: () => {
                ...
                return $.get(url, { inbox_id: $('#inboxSelector').val() }).then(r => r.data);
            },
        },
        columns: [
    ...
    $(document).on('click', '[data-action="restore"]', function (e) {
        ...
        $.ajax({ url: routes.restore(id), method: 'POST', headers: { 'X-CSRF-TOKEN': csrf } })
            .done(() => { grid.refresh(); toastr.success('Objetivo restaurado.'); })
