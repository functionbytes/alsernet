    ...
    <div class="card mb-3">
        <div class="card-body d-flex align-items-center gap-3 flex-wrap">
            ...
            <div class="ms-auto d-flex gap-2">
                ...
                <button class="btn btn-primary" id="btnNewFlow"><i class="fas fa-plus me-1"></i>Nuevo flujo</button>
                @endcan
            </div>
        </div>
    </div>
...
$(function () {
    const routes = {
        ...
        trashed: '{{ route('settings.engagement.automation.trashed') }}',
        restore: (id) => '{{ route('settings.engagement.automation.restore', ['id' => '__ID__']) }}'.replace('__ID__', id),
    };
    ...
    $(document).on('click', '[data-action="restore"]', function (e) {
        ...
        $.ajax({ url: routes.restore(id), method: 'POST', headers: { 'X-CSRF-TOKEN': csrf } })
            .done(() => { grid.refresh(); toastr.success('Flujo restaurado.'); })
