        ...
        <div class="card-header border-bottom p-3">
            <div class="row g-3 align-items-end">
                <div class="col-12 col-md-4">
                    ...
                    <form method="GET" action="{{ route('manager.helpdesk.settings.tickets.attributes.index') }}" class="d-flex gap-2">
                ...
                <div class="col-12 col-md-3">
                    ...
                    <select name="permission" class="form-select" id="permissionSelect">
                ...
                <div class="col-12 col-md-2 text-end">
                    @if(request('search') || request('format') || request('permission'))
                        <a href="{{ route('manager.helpdesk.settings.tickets.attributes.index') }}" class="btn btn-secondary btn-sm">
                            Clear filters
                        </a>
                    @endif
                </div>
                            ...
                            @foreach($attributes as $attribute)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $formatLabels = [
                                                'text' => ['label' => 'Text', 'icon' => 'ti-text', 'color' => 'primary'],
                                                'textarea' => ['label' => 'Text area', 'icon' => 'ti-forms', 'color' => 'info'],
                                                'number' => ['label' => 'Number', 'icon' => 'ti-123', 'color' => 'success'],
                                                'switch' => ['label' => 'Switch', 'icon' => 'ti-toggle-left', 'color' => 'warning'],
                                                'rating' => ['label' => 'Rating', 'icon' => 'ti-star', 'color' => 'danger'],
                                                'select' => ['label' => 'Select', 'icon' => 'ti-list', 'color' => 'secondary'],
                                                'checkboxGroup' => ['label' => 'Checkboxes', 'icon' => 'ti-checkbox', 'color' => 'dark'],
                                                'date' => ['label' => 'Date', 'icon' => 'ti-calendar', 'color' => 'purple']
                                            ];
                                    ...
                                    <td class="text-center">
                                        <form method="POST" action="{{ route('manager.helpdesk.settings.tickets.attributes.toggle', $attribute->id) }}" class="toggle-form">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="{{ route('manager.helpdesk.settings.tickets.attributes.edit', $attribute->id) }}"
            ...
            @else
                <div class="text-center py-5">
                    <div class="d-flex flex-column align-items-center">
                        ...
                        @if(!request('search') && !request('format') && !request('permission'))
                            <a href="{{ route('manager.helpdesk.settings.tickets.attributes.create') }}" class="btn btn-primary btn-sm">
...
$(document).ready(function() {
    ...
    $('.delete-attribute').on('click', function() {
        ...
        const deleteRoute = '{{ route("manager.helpdesk.settings.tickets.attributes.destroy", ":id") }}'.replace(':id', attributeId);
