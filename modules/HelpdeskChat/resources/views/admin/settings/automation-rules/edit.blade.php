...
<div class="container-fluid">
    <div class="row mb-3">
        <div class="col">
            <h1 class="h3">Edit Automation Rule</h1>
        </div>
        <div class="col-auto">
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.automation-rules.update', $automationRule) }}" method="POST" id="automation-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                   id="name" name="name" value="{{ old('name', $automationRule->name) }}" required>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror"
                                      id="description" name="description" rows="2">{{ old('description', $automationRule->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('event_name') is-invalid @enderror"
                                ...
                                <option value="conversation_created" {{ old('event_name', $automationRule->event_name) == 'conversation_created' ? 'selected' : '' }}>Conversation Created</option>
                                <option value="conversation_updated" {{ old('event_name', $automationRule->event_name) == 'conversation_updated' ? 'selected' : '' }}>Conversation Updated</option>
                                <option value="conversation_status_changed" {{ old('event_name', $automationRule->event_name) == 'conversation_status_changed' ? 'selected' : '' }}>Conversation Status Changed</option>
                                <option value="message_created" {{ old('event_name', $automationRule->event_name) == 'message_created' ? 'selected' : '' }}>Message Created</option>
                            </select>
                            @error('event_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <div id="conditions-container">
                                @foreach(old('conditions', $automationRule->conditions ?? []) as $index => $condition)
                                            ...
                                            <div class="row align-items-start">
                                                <div class="col-md-3">
                                                    ...
                                                    <select class="form-select form-select-sm condition-attribute"
                                                        ...
                                                        <option value="status" {{ ($condition['attribute'] ?? '') == 'status' ? 'selected' : '' }}>Status</option>
                                                        <option value="priority" {{ ($condition['attribute'] ?? '') == 'priority' ? 'selected' : '' }}>Priority</option>
                                                        <option value="assignee_id" {{ ($condition['attribute'] ?? '') == 'assignee_id' ? 'selected' : '' }}>Assignee</option>
                                                        <option value="team_id" {{ ($condition['attribute'] ?? '') == 'team_id' ? 'selected' : '' }}>Team</option>
                                                        <option value="message_content" {{ ($condition['attribute'] ?? '') == 'message_content' ? 'selected' : '' }}>Message Content</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-2">
                                                    ...
                                                    <select class="form-select form-select-sm"
                                                        ...
                                                        <option value="equal_to" {{ ($condition['operator'] ?? '') == 'equal_to' ? 'selected' : '' }}>Equal to</option>
                                                        <option value="not_equal_to" {{ ($condition['operator'] ?? '') == 'not_equal_to' ? 'selected' : '' }}>Not equal to</option>
                                                        <option value="contains" {{ ($condition['operator'] ?? '') == 'contains' ? 'selected' : '' }}>Contains</option>
                                                        <option value="does_not_contain" {{ ($condition['operator'] ?? '') == 'does_not_contain' ? 'selected' : '' }}>Does not contain</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="actions-container">
                                @foreach(old('actions', $automationRule->actions ?? []) as $index => $action)
                        ...
                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="active" name="active" value="1" {{ old('active', $automationRule->active) ? 'checked' : '' }}>
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="fa fa-save me-1"></i> Update Rule
                            </button>
...
@php
    ...
    // Get existing conditions and actions for JavaScript
    $existingConditions = old('conditions', $automationRule->conditions ?? []);
    $existingActions = old('actions', $automationRule->actions ?? []);
@endphp
...
20 20 12 61 79 80 81 98 33 100 204 250 395 398 399 400 701function() {
    ...
    function updateConditionValueField(conditionItem) {
        ...
        const currentValue = existingConditions[index]?.value || '';
    ...
    function updateActionValueField(actionItem) {
        ...
        // Get current value if exists
        const currentValue = existingActions[index]?.value || '';
        container.html( createActionValueInput(actionType, index, currentValue);
    }

    // Initialize existing conditions and actions with their value inputs
    document.querySelectorAll('.condition-item').forEach(item => {
        updateConditionValueField(item);
    });

    document.querySelectorAll('.action-item').forEach(item => {
        updateActionValueField(item);
    });
