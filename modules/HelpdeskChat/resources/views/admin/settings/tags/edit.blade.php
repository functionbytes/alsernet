...
<div class="container-fluid">
    ...
    <div class="row mb-4 align-items-center">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="mb-1 fw-bold">Edit tag</h4>
                    <p class="text-muted small mb-0">Update the tag information</p>
                </div>
                @if($tag->usage_count > 0)
                    <div class="badge bg-info">
                        Used in {{ $tag->usage_count }} conversation(s)
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="card">
        ...
        <form id="tagForm" method="POST" action="{{ route('manager.helpdesk.settings.tickets.tags.update', $tag->id) }}" novalidate>
            @csrf
            @method('PUT')
                ...
                <div class="row g-4">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $tag->name) }}" placeholder="e.g., Urgent">
                            @error('name')
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="slug" class="form-control" value="{{ old('slug', $tag->slug) }}" placeholder="Auto-generated if empty">
                            @error('slug')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control" rows="3" placeholder="Optional description of the tag">{{ old('description', $tag->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <input type="color" name="color" class="form-control form-control-color" value="{{ old('color', $tag->color ?? '#081A28') }}" id="colorPicker" style="width: 60px; height: 40px;">
                                <input type="text" id="colorHex" class="form-control" value="{{ old('color', $tag->color ?? '#081A28') }}" readonly style="max-width: 120px;">
                                <div id="colorPreview" class="rounded border" style="width: 40px; height: 40px; background-color: {{ old('color', $tag->color ?? '#081A28') }};"></div>
                            </div>
