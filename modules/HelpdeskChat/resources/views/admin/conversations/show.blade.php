    ...
    <div class="d-flex">
        ...
        <div class="left-part border-end w-20 flex-shrink-0 d-none d-lg-block">
            ...
            <div class="px-9 pt-4 pb-3">
                <a href="{{ route('manager.helpdesk.conversation.create') }}" class="btn btn-primary fw-semibold py-8 w-100">
            ...
            <ul class="list-group mh-n100" data-simplebar>
                ...
                @forelse($views as $view)
                    <li class="list-group-item border-0 p-0 mx-9">
                        <a class="d-flex align-items-center gap-6 list-group-item-action text-dark px-3 py-8 mb-1 rounded-1 {{ $currentView && $currentView->id == $view->id && !request('group') ? 'bg-primary-subtle' : '' }}"
                           href="{{ route('manager.helpdesk.conversation.show', array_merge(['conversation' => $conversation->id], ['viewId' => $view->id])) }}">
                ...
                <li class="list-group-item border-0 p-0 mx-9">
                    <a class="d-flex align-items-center gap-6 list-group-item-action text-dark px-3 py-8 mb-1 rounded-1 {{ !request('group') ? 'bg-primary-subtle' : '' }}"
                       href="{{ route('manager.helpdesk.conversation.show', array_merge(['conversation' => $conversation->id], request()->except('group'), $currentView ? ['viewId' => $currentView->id] : [])) }}">
                ...
                @forelse($groups as $group)
                    <li class="list-group-item border-0 p-0 mx-9">
                        <a class="d-flex align-items-center gap-6 list-group-item-action text-dark px-3 py-8 mb-1 rounded-1 {{ request('group') == $group->id ? 'bg-primary-subtle' : '' }}"
                           href="{{ route('manager.helpdesk.conversation.show', array_merge(['conversation' => $conversation->id], request()->except('group'), ['group' => $group->id], $currentView ? ['viewId' => $currentView->id] : [])) }}">
                ...
                <li class="list-group-item border-0 p-0 mx-9" id="filtersPanel" style="display: none;">
                    <form method="GET" action="{{ route('manager.helpdesk.conversation.show', $conversation) }}" id="filtersForm">
                        @if($currentView)
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('manager.helpdesk.conversation.show', array_merge(['conversation' => $conversation->id], $currentView ? ['viewId' => $currentView->id] : [])) }}" class="btn btn-light btn-sm">
        ...
        <div class="w-30 d-none d-lg-block border-end user-chat-box">
            <div class="px-4 pt-9 pb-6">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2" href="{{ route('manager.helpdesk.conversation.create') }}">
                ...
                <form class="position-relative mb-4" method="GET" action="{{ route('manager.helpdesk.conversation.show', $conversation) }}">
                    @if($currentView)
                        <input type="hidden" name="viewId" value="{{ $currentView->id }}">
                    @endif
                    @if(request('group'))
                ...
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    @if(request()->hasAny(['status', 'priority', 'search']))
                        <a href="{{ route('manager.helpdesk.conversation.show', array_merge(['conversation' => $conversation->id], $currentView ? ['viewId' => $currentView->id] : [])) }}" class="text-primary small">
            ...
            <div class="app-chat">
                <ul class="chat-users mb-0 mh-n100" data-simplebar>
                    @forelse($conversations as $conv)
                        <li>
                            <a href="{{ route('manager.helpdesk.conversation.show', $conv) }}"
        ...
        <div class="flex-fill d-flex">
            ...
            <div class="flex-fill bg-white d-flex flex-column" style="height: calc(100vh - 140px);">
                ...
                <div class="border-top p-3 bg-light">
                    <form method="POST" action="{{ route('manager.helpdesk.conversation.messages.store', $conversation) }}" enctype="multipart/form-data" id="messageForm">
                        @csrf
                        <div class="mb-2">
                    ...
                    @if($recentConversations->count() > 0)
                        <div class="list-group list-group-flush">
                            @foreach($recentConversations as $recent)
                                <a href="{{ route('manager.helpdesk.conversation.show', $recent) }}"
...
<script>
    ...
    function updatePriority(priority) {
        fetch('{{ route('manager.helpdesk.conversation.update', $conversation) }}', {
            method: 'PUT',
            headers: {
