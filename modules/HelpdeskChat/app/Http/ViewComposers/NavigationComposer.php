...
namespace Modules\HelpdeskChat\Http\ViewComposers;
...
class NavigationComposer
{
    ...
    {
        // ✅ Check if HelpdeskChat module is enabled before showing navigation
        if (! ModuleStatusHelper::isModuleEnabled('HelpdeskChat')) {
            $view->with('chatNavigation', []);

            return;
        }

        $navigationConfig = config('helpdeskchat.navigation', []);

        $chatNavigationItems = $this->buildNavigationItems($navigationConfig);

        $view->with('chatNavigation', $chatNavigationItems);
    }
