...
class EvolutionApiProvider extends AbstractWhatsappProvider
{
    ...
    protected function handleMessageUpsert(array $data): void
    {
        try {
            ...
            $contact = $this->getOrCreateContact($phoneNumber, $contactName);
            $contactInbox = $this->getOrCreateContactInbox($contact, $phoneNumber);
            $conversation = $this->getOrCreateConversation($contact, $contactInbox);
    ...
    protected function handleContactsUpsert(array $data): void
    {
        try {
            ...
            $contact = $this->getOrCreateContact($phoneNumber, $contactName ?? $phoneNumber);
            ...
            Log::info('Contact upserted', ['phone' => $phoneNumber]);
        } catch (\Exception $e) {
    ...
    protected function getOrCreateContact(string $phoneNumber, string $name): Contact
    {
    ...
    protected function getOrCreateContactInbox(Contact $contact, string $sourceId): ContactInbox
    {
    ...
    protected function getOrCreateConversation(Contact $contact, ContactInbox $contactInbox): Conversation
    {
