@php
    try {
        $errorPageId = setting("error-page-429");
        $errorPage = $errorPageId
            ? \Modules\Page\Models\Page::find($errorPageId)
            : \Modules\Page\Models\Page::findByPageType("error_429");
    } catch (\Throwable) {
        $errorPage = null;
    }
@endphp
...
<head>
    ...
    <title>{{ $errorPage?->title ?? 'Error 429' }}</title>
...
<body>
    <div class="error-card">
        ...
        @else
            ...
            <h4 class="mt-3 mb-2 fw-bold">Error 429</h4>
