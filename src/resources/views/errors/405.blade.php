@php
    try {
        $errorPageId = setting("error-page-405");
        $errorPage = $errorPageId
            ? \Modules\Page\Models\Page::find($errorPageId)
            : \Modules\Page\Models\Page::findByPageType("error_405");
    } catch (\Throwable) {
        $errorPage = null;
    }
@endphp
...
<head>
    ...
    <title>{{ $errorPage?->title ?? 'Error 405' }}</title>
...
<body>
    <div class="error-card">
        ...
        @else
            ...
            <h4 class="mt-3 mb-2 fw-bold">Error 405</h4>
