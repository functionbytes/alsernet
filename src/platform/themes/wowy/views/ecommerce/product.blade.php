@php
    $layout = theme_option('product_layout') ?: 'product-full-width';
    Theme::layout($layout);
...
<div class="product-detail accordion-detail">
            ...
            <div class="detail-info mt-20">
                ...
                <div class="product-detail-rating">
                    @if ($product->brand_id)
                        <div class="pro-details-brand">
                            <span class="d-inline-block me-1">{{ __('Brands') }}:</span> <a href="{{ route('shop.brand', $product->brand?->slug) }}">{{ $product->brand?->name }}</a>
                        </div>
                    @endif
                    ...
                    @if (EcommerceHelper::isReviewEnabled())
                        ...
                        <div class="product-rate-cover text-end">
                            <div class="rating_wrap">
                                ...
                                <span class="rating_num">({{ __(':count reviews', ['count' => $reviewsCount]) }})</span>
                            </div>
                        </div>
                    @endif
                </div>
                <div class="clearfix product-price-cover">
                    <div class="product-price primary-color float-left">
                        <ins><span class="text-brand">{{ format_price($product->final_price) }}</span></ins>
                        @if ($product->final_price != $product->price)
                            <ins><span class="old-price font-md ml-15">{{ format_price($product->price) }}</span></ins>
                            <span class="save-price font-md color3 ml-15"><span class="percentage-off d-inline-block">{{ get_sale_percentage((float) $product->price, (float) $product->final_price) }}</span></span>
                        @endif
                    </div>
                </div>
                ...
                <form class="add-to-cart-form" method="POST" action="{{ route('cart.add', $product->id) }}">
                    ...
                    @if ($product->variations()->count() > 0)
                        ...
                        @endphp
                        <div class="number-items-available" style="@if (!$isOutOfStock) display: none; @endif margin-bottom: 10px;">
                            @if ($isOutOfStock)
                    ...
                    @php $isOutOfStock = ($product->quantity ?? 0) <= 0 && $product->with_storehouse_management; @endphp
                    @if (EcommerceHelper::isCartEnabled())
                        ...
                        <div class="detail-extralink">
                            @if (EcommerceHelper::isCartEnabled() && !$isOutOfStock)
                            ...
                            <div class="product-extra-link2 @if (EcommerceHelper::isQuickBuyButtonEnabled()) has-buy-now-button @endif">
                                ...
                                @if (EcommerceHelper::isWishlistEnabled())
                                    <a aria-label="{{ __('Add To Wishlist') }}" class="action-btn hover-up js-add-to-wishlist-button" data-url="{{ route('ecommerce.wishlist.store', $product->id) }}" href="#"><i class="fa fa-heart"></i></a>
                                @endif
                            </div>
                        </div>
                    @endif
                ...
                <ul class="product-meta font-xs color-grey mt-30">
                    ...
                    @if ($product->tags->isNotEmpty())
                        ...
                        @foreach($product->tags as $tag)
                            <a href="#" rel="tag" title="{{ $tag->name }}">{{ $tag->name }}</a>@if (!$loop->last),@endif
                        @endforeach
                        </li>
                    @endif

                    <li><span class="d-inline-block me-1">{{ __('Availability') }}:</span> <span class="in-stock text-success ml-5">{{ $isOutOfStock ? __('Out of stock') : __('In stock') }}</span></li>
                </ul>
            </div>
            <!-- Detail Info -->
    ...
    <div class="tab-style3">
        <ul class="nav nav-tabs text-uppercase">
            ...
            @if (EcommerceHelper::isProductSpecificationEnabled() && isset($product->specificationAttributes) && $product->specificationAttributes->where('pivot.hidden', false)->isNotEmpty())
                <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#tab-specification">{{ __('Specification') }}</a></li>
            @endif
            @if (EcommerceHelper::isReviewEnabled())
                <li class="nav-item">
                    <a class="nav-link" id="Reviews-tab" data-bs-toggle="tab" href="#Reviews">{{ __('Reviews') }} ({{ $reviewsCount ?? 0 }})</a>
                </li>
            @endif
            @if (is_plugin_active('faq'))
                @if (isset($product->faq_items) && count($product->faq_items) > 0)
                    <li class="nav-item">
        ...
        <div class="tab-content shop_info_tab entry-main-content">
            ...
            @if (is_plugin_active('faq') && isset($product->faq_items) && count($product->faq_items) > 0)
