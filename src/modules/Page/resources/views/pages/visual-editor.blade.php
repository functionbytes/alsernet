                        ...
                        <svg class="ve-stat-spark" viewBox="0 0 200 36" preserveAspectRatio="none">
                            <defs>
                                <linearGradient id="ecCg" x1="0" y1="0" x2="0" y2="1">
                                    <stop offset="0%" stop-color="#90bb13" stop-opacity=".3"/>
                                    <stop offset="100%" stop-color="#90bb13" stop-opacity="0"/>
                                </linearGradient>
                            </defs>
                            <path id="ve-chart-area" fill="url(#ecCg)"/>
                            <path id="ve-chart-line" fill="none" stroke="#90bb13" stroke-width="1.5" stroke-linejoin="round"/>
                        </svg>
                    </div>
                    <div class="vm-score-counts">
...
(function ($) {
                ...
                .done(function () {
                    ...
                    setTimeout(function () {
                        $btn.html('<i class="fa-solid fa-save me-1"></i>Guardar')
                            .css({ background: '#90bb13', 'border-color': '#90bb13' });
                    }, 2500);
                })
                .fail(function (xhr) {
    ...
    $('#ve-code-btn-theme').on('click', function () {
        ...
        $('#ve-code-apply').css({ background: veCodeDark ? '#444' : '#90bb13', 'border-color': veCodeDark ? '#444' : '#90bb13' });
    });
...
(function () {
    var BS_CLASSES = [
        ...
        'border-bottom','border-bottom-0','','-0',
        'border-primary','border-secondary','border-success','','border-warning','border-info','border-light','border-dark','border-white',
...
(function($){
    ...
    if (origAS && $saveBar.length) {
        new MutationObserver(function(){
            ...
            if (cls.indexOf('saving') !== -1) {
                ...
                $saveBar.html('<i class="fa-solid fa-times" style="color:#90bb13;"></i>');
    ...
    cmdActions.push({ cat:'Herramientas', label:'Comparar con versión guardada', icon:'fa-code-compare', action: function() {
        ...
        var html = '<div style="text-align:center;padding:12px;">' +
            ...
            '<div style="padding:12px 20px;background:#fce4ec;border-radius:8px;"><span style="font-size:20px;font-weight:700;color:#90bb13;">-' + removed + '</span><br><small style="color:#666;">líneas eliminadas</small></div>' +
    ...
    function updateSeoScore() {
        ...
        var ringColor = pct >= 80 ? '#43a047' : pct >= 60 ? '#333' : pct >= 40 ? '#f9a825' : '#90bb13';
    ...
    var bsClasses = [
        ...
        // Borders
        'border','border-0','border-top','border-bottom','','border-end',
    ...
    function checkBrokenSentinels() {
        ...
        if (broken.length > 0) {
            if (!$badge.length) {
                $('<span id="ve-broken-sc-badge" title="Shortcodes no registrados: ' + broken.join(', ') + '" style="font-size:10px;color:#90bb13;cursor:pointer;margin-left:6px;">&#9888; ' + broken.length + ' roto(s)</span>')
                    .appendTo('#ve-topbar');
            } else {
