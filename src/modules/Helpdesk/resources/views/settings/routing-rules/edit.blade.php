...
@section('title', 'Editar regla: ' . $routingRule->keyword)

@section('page_header')
    @include('core::components.card', ['title' => 'Editar regla de enrutamiento'])
@endsection
...
@section('content')
    ...
    <div class="row g-3">
        ...
        <div class="col-12 col-lg-8">
            <div class="card">
                <form action="{{ route('settings.helpdesk.routing-rules.update', $routingRule->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="card-header border-bottom p-3">
                        <h5 class="mb-0 fw-bold">Editar: {{ $routingRule->keyword }}</h5>
                        <small class="text-muted">Modifica las propiedades de la regla de enrutamiento</small>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <p class="text-muted small mb-3">Agente o equipo al que se asignara la conversacion</p>
                        <div class="row g-3 mb-4">
                                    ...
                                    <select name="assign_to_user_id" class="form-select @error('assign_to_user_id') is-invalid @enderror">
                                        ...
                                        @foreach($agents as $agent)
                                            <option value="{{ $agent->id }}"
                                                {{ old('assign_to_user_id', $routingRule->assign_to_user_id) == $agent->id ? 'selected' : '' }}>
                            ...
                            <div class="col-12 col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="priority"
                                           ...
                                           value="{{ old('priority', $routingRule->priority) }}"
                                           min="0" max="9999">
                            ...
                            <div class="col-12 col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select name="is_active" class="form-select">
                                        <option value="1" {{ old('is_active', $routingRule->is_active ? '1' : '0') === '1' ? 'selected' : '' }}>Activa</option>
                                        <option value="0" {{ old('is_active', $routingRule->is_active ? '1' : '0') === '0' ? 'selected' : '' }}>Inactiva</option>
                                    </select>
                                </div>
                            </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-text text-muted">
                        Cuando un cliente envia un mensaje que coincide con la palabra clave, la conversacion se asigna automaticamente al agente configurado.
                    </p>
                </div>
                <hr class="my-0">
                <div class="card-body">
                ...
                <div class="card-body">
                    ...
                    <ul class="list-unstyled mb-0">
                        <li class="mb-2 text-muted small">
                            <span class="fw-semibold">Creada:</span> {{ $routingRule->created_at->format('d/m/Y H:i') }}
                        </li>
                        <li class="text-muted small">
                            <span class="fw-semibold">Actualizada:</span> {{ $routingRule->updated_at->format('d/m/Y H:i') }}
                        </li>
                    </ul>
                </div>
            </div>
