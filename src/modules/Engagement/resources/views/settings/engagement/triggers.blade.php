    ...
    <div class="card mb-3">
        <div class="card-body d-flex align-items-center gap-3 flex-wrap">
            ...
            <div class="ms-auto d-flex gap-2">
                ...
                <button class="btn btn-primary" id="btnNewTrigger">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <div id="triggerConditionBuilder"></div>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <select id="actionType" class="form-select mb-2">
                            ...
                            <option value="show_message_in_chat">Mensaje automático en chat</option>
                            <option value="prefill_form">Pre-llenar formulario</option>
                            <option value="request_rating">Solicitar valoración</option>
                            <option value="inject_recommendations">Inyectar recomendaciones</option>
                        </select>
...
$(function () {
    const routes = {
        ...
        trashed:   '{{ route('settings.engagement.triggers.trashed') }}',
        restore:   (id) => '{{ route('settings.engagement.triggers.restore', ['id' => '__ID__']) }}'.replace('__ID__', id),
    };
    ...
    // ---- Condition Builder ----
    let triggerBuilder = null;

    function initTriggerBuilder(conditions) {
        if (triggerBuilder) {
            triggerBuilder.rules = conditions.rules ?? [];
            triggerBuilder.operator = conditions.operator ?? 'AND';
            triggerBuilder.render();
        } else {
            triggerBuilder = new ConditionBuilder('#triggerConditionBuilder', {
    ...
    function buildConditions() {
        const val = triggerBuilder ? triggerBuilder.getValue() : { operator: 'AND', rules: [] };
        return {
    ...
    function loadConditions(group) {
        initTriggerBuilder(group ?? { operator: 'AND', rules: [] });
    }
    ...
    $('#actionType').on('change', function () {
        ...
        } else if (t === 'show_message_in_chat') {
            html = `<textarea id="actionText" class="form-control" rows="3" placeholder="Mensaje del bot que aparecerá al abrir el chat"></textarea>
                    <small class="text-muted">El widget se abrirá automáticamente y mostrará este mensaje.</small>`;
        } else if (t === 'prefill_form') {
            html = `<textarea id="actionFields" class="form-control" rows="4" placeholder='{"name":"Juan","email":"juan@example.com"}'></textarea>
                    <small class="text-muted">JSON con los campos a pre-llenar en el pre-chat form.</small>`;
        } else if (t === 'inject_recommendations') {
            html = `<textarea id="actionProducts" class="form-control" rows="5" placeholder='[{"id":1,"name":"Producto","price":29.99,"image_url":"...","url":"..."}]'></textarea>
                    <small class="text-muted">Array JSON de productos a recomendar en el chat.</small>`;
        } else if (t === 'request_rating') {
            html = `<small class="text-muted">Sin parámetros — abre el formulario post-chat para que el visitante valore la conversación.</small>`;
        }
    ...
    function buildAction() {
        ...
        } else if (type === 'show_message_in_chat') {
            action.text = $('#actionText').val();
        } else if (type === 'prefill_form') {
            try {
                action.fields = JSON.parse($('#actionFields').val() || '{}');
            } catch (e) {
                action.fields = {};
            }
        } else if (type === 'inject_recommendations') {
            try {
                action.products = JSON.parse($('#actionProducts').val() || '[]');
            } catch (e) {
                action.products = [];
            }
        }
    ...
    function loadAction(action) {
        ...
        } else if (action.type === 'show_message_in_chat') {
            $('#actionText').val(action.text ?? '');
        } else if (action.type === 'prefill_form') {
            $('#actionFields').val(JSON.stringify(action.fields ?? {}, null, 2));
        } else if (action.type === 'inject_recommendations') {
            $('#actionProducts').val(JSON.stringify(action.products ?? [], null, 2));
        }
    }
