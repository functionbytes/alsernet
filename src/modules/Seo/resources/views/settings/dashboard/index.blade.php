...
@section('content')
    <div class="widget-content">
        ...
        $gradeConfig = [
            ...
            'F' => ['color' => '#90bb13', 'label' => 'Grado F (0-39)'],
        ];
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center gap-2">
                                                        <div class="progress progress-thin flex-fill" style="min-width:50px;height:4px;">
                                                            <div class="progress-bar" style="width:{{ round($issue['count'] / $maxIssueCount * 100) }}%;background:#90bb13;"></div>
                                                        </div>
                                                ...
                                                <tbody class="border-top">
                                                    @foreach($duplicateTitles as $dup)
                                                        <tr>
                                                            ...
                                                            <td class="text-end">
                                                                <span class="badge rounded-pill" style="background:{{ $dup->count > 3 ? '#90bb13' : '#e8e8e8' }};color:{{ $dup->count > 3 ? '#fff' : '#333333' }}">
                                                                    {{ $dup->count }}
                                                                </span>
                                                            </td>
                                                        </tr>
                            ...
                            <div class="tab-pane fade" id="tab-declining" role="tabpanel">
                                @forelse($declining as $page)
                                    <div class="d-flex align-items-center justify-content-between {{ !$loop->last ? 'mb-4' : '' }}">
                                        ...
                                        <h6 class="mb-0 fw-semibold" style="color:#90bb13;">{{ $page->score_change }}</h6>
                                    </div>
                                @empty
                                    <div class="text-center py-5">
                                        <div class="rounded-circle d-inline-flex align-items-center justify-content-center mb-3" style="width:56px;height:56px;background:#f5f6f8;">
...
(function () {
    ...
    new ApexCharts(document.querySelector('#spark-score'), sparkCfg(trendScores, '#90bb13', 'area')).render();
    ...
    ['#spark-score', '#spark-metas', '#spark-redirects', '#spark-og'].forEach(function (sel, i) {
        ...
        if (el) new ApexCharts(el, sparkCfg(staticData, ['#90bb13','#333333','#7b0000','#555555'][i], i % 2 === 0 ? 'area' : 'bar')).render();
    });
    @endif
