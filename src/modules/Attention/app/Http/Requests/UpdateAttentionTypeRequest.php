...
class UpdateAttentionTypeRequest extends FormRequest
{
    /**
    ...
    {
        $attentionTypeId = $this->route('attention_type') ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:100', Rule::unique('attention_types', 'name')->ignore($attentionTypeId)],
            'code' => ['required', 'string', 'max:10', Rule::unique('attention_types', 'code')->ignore($attentionTypeId)],
