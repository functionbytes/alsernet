...
class PriceLabelPdfService
{
    ...
    private function buildPages(PriceLabelTemplate $template, array $rows, string $orientation, array $canvas): array
    {
        ...
        $typeByKey = collect($template->field_definitions ?: $this->templateService->defaultFieldDefinitions())
            ->pluck('type', 'key')
            ->all();
        ...
        foreach (array_chunk($rows, $slotsPerPage) as $chunk) {
            ...
            foreach (array_values($chunk) as $index => $row) {
                $slots[] = $this->buildSlot($template, $row, $index + 1, $fields, $positions, $canvas, $orientation, $keys, $typeByKey);
            }
            $pages[] = $slots;
        }
    ...
    private function buildSlot(PriceLabelTemplate $template, array $row, int $slot, array $fields, array $positions, array $canvas, string $orientation, array $keys, array $typeByKey): array
    {
        ...
        foreach ($keys as $key) {
            ...
            $elements[$key] = [
                ...
                'font_family' => self::FONT_STACKS[$family] ?? self::FONT_STACKS['helvetica'],
