...
@section('title', 'Nuevo grupo de tickets')

@section('page_header')
    @include('core::components.card', ['title' => 'Nuevo grupo de tickets'])
@endsection
            ...
            <div class="card">
                <form id="groupForm" action="{{ route('manager.helpdesk.settings.ticket-groups.store') }}" method="POST">
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="name"
                                           ...
                                           value="{{ old('name') }}"
                                           placeholder="Ej: Soporte tecnico nivel 1"
                                           required>
                                    @error('name')
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <textarea name="description"
                                              ...
                                              rows="3"
                                              placeholder="Describe el proposito de este grupo">{{ old('description') }}</textarea>
                                    @error('description')
                                        <span class="field-validation-error"><i class="fas fa-circle-exclamation"></i> {{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('assignment_mode') is-invalid @enderror" id="assignment_mode" name="assignment_mode" required>
                                        <option value="manual" {{ old('assignment_mode', 'manual') == 'manual' ? 'selected' : '' }}>Manual — un agente asigna tickets</option>
                                        <option value="round_robin" {{ old('assignment_mode', 'manual') == 'round_robin' ? 'selected' : '' }}>Round robin — rotacion entre miembros</option>
                                        <option value="load_balanced" {{ old('assignment_mode', 'manual') == 'load_balanced' ? 'selected' : '' }}>Balanceo de carga — menos tickets abiertos recibe primero</option>
                                    </select>
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-12">
                                <div id="members-container">
                                    @foreach(old('users', []) as $index => $userId)
                                        ...
                                        @if($user)
                                            <div class="member-row d-flex align-items-center gap-2 mb-2">
                                                ...
                                                <span class="flex-fill form-control-plaintext fw-semibold">{{ $user->firstname }} {{ $user->lastname }}</span>
                                                <select name="user_priorities[]" class="form-select form-select-sm w-auto">
                                                    <option value="primary" {{ (old('user_priorities.' . $index, 'primary') === 'primary') ? 'selected' : '' }}>Principal</option>
                                                    <option value="backup" {{ (old('user_priorities.' . $index) === 'backup') ? 'selected' : '' }}>Respaldo</option>
                                                </select>
                            ...
                            <div class="col-12 col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('is_default') is-invalid @enderror" id="is_default" name="is_default">
                                        <option value="0" {{ old('is_default', '0') == '0' ? 'selected' : '' }}>No — asignacion explicita</option>
                                        <option value="1" {{ old('is_default', '0') == '1' ? 'selected' : '' }}>Si — asignado por defecto a tickets sin grupo</option>
                                    </select>
                                    @error('is_default')
                                        <span class="field-validation-error"><i class="fas fa-circle-exclamation"></i> {{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-12 col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('is_active') is-invalid @enderror" id="is_active" name="is_active">
                                        <option value="1" {{ old('is_active', '1') == '1' ? 'selected' : '' }}>Activo — disponible para asignar</option>
                                        <option value="0" {{ old('is_active', '1') == '0' ? 'selected' : '' }}>Inactivo — no disponible</option>
                                    </select>
                                    @error('is_active')
                                        <span class="field-validation-error"><i class="fas fa-circle-exclamation"></i> {{ $message }}</span>
                                    @enderror
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary w-100 mb-1">Guardar grupo</button>
