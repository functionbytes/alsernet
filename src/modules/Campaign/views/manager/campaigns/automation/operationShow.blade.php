...
<p>{!! trans('messages.automation.operation.' .$element->getOption('operation_type'). '.desc', [
    'list_name' => $element->getOption('target_list_uid') ? \Modules\Campaign\Models\MailList::findByUid($element->getOption('target_list_uid'))->name : '',
    ...
    <table class="table my-3" style="border-bottom: solid 1px #ddd">
        ...
        <tbody>
            @foreach($element->getOption('update') as $update)
                <tr>
                    <th scope="row">{{ \Modules\Campaign\Models\Field::findByUid($update->field_uid)->label }}</th>
                    <td>{{ $update->value }}</td>
                </tr>
            @endforeach
        </tbody>
