...
use Modules\Campaign\Jobs\UpdateAutomation;
...
// use Modules\Mail\ModelsList; // legacy
...
class Automation extends Model
{
    ...
    /**
     * Scan TODAS las automatizaciones activas y comprueba triggers.
     *
     * Se llama desde DispatchAutomationJobsCommand cada 5 minutos. En este
     * destino no-SaaS no hay Customer/Subscription scope: las automatizaciones
     * son globales y se procesan todas.
     *
     * Cada automation se procesa con un lock por uid para evitar que dos
     * cron concurrentes la dupliquen.
     */
    public static function run(): void
    {
        $automations = static::query()->where('status', 'active')->get();

        foreach ($automations as $automation) {
            $lockFile = storage_path('tmp/automation-lock-'.$automation->uid);
            ...
            $lock->getExclusiveLock(
                function ($f) use ($automation): void {
                    try {
                        $automation->logger()->info(sprintf('Checking automation "%s"', $automation->name));
                        $automation->check();
                        $automation->update(['last_executed_at' => now()]);
                    } catch (Throwable $e) {
                        $automation->logger()->error('Error: '.$e->getMessage());
                        $automation->update(['last_error' => $e->getMessage()]);
                    }
                },
                5, // timeout seg
                fn () => $automation->logger()->info("LOCK timeout, otra ejecución activa para {$automation->name}"),
            );
        }
    }
    ...
    public function checkForNewTriggers()
    {
        ...
        switch ($triggerName) {
            ...
            case 'others':
                ...
                break;
            default:
                throw new Exception(sprintf('[Automation: `%s`, Customers account: `%s`] Unknown Automation trigger type: %s', $this->name, $this->customer->uid, $triggerName));
        }
        $this->updateCache();
    ...
    {
        if (is_null(config('app.pages')) || config('app.pages') == false) {
    ...
    public function createFromArray($params)
    {
        ...
        $this->mail_list_id = ModelsList::findByUid($params['mail_list_uid'])->id;
