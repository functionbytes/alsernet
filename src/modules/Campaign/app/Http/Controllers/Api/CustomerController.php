...
use Modules\Campaign\Library\Facades\Billing;
use Modules\Campaign\Library\TransactionResult;
use Modules\Campaign\Models\PlanGeneral;
use Modules\Campaign\Models\User;
...
class CustomerController extends Controller
{
    ...
    {
        $customers = \Modules\Campaign\Models\Customer::select('customers.*')
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        $customer = \Modules\Campaign\Models\Customer::newCustomer();
        $user = new User;
    ...
    public function update(Request $request, $uid)
    {
        // Get current user
        $current_user = \Auth::guard('api')->user();
        $customer = \Modules\Campaign\Models\Customer::findByUid($uid);

        // check if item exists
        if (! $customer) {
            return \Response::json(['status' => 0, 'message' => 'Customers not found'], 404);
        }
    ...
    public function assignPlan(Request $request, $uid, $plan_uid)
    {
        $user = \Auth::guard('api')->user();
        $customer = \Modules\Campaign\Models\Customer::findByUid($uid);
        $offlineGateway = Billing::getGateway('offline');
        ...
        $plan = PlanGeneral::findByUid($plan_uid);
        ...
        if ($request->disable_billing && $request->disable_billing !== 'false') {
            ...
            if (! $offlineGateway->isActive() || ! Billing::isGatewayEnabled($offlineGateway)) {
