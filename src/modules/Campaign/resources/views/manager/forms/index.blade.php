...
@section('title', trans('campaign::forms.page.title'))

@section('page-header')
    <div class="mc-page-header">
        <div>
            <h1 class="mc-page-title">{{ trans('campaign::forms.page.title') }}</h1>
            <p class="mc-page-subtitle">{{ trans('campaign::forms.page.subtitle') }}</p>
        </div>
        <div class="mc-page-actions">
            <a href="{{ route('manager.forms.create') }}" class="mc-btn mc-btn-primary mc-btn-sm">
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:var(--space-4);margin-bottom:var(--space-5);">
    <div class="mc-stat-card">
        ...
        <div class="mc-stat-content">
            <div class="mc-stat-label">{{ trans('campaign::forms.stat.total') }}</div>
    ...
    <div class="mc-stat-card">
        ...
        <div class="mc-stat-content">
            <div class="mc-stat-label">{{ trans('campaign::forms.stat.published') }}</div>
    ...
    <div class="mc-stat-card">
        ...
        <div class="mc-stat-content">
            <div class="mc-stat-label">{{ trans('campaign::forms.stat.draft') }}</div>
...
<div class="mc-alert mc-alert-teal" style="margin-bottom:var(--space-5);">
    ...
    <div class="mc-alert-content">
        <div class="mc-alert-title">{{ trans('campaign::forms.banner.title') }}</div>
        <div class="mc-alert-text">{{ trans('campaign::forms.banner.desc') }}</div>
    </div>
</div>
    ...
    <div class="mc-filter-bar">
        <div class="mc-filter-search">
            ...
            <input class="mc-form-input mc-search-input" type="text" name="keyword"
                   placeholder="{{ trans('campaign::forms.filter.search') }}"
                   data-list-filter="keyword">
        </div>
        <select class="mc-form-input mc-form-select mc-select-inline" name="status" data-list-filter="status">
            <option value="">{{ trans('campaign::forms.filter.status_all') }}</option>
            <option value="published">{{ trans('campaign::forms.status.published') }}</option>
            <option value="draft">{{ trans('campaign::forms.status.draft') }}</option>
        </select>
    </div>

    <div id="forms-list"
         data-url="{{ route('manager.forms.listing') }}"
...
document.addEventListener('DOMContentLoaded', function() {
    var container = document.getElementById('forms-list');
    if (!container) return;
    ...
    document.addEventListener('click', function(e) {
        ...
        if (deleteBtn) {
            ...
            window.McDialog.confirm({
                title: '{{ trans("campaign::forms.action.delete") }}',
                message: '{{ trans("campaign::forms.empty.desc") }}',
                type: 'danger',
                confirmText: '{{ trans("campaign::forms.action.delete") }}',
                onConfirm: function() {
