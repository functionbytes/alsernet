...
@section('title', trans('campaign::email-templates.create.title'))

@section('page-header')
    <div class="mc-page-header">
        <div>
            <h1 class="mc-page-title">{{ trans('campaign::email-templates.create.title') }}</h1>
            <p class="mc-page-subtitle">{{ trans('campaign::email-templates.create.subtitle') }}</p>
        </div>
        <div class="mc-page-actions">
            <a href="{{ route('manager.my_email_templates.index') }}" class="mc-btn mc-btn-secondary mc-btn-sm">
                @include('campaign::refactor.components.icons.mc-icon', ['icon' => 'arrow-left', 'size' => 16])
                {{ trans('campaign::email-templates.create.back_to_index') }}
            </a>
        </div>
    </div>
@endsection
...
@section('content')
    <form id="admin-form-template-create-form" method="POST" action="{{ route('manager.my_email_templates.store') }}">
        @csrf
        ...
        <div class="mc-card" style="margin-bottom:var(--space-5);">
            <div class="mc-card-header">
                <h3 class="mc-card-title">
                    ...
                    {{ trans('campaign::email-templates.create.name_card_title') }}
                </h3>
            </div>
            <div style="padding:0 var(--space-5) var(--space-5);">
                <input type="text"
                       ...
                       class="mc-form-input"
                       placeholder="{{ trans('campaign::email-templates.fields.name_placeholder') }}"
                       value="{{ old('name', trans('campaign::email-templates.fields.name_placeholder')) }}"
                       autofocus
                       required>
        ...
        <div class="mc-card mc-card-flush" style="margin-bottom:var(--space-5);">
            <div class="mc-card-header">
                <div>
                    <h3 class="mc-card-title">
                        ...
                        {{ trans('campaign::email-templates.create.gallery_card_title') }}
                    </h3>
                    <p class="mc-card-subtitle">{{ trans('campaign::email-templates.create.preview_hint') }}</p>
                </div>
            ...
            <div id="admin-form-tpl-gallery"
                 data-gallery-url="{{ route('manager.my_email_templates.gallery') }}"
                ...
                <div style="padding:var(--space-12) var(--space-6);text-align:center;color:var(--color-text-muted);">
                    ...
                    <div style="margin-top:var(--space-3);font-size:var(--text-sm);">
                        {{ trans('campaign::email-templates.create.loading') }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mc-form-actions" style="display:flex;justify-content:flex-end;gap:var(--space-3);">
            <a href="{{ route('manager.my_email_templates.index') }}" class="mc-btn mc-btn-secondary">
                {{ trans('campaign::email-templates.buttons.cancel') }}
            </a>
            <button type="submit" id="admin-form-tpl-submit" class="mc-btn mc-btn-primary" disabled>
                ...
                {{ trans('campaign::email-templates.create.submit') }}
            </button>
        </div>
    </form>
@endsection
...
(function() {
    ...
    function loadGallery(page) {
        ...
        galleryEl.innerHTML = '<div style="padding:var(--space-12) var(--space-6);text-align:center;color:var(--color-text-muted);"><div class="mc-spinner"></div><div style="margin-top:var(--space-3);font-size:var(--text-sm);">{{ trans('campaign::email-templates.create.loading') }}</div></div>';
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!selectedInput.value) {
            ...
            err.textContent = '{{ trans('campaign::email-templates.create.select_first') }}';
        ...
        .then(function(res) {
            if (res.status === 422 && res.data.errors) {
                ...
                window.location.href = res.data.redirect || '{{ route('manager.my_email_templates.index') }}';
            } else {
                submitBtn.disabled = false;
            }
        })
