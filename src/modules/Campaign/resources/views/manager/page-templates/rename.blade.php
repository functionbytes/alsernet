...
@section('content')
<form id="form-template-rename-form" action="{{ route('manager.page_templates.update_name', $item->uid) }}" method="POST">
    @csrf
    ...
    <div class="mc-alert mc-alert-teal" style="margin-bottom:var(--space-5);">
        ...
        <div class="mc-alert-content">
            <div class="mc-alert-text">{{ trans('campaign::page-templates.rename.desc') }}</div>
        </div>
    </div>

    <div class="mc-form-group">
        <label class="mc-form-label">{{ trans('campaign::page-templates.fields.name') }}</label>
        <input type="text" name="name" class="mc-form-input" value="{{ $item->name }}" autofocus>
    ...
    <div class="mc-confirm-actions" style="margin-top:var(--space-5);">
        <button type="button" class="mc-btn mc-btn-secondary" onclick="McPopup.close()">{{ trans('campaign::page-templates.buttons.cancel') }}</button>
...
<script>
(function() {
    var form = document.getElementById('form-template-rename-form');
    if (!form) return;
