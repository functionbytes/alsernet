...
class SendCartRecoveryMailJob implements ShouldQueue
{
    use Queueable;
    ...
    public function __construct(
        ...
        private readonly array $cartPayload,
    ) {
    ...
    public function handle(): void
    {
        ...
        try {
            Mail::to($customer->email)->send(new CartRecoveryMail($customer, $this->cartPayload));
            $this->logTrigger($customer, null, true);
        } catch (\Throwable $e) {
            Log::warning('SendCartRecoveryMailJob failed', [
    ...
    {
        Log::error('SendCartRecoveryMailJob permanently failed', [
    ...
    private function logTrigger(Customer $customer, ?string $skipReason, bool $sent = false): void
    {
        DB::table('remarketing_automation_triggers_log')->insert([
            ...
            'customer_id' => $customer->id,
            'context' => json_encode($this->cartPayload),
            'email_sent' => $sent,
