...
class ErpService
{
    ...
    private function construirDatosPedido(int $idPedido, ?string $idClienteGestion): ?array
    {
        ...
        // Por ejemplo: Order, Customers, Address, etc.
    ...
    {
        try {
            // Bundle key + per-key Setting caches
            Setting::clearErpSettingsCache();

            // Best-effort SCAN/DEL for the `erp_*` namespace (Redis store only).
            // For non-Redis cache stores there's no portable wildcard delete; the
            // per-key forgets above already invalidate the hot path.
            ...
            if (method_exists($store, 'connection')) {
                ...
                $cursor = null;
                do {
                    [$cursor, $keys] = $redis->scan($cursor ?? 0, ['match' => $prefix.'erp_*', 'count' => 200]);
                    if (! empty($keys)) {
