...
class UpdateSubscriberRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('mailrelay.subscribers.update');
    }

    public function rules(): array
    {
        $id = $this->route('id') ?? $this->route('subscriber');

        return [
            'email' => ['required', 'email', Rule::unique('mails_subscribers', 'email')->ignore($id)],
