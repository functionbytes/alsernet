    ...
    $(function () {
        ...
        $(document).on('click', '.bv-conv', function (e) {
            ...
            if (window.innerWidth < 1024 && url) {
                $('.inbox-v4').attr('data-bv-mobile-tab', 'thread');
        ...
        $(document).on('click', '.bv-mobile-tab', function () {
            ...
            $('.inbox-v4').attr('data-bv-mobile-tab', tab);
            $('.bv-mobile-tab').removeClass('on');
            $(this).addClass('on');
        });
        ...
        function archiveCurrentConversation() {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    toastr && toastr.success(resp.message || 'Conversacion archivada');
                })
                .fail(function (xhr) {
        ...
        function appendBubbleToThread(item, isInternal) {
            ...
            if (urls.length) {
                ...
                urls.forEach((u, i) => {
                    attachmentsHtml += buildAttachmentHtml(u, metas[i] || {});
                });
                attachmentsHtml += '</div>';
            }
        ...
        $(document).on('click', '[data-bv-action="archive"]', function (e) {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    ...
                    if (window.toastr) {
                        toastr.success(resp.message || 'Conversación archivada');
                    }
                })
                .fail(function (xhr) {
        ...
        $(document).on('click', '[data-bv-apply]', function () {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    ...
                    closeModal($modal);
                    if (window.toastr) {
                        toastr.success(resp.message || 'Actualizado');
                    }
                })
                .fail(function (xhr) {
        ...
        $(document).on('click', '[data-bv-modal-name="assign"] #assign-btn-notify, [data-bv-modal-name="assign"] #assign-btn-silent', function () {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    closeModal($modal);
                    toastr && toastr.success(resp.message || 'Asignado correctamente');
                })
                .fail(function (xhr) {
        ...
        $(document).on('click', '[data-bv-modal-name="close-conv"] #bv-close-apply', function () {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    ...
                    closeModal($modal);
                    toastr && toastr.success(resp.message || 'Conversación cerrada');
                })
                .fail(function (xhr) {
        ...
        $(document).on('click', '#bv-btn-reopen', function () {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    toastr && toastr.success(resp.message || 'Conversación reabierta');
        ...
        $(document).on('click', '[data-bv-modal-name="tags"] #bv-tags-apply', function () {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    closeModal($modal);
                    toastr && toastr.success(resp.message || 'Etiquetas actualizadas');
                })
                .fail(function (xhr) {
        ...
        function ajaxConversationUpdate(payload, successMsg) {
            ...
            return $.ajax({
                ...
                if (window.toastr) toastr.success(resp?.message || successMsg || 'Actualizado');
            }).fail(function (xhr) {
        ...
        $(document).on('click', '#bv-close-apply', function () {
            ...
            $.ajax({
                ...
                if (window.toastr) toastr.success(resp?.message || 'Conversación cerrada');
        ...
        $(document).on('drop', '[data-bv-droptarget="team"]', function (e) {
            ...
            $.ajax({
                ...
                .done(function () {
                    if (window.toastr) toastr.success('Conversación movida al equipo');
        ...
        $(document).on('click', '.bv-bubble-menu-item', function () {
            ...
            switch (action) {
                ...
                case 'copy':
                    ...
                    break;
                case 'react':
                    // Foco al primer reaction picker existente del bubble; si no, lanza emoji picker
                    $bubble.find('.bv-reaction').first().trigger('click');
                    break;
                case 'forward':
                    if (window.toastr) toastr.info('Función "Reenviar" disponible próximamente');
                    break;
                case 'info':
                    if (window.toastr) toastr.info('Mensaje #' + id + ' · ' + author + (body ? ' · ' + preview : ''), '', { timeOut: 5000 });
                    break;
                case 'delete':
        ...
        async function uploadFiles(files) {
            ...
            try {
                ...
                if (window.toastr) toastr.success(resp?.message || 'Archivos subidos');
        ...
        $(document).on('click', '[data-bv-attach-type]', function (e) {
            ...
            if (type === 'contact' || type === 'location') {
                ...
                if (window.toastr) toastr.info('Usa "Tienda" para compartir información de la empresa');
                return;
            }
        ...
        $(document).on('click', '.bv-mic-denied-copy, .bv-mic-denied-url', async function () {
            ...
            try {
                ...
                if (window.toastr) toastr.success('Copiado', '', { timeOut: 1500 });
            } catch (e) {
                if (window.toastr) toastr.info('Cópialo manualmente: ' + text, '', { timeOut: 6000 });
            }
        });
        ...
        $(document).on('click', '#bv-mic-denied-retry', async function () {
            try {
                ...
                stream.getTracks().forEach(t => t.stop());
                $('#bv-mic-denied').remove();
                if (window.toastr) toastr.success('Micrófono activado. Vuelve a hacer clic en "Grabar audio"');
            } catch (e) {
                if (window.toastr) {
        ...
        $(document).on('click', '.bv-forward-target', async function () {
            ...
            try {
                ...
                if (window.toastr) toastr.success('Archivo reenviado');
                $modal.remove();
                $('body').css('overflow', '');
            } catch (e) {
        ...
        $(document).on('click', '#bv-toggle-notifications', async function (e) {
            ...
            if (Notification.permission === 'granted') {
                if (window.toastr) toastr.info('Notificaciones ya activadas');
                return;
            }
            ...
            if (result === 'granted') {
                ...
                if (window.toastr) toastr.success('Notificaciones activadas');
        ...
        (function injectFocusModeStyles() {
            ...
            var css = [
                ...
                'body.bv-focus-mode .inbox-v4 { grid-template-columns: var(--bv-nav-w, 240px) 1fr !important; }',
        ...
        var BvTheme = (function () {
            ...
            function init() {
                ...
                // now set the definitive data-theme on .inbox-v4)
                applyTheme(isDark());
        ...
        $(document).on('click', '#bv-save-view-confirm', function () {
            ...
            $.ajax({
                ...
                if (window.toastr) toastr.success(resp.message || 'Vista guardada');
                $('#bv-save-view-modal').removeClass('on').hide();
        ...
        $(document).on('click', '.bv-nav-view-del', function (e) {
            ...
            $.ajax({
                ...
                $item.remove();
                if (window.toastr) toastr.success('Vista eliminada');
            }).fail(function () {
