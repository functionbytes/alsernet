 ...
 *   'fileupload:uploaded' — bytes finished uploading, server still processing (detail: { file })
...
class McFileUpload {
    ...
    _uploadFile(file) {
        ...
        // Bytes finished uploading; server is now processing. Fires BEFORE the
        // response arrives, so consumers can show a "please wait" overlay
        // during the post-upload server-side work (esp. for large patch files).
        xhr.upload.addEventListener('load', function() {
            self.el.dispatchEvent(new CustomEvent('fileupload:uploaded', { detail: { file: file } }));
        });
        ...
        xhr.addEventListener('load', function() {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                try {
                    ...
                    if (xhr.status === 413) {
                        msg = 'File too large — rejected by web server (HTTP 413). Check nginx client_max_body_size / Apache LimitRequestBody.';
                    } else if (xhr.status === 0) {
                        msg = 'Upload aborted — connection dropped before response.';
                    } else if (xhr.status >= 500) {
                        msg = 'Upload failed (HTTP ' + xhr.status + ') — request likely exceeded an Apache mod_fcgid / nginx body-size limit before PHP could run. Check the server error log.';
                    } else {
