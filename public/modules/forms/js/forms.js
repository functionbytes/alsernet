...
(function($) {
    ...
    function initForm(formId, config) {
        ...
        /* El artefacto de cada formulario lleva su propia copia del motor, y el
           arranque recorre TODAS las claves de FormsConfig. En una página con
           dos formularios (la ficha de producto lleva "Consulta de precio" y
           "Consulta a un experto") el motor corre dos veces y cada pasada
           inicializa los dos: los manejadores quedaban duplicados -- 4 de
           submit y 10 de click por formulario en vez de la mitad.
           El envío no se duplicaba porque jQuery Validate devuelve el validador
           que ya existe en la segunda llamada a .validate(), pero cualquier
           manejador que no sea idempotente sí se disparaba dos veces. */
        if ($form.data('formsInitialised')) return;
        $form.data('formsInitialised', true);
    ...
    function initSelect2($form) {
        ...
        $form.find('select').each(function() {
            ...
            // Accessible name: Select2 hides the original <select> (which has a
            // real <label for="...">) behind a custom widget that screen readers
            // and AI agents can't associate with that label. Copy the label text
            // onto the visible, focusable .select2-selection element instead.
            var $selection = $select.next('.select2-container').find('.select2-selection');
            if ($selection.length) {
                var labelText = '';
                if ($select.attr('id')) {
                    labelText = $('label[for="' + $select.attr('id') + '"]').clone().find('.text-danger').remove().end().text().trim();
                }
                if (!labelText) { labelText = $select.attr('aria-label') || $select.attr('title') || placeholder || ''; }
                if (labelText) { $selection.attr('aria-label', labelText); }
            }

            $select.on('change.select2validate', function() {
                if (!$select.closest('form').data('validator')) return;
                // Select2 dispara un `change` propio sobre el <select> oculto al
                // inicializarse: sin este guard, todos los desplegables
                // obligatorios se marcaban en rojo nada mas cargar la pagina,
                // antes de que el visitante tocara nada. Se revalida cuando ya
                // hay valor, o cuando el campo venia marcado (para poder
                // limpiar el error).
                if (!$select.val() && !$select.hasClass('is-invalid')) return;
                $select.valid();
            });
        });
    }
    ...
    /**
     * Mensajes de jQuery Validate por idioma.
     *
     * En la tienda estos mensajes salían en el idioma correcto de rebote: el
     * único sitio que hacía `$.extend($.validator.messages, ...)` era el bloque
     * de cada formulario *legacy* de scripts.js, leyendo su propio input oculto
     * `_alsernetforms_language`. Nuestros formularios no tienen ese input ni
     * pasan por esos bloques, así que heredaban lo que hubiera dejado puesto
     * otro formulario de la página (el de la newsletter del pie, normalmente).
     * En una página donde no hubiera ninguno, salían en inglés.
     *
     * Aquí van los seis idiomas de la tienda: es/en/fr/pt con la redacción de
     * su propio messages.js, de/it con la localización oficial de jQuery
     * Validate, que esos dos no estaban declarados en ninguna parte.
     */
    ...
    /**
     * Idioma real de la página. En la tienda manda PrestaShop (el artefacto
     * guarda un solo JS para los seis idiomas); fuera de ella, la configuración
     * del formulario.
     */
    function resolveLocale(config) {
        const tienda = (window.prestashop && window.prestashop.language && window.prestashop.language.iso_code) || '';
        return (tienda || config.locale || 'es').toLowerCase().slice(0, 2);
    }

    function applyValidationMessages(config) {
        const messages = VALIDATION_MESSAGES[resolveLocale(config)];

        if (!messages || !$.validator) {
            return;
        }

        // Los que llevan {0} necesitan $.validator.format, no una cadena suelta.
        const traducidos = {};
        $.each(messages, function(regla, texto) {
            traducidos[regla] = texto.indexOf('{0}') === -1 ? texto : $.validator.format(texto);
        });

        $.extend($.validator.messages, traducidos);
    }

    function initValidation($form, config) {
        applyValidationMessages(config);
    ...
    /**
     * Resuelve una ruta relativa contra la raiz de la tienda.
     *
     * No vale dejar que el navegador la resuelva contra la pagina actual: en
     * /module/alsernetforms/form?key=x acabaria pidiendo
     * /module/alsernetforms/modules/alsernetforms/... y dando 404. PrestaShop
     * publica su base real en window.prestashop.urls.base_url, que ademas es
     * correcta con instalaciones en subdirectorio y con multitienda.
     */
    ...
    function handleSubmit($form, config) {
        ...
        const isStore = config.platform === 'prestashop';
        ...
        // El enrutador de la tienda (alsernetforms/controllers/routes.php) espera
        // 'action' e 'iso' como campos del cuerpo, no en la URL. El resto de
        // formularios del modulo hacen exactamente este mismo append.
        if (isStore) {
            formData.append('action', config.formKey);
            // El idioma sale de la propia página, no de la configuración: el
            // artefacto guarda un solo JS para los seis idiomas (solo el HTML
            // se compila por idioma), así que hornear aquí el locale haría que
            // un visitante inglés enviase iso=es y recibiera la respuesta en
            // español.
            formData.append('iso', resolveLocale(config));
        }
        ...
        function genericError(message) {
            const text = message || 'Ha ocurrido un error al enviar el formulario. Por favor, inténtalo de nuevo.';
        ...
        $.ajax({
            url: isStore ? storeUrl(config.submitUrl) : config.submitUrl,
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            dataType: 'json',
            success: function(res) {
                // La tienda responde SIEMPRE con HTTP 200 y el resultado real en
                // `status`; sin esto, un envio rechazado (reCAPTCHA invalido,
                // email mal, formulario desactivado) se celebraria como exito.
                if (isStore && res && res.status && res.status !== 'success') {
                    restoreButton();
    ...
    function launchConfetti() {
        ...
        const colors = ['#F5B754', '#13C672', '#FEC90F', '#FA896B'];
    ...
    function initEstimateEnhancements($form) {
        ...
        $form.on('click', '.estimate-wa-submit', function() {
            var t = '\ud83c\udfe0 *Solicitud de cotizaci\u00f3n \u2014 INOQUALAB*\n\n';
